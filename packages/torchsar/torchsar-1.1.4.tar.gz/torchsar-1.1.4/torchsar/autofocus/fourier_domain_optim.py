...
def af_ffo_sm(x, p=2, niter=10, delta=None, toli=1e-2, tolo=1e-2, ftshift=True, islog=False):
    ...
    So = __fdnorm(tb.ematmul(X, ephi), p)
    ...
    while(ii < niter):
        ...
        while True:
            ...
            for a in range(Na):
                ...
                S1 = __fdnorm(tb.ematmul(X, ephi1), p)
                ephi2 = th.cat((th.cos(phi2), -th.sin(phi2)), dim=-1)
                S2 = __fdnorm(tb.ematmul(X, ephi2), p)
...
def af_ffo(g, w=None, p=2, niter=10, eta=0.5, tol=1e-2, ftshift=False):
    ...
    G = tb.fft(g, axis=-3, shift=ftshift)
    ...
    g = tb.ifft(G, axis=-3, shift=ftshift)
    return g
