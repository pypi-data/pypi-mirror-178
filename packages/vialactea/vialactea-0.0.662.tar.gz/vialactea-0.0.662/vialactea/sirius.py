    ...
    class qr(qrcode.QRCode):
        ...
        def create(self):
            ...
            if self.current_data == '' or None:
                print(f'Current data is empty: {self.current_data}')
        ...
        def create_from_import(self):
            ...
            for line in lines:
                ...
                try:
                    ...
                    print(f'{file_name} saved in {self.export_src}')
                except NameError:
                    print(NameError)
                self.reset_data()
        ...
        def format_upper_list():
            ...
            if user_resp.upper() == 'S':
                final_formula = ''
                with open(os.path.join(__bifrost.vialactea_import, "formula.txt"), 'r',
                          encoding='utf8') as formula_file:
                    formula_file_r = formula_file.readlines()
                    for line in formula_file_r:
                        final_formula += f'{line.rstrip().upper()}, '

                try:
                    with open(os.path.join(__bifrost.vialactea_export, f"{sku_name}_parsed_formula.txt"), 'w',
                              encoding='utf8') as formula_export:
