    ...
    def send(self, data):
        if data:
            if isinstance(data, str):
                data=bytearray().extend(data.encode())
...
class Server(Communicator):
    ...
    def stateMachineManager(self):
        ...
        if self._state==0:
            ...
            self.logger.debug('WAITING FOR <%s>' % self._controlEquipmentAddress)
        # --------------------------------------
        # wait for '1'
        elif self._state==1:
            if self.waitChar(self._controlEquipmentAddress):
                ...
                self.logger.debug('<%s>OK, WAITING FOR <ENQ>' % self._controlEquipmentAddress)
        # --------------------------------------
        # wait for 'ENQ'
        elif self._state==2:
