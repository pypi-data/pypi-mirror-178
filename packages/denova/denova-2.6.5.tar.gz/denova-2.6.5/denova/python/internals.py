...
def caller(ignore=None, this_module_valid=False):
    ...
    if ignore:
        ...
        _ignore = []
    # ignore denova.python.internals unless this_module_valid=True
...
def is_package_type(object):
    '''
        ...
        >>> is_package_type(denova.python.internals)
        False
    '''
...
def is_module_type(object):
    ''' Returns True if object is a python module, else False.
        ...
        True
        >>> import denova.python.internals
        >>> is_module_type(denova.python.internals)
        True
    '''
...
def import_file(name, path):
    '''
        ...
        <<< module = import_file(__file__, '/usr/local/lib/python3.9/dist-packages/denova/os/user.py')
        <<< print(str(module))
        <module 'python.py' from '/usr/local/lib/python3.9/dist-packages/denova/os/user.py'>
    '''
...
def get_module(name):
    ''' Get the module based on the module name.
        ...
        >>> module_str.startswith("<module 'denova.python.internals'") or module_str.startswith("<module '__main__' from")
        True
...
def exec_trace(code, ignoredirs=[sys.prefix, sys.exec_prefix], globals=None, locals=None, coverdir='/tmp'):
    ''' Trace code.
        ...
        Example:
            from denova.python.internals import exec_trace
            exec_trace("""
...
def pdb_break():
    ...
    Usage:
        from denova.python.internals import pdb_break ; pdb_break()
    '''
...
def winpdb_break():
    ''' Breakpoint for winpdb debugger.
        ...
        Example:
            from denova.python.internals import winpdb_break; winpdb_break() #DEBUG
    '''
...
def log_message(message):
    '''
        ...
        use denova.python.log in this module
    '''
    _debug = False
    if _debug:
        print(msg)
