...
def run(*command_args, **kwargs):
    ''' Run a command line.
        ...
        input and check params, we don't use Popen() for foreground
    ...
    try:
        ...
        log(f'CalledProcessError format_exc(): {format_exc(chain=True)}') # DEBUG
...
def _init_log():
    ...
    if log is None:
        ...
        from denova.python.log import Log    # pylint: disable=import-outside-toplevel
        log = Log()
