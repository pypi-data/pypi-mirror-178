...
export class DialogRoot {
    static use(parentCB) {
        return {
            provide: DialogRoot,
            useFactory(injector) {
                const inc = new DialogRoot(injector)
                inc.parent = parentCB(injector)
                dialogParent.push(inc.parent)
                return inc
            },
            lifecycle: true
        }
    ...
    diCreated(vm) {
        if(!this.parent) {
            if(this.injector.get(Mute(Dialog))) {
    ...
    diDestroyed(vm) {
        dialogParent.pop(this.parent || vm)
    }
}
