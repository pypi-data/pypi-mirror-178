...
from servicefoundry.lib.model.entity import DeviceCode, TenantInfo, Token
from servicefoundry.logger import logger


class AuthServiceClient:
    def __init__(self, auth_server: str, tenant_name: str):
        self.auth_server = auth_server
        self._tenant_name = tenant_name

    def get_token_from_api_key(self, api_key: str) -> Token:
        url = f"{self.auth_server}/api/{VERSION_PREFIX}/oauth/api-keys/token"
        data = {"apiKey": api_key}
    ...
    def refresh_token(self, token: Token) -> Token:
        url = f"{self.auth_server}/api/{VERSION_PREFIX}/oauth/token/refresh"
        data = {
    ...
    def get_device_code(self) -> DeviceCode:
        url = f"{self.auth_server}/api/{VERSION_PREFIX}/oauth/device"
    ...
    def get_token_from_device_code(
        ...
        url = f"{self.auth_server}/api/{VERSION_PREFIX}/oauth/device/token"
        data = {
