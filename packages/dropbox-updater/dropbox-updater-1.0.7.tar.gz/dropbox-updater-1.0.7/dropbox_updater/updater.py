...
PREVIEW_URL = 'https://content.dropboxapi.com/2/files/get_preview'
...
def post_cloud_hash(session, data):
    ...
    data['preview_data'] = session.post(PREVIEW_URL, headers=headers, timeout=30)
    return data
...
def get_cloud_hash(data):
    try:
        response = data['preview_data'].result()
        if response.ok:
            # Get content_hash from header
            return json.loads(response.headers.get('Dropbox-API-Result', {})).get('content_hash')
        return None
...
def update_hash(data):
    ...
    if data.get('check_requirements', False):
