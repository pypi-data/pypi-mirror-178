...
class ValidationManager(object):
    ...
    def make_data(self, status):
        ...
        for name, info in self.input_assets.items():
            if info['shape'] is not None:
                arr = np.random.rand(*info['shape']).astype(dtype=info['dtype'])
                bts = arr.tobytes()
                with open(self.data_path / "in" / f"{name}.arr", 'wb') as fp:
                    fp.write(bts)
            else:
                with open(self.data_path / "in" / f"{name}.geojson", 'w') as fp:
    ...
    def test_model_data(self, status):
        ...
        for name, info in self.input_assets.items():
            if info['shape'] is not None:
                ...
                type_ = 'features'
                filepath = f"/tmp/data/in/{name}.geojson"
    ...
    def test_features_response(self, resp, expected_asset, result, n_tests) -> int:
        ...
        try:
            ...
            result.append(
                Exception(f"unable to load geojson features: {str(e)}")
            )
