...
def commandLine():
    ...
    parser.add_argument('--nbd',help='no block display',dest="nbd", action="store_false", default=True)
    ...
    try:
        ...
        while(uns.unsin.nextFrame("")):
            c.fullProcess(uns=uns,out=args.out,component=args.component,xrange=float(args.range),sigma=args.sigma,
                          ...
                          rect_sim=args.rect_sim,rect_comp=args.rect_comp,rect_dir=args.rect_dir,rect_suffix=args.rect_suffix, circle=args.circle,nbd=args.nbd)
            cpt =cpt+1
#        else:
