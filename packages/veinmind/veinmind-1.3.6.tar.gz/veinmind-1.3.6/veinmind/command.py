...
def _iac_command(name=None, **kwargs):
    return decorators.command(name=name, cls=IACCommand, **kwargs)
...
class IACCommand(PluginCommand):
    """IACCommand is the command that accepts in either an
    argument of the iac paths."""

    def __init__(self, **kwargs):
        super(IACCommand, self).__init__(**kwargs)
        self.params.append(core.Option(["--iac-type"],
                                       default="dockerfile",
                                       help="specify iac type"))
        self.allow_extra_args = True
        self.no_args_is_help = False
        self.__veinmind_command__ = {
            "type": "iac",
    ...
    def invoke_plugin(self, ctx):
        ...
        iac_type = ctx.params.pop("iac-type", "dockerfile")
        for path in paths:
            i = iac.IAC(path=path, iac_type=iac_type)
            ctx.invoke(self.callback, i)
...
class Group(core.Group):
    ...
    def iac_command(self, *args, **kwargs):
        """Decorator creating an iac command under current group."""

        def g(f):
            cmd = _iac_command(*args, **kwargs)(f)
            self.add_command(cmd)
            return cmd
...
def iac_command(name=None, attach_main=True, **kwargs):
    """Create an iac command with parameters."""
    f = main.iac_command if attach_main else _iac_command
    return f(name=name, **kwargs)
