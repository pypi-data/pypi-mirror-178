...
class Aruba505Driver(NetworkDriver):
    ...
    def switch_to_config_mode(self):
        ...
        self.device.send_command(cmd, expect_string=r'#', read_timeout=10)
    ...
    def load_merge_candidate(self, filename=None, config=None):
        ...
        time.sleep(8)
        if _running_config:
            print(f"***************** We have a copy of the running config!! *****************")
            time.sleep(2)
        ...
        if config:
            print(f"*****************We have command to send")
            self.config_commands = [i for i in config.split('\n')]
            print(f"{self.config_commands} ******")
            time.sleep(2)
        if not config:
        ...
        if self.config_commands:
            ...
            print(f"Inside the config mode.......")
            ...
            for cmd in self.config_commands:
                ...
                print(f"Exiting the config mode ......\n")
        if filename:
            # TODO: handle this portion latter.
            ...
    ...
    def compare_config(self):
        ...
        print(f"The new running config+++++++++++++++++++++++++++++++++++++++++")
        print(self.new_running_config)

        time.sleep(2) ## this temporay code
        diff = ""
