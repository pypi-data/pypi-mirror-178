...
class FlareLightCurve(lk.LightCurve):
    ...
    def _standardize(self):
        ...
        self.meta["global_std"] = mad_std(self.detrended_flux.value, ignore_nan=True)
    ...
    def detrend(self, window_length=0.3):
        ...
        with warnings.catch_warnings():
            ...
            clip_mask = np.logical_xor(self.flux < np.nanmedian(self.flux) - 3 * sigma, self.flux.mask)
    ...
    def find_candidates(self, n_sigma: float = 3, n_consecutive: int = 2):
        ...
        if self.otype not in CVs:
            ...
            if i_start_array is not None:
                ...
                expel_mask = np.zeros_like(i_start_ext_array, dtype=bool)
                for i in range(expel_mask.size):
                    at_edge = self._is_at_edge(i_start_ext_array[i], i_stop_ext_array[i])
                    small_amp = (
                        self.standardized_flux[i_start_ext_array[i] : i_stop_ext_array[i] + 1].max()
                        < 3 * self.global_std
                    )
                    expel_mask[i] = at_edge or small_amp

                if not expel_mask.all():
                    i_start_ext_array = i_start_ext_array[~expel_mask]
                    i_stop_ext_array = i_stop_ext_array[~expel_mask]
