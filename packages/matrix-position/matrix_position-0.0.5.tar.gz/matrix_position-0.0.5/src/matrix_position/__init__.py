...
class matrix():
    ...
    def show(self, quick = False):
        ...
        -
        if `quick` = False (fault), it prints a value of the matrix every time, but will ensure that the entire matrix is aligned.

        if `quick` = True, it will prints one row of the matrix at a time.That's why it's fast.
        '''
        if quick:
            ...
            for x in range(self.length):
                for y in range(self.width):
                    if max_len < len(str(self.matrix[x][y])):
                        max_len = len(str(self.matrix[x][y]))
            
            for x in range(self.length):
                for y in range(self.width):
                    print('{}'.format(self.matrix[x][y]).center(max_len+2), end='')
                print()
    ...
    def get_position(self, value, return_first_one = False):
        ...
        -
        if `return_first_one`= False (fault), it return all coordinate from the matrix.
    ...
    def fourD(self, position, return_list=False):
        '''
        introduction:
        -
        ...
        parameters: (`position`, `return_list=False`)
        ---
        Note: input's position need to be equal to the format of (x, y).

        if `return_list`= False (fault), it return input position surrounding positions and values including four directions.

        if `return_list`= True, it will return the only coordinate about surrounding positions including four directions.
        '''
        x = position[0]
        y = position[1]
        if return_list:
    ...
    def eightD(self, position, return_list=False):
        '''
        introduction:
        -
        ...
        if `return_list`= False (fault), it return input position surrounding positions and values including eight directions.

        if `return_list`= True, it will return the only coordinate about surrounding positions including eight directions.
        '''
        x = position[0]
        y = position[1]
        if return_list:
            ...
            for i, j in direction_iter(8):  # eightD_order : ← ↖ ↘ ↑ ↓ ↙ ↗ →
                eightD_list.append((x+i, y+j))
            return eightD_list
        else:
            eightD_dict = {}
            for i, j in direction_iter(4):  # eightD_order : ← ↖ ↘ ↑ ↓ ↙ ↗ →
                try:
                    if x + i < 0 or y + j < 0:
