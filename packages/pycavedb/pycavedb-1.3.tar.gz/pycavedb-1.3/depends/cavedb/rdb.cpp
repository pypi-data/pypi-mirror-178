...
namespace lyramilk{ namespace cave
{
		...
		switch(type){
		  ...
		  case RDB_TYPE_LIST_QUICKLIST:{
				...
				while(len--){
					...
					if(zl == nullptr){
						log(lyramilk::log::error,"init") << D("redis 文件格式错误1") << std::endl;
					}
				...
				for(unsigned int i=0;i <sz; i+=2){
					lyramilk::data::var value;
					{
						...
						if(!ziplistGet(sptr,&sval,&slen,&lval)){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误2") << std::endl;
						}
						if(sval){
						...
						if(!sptr){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误3") << std::endl;
						}
					}
					notify_rpush(key,value);
				}
		  ...
		  case RDB_TYPE_ZSET_ZIPLIST:{
				...
				for(unsigned int i=0;i <sz; i+=2){
					lyramilk::data::var value;
					{
						...
						if(!ziplistGet(sptr,&sval,&slen,&lval)){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误4") << std::endl;
						}
						if(sval){
							value.assign(lyramilk::data::string((const char*)sval,slen));
						}else{
						...
						sptr = ziplistNext(zl,sptr);
						if(!sptr){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误5") << std::endl;
						}
					}
					double score;
					{
						...
						if(!ziplistGet(sptr,&sval,&slen,&lval)){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误6") << std::endl;
						}
		  ...
		  case RDB_TYPE_HASH_ZIPLIST:{
				...
				for(;sptr;sptr = ziplistNext(zl,sptr)){
					lyramilk::data::var field;
					{
						...
						if(!ziplistGet(sptr,&sval,&slen,&lval)){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误7") << std::endl;
						}
						if(sval){
						...
						sptr = ziplistNext(zl,sptr);
						if(!sptr){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误8") << std::endl;
						}
					}
					lyramilk::data::var value;
					{
						...
						if(!ziplistGet(sptr,&sval,&slen,&lval)){
							log(lyramilk::log::error,"init") << D("redis 文件格式错误9") << std::endl;
						}
						if(sval){
							value.assign(lyramilk::data::string((const char*)sval,slen));
						}else{
		  ...
		  default:
			log(lyramilk::log::error,"init") << D("redis 文件格式错误def：key=%s,value=%d",key.c_str(),type) << std::endl;
		}
	}
	...
	bool rdb::init(std::istream& is)
	{
		...
		if (memcmp(buf,"REDIS",5) != 0) {
			log(lyramilk::log::error,"init") << D("redis 文件格式错误0") << std::endl;
			return false;
		}
