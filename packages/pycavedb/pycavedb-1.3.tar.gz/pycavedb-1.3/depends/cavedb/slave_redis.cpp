	...
	int slave_redis::svc()
	{
		...
		if(tid==1){
			...
			while(status != st_stop){
				++tmp;
				sleep(1);
				if(psync_support == 0) continue;
				if(psync_support == 2) break;
				if(tmp%3 == 0){
					lyramilk::data::array ar;
			...
			while(status != st_stop){
				if(!(is.good() && c.isalive())){
					...
					{
						if(psync_support == 0 || psync_support == 1){
							lyramilk::data::var ret;
							lyramilk::data::array ar;
							ar.push_back("psync");
							...
							if(ret.type_like(lyramilk::data::var::t_str)){
								...
								if(str.find("ERR unknown command") != str.npos){
									psync_support = 2;
									log(lyramilk::log::warning,"psync") << D("主库不支持psync，改用sync模式。该模式将强制全量同步。") << std::endl;
									continue;
								}
								if(str.find("OK") != str.npos){
									psync_support = 1;
								}
							}
							...
							if(ret.type_like(lyramilk::data::var::t_str)){
								lyramilk::data::istringstream iss(ret.str());
								myrdb r(peventhandler);
								r.init(iss);
							}

							if(!peventhandler->notify_psync(psync_replid,psync_offset)){
								log(lyramilk::log::error,"sync") << D("从redis同步时保存sync数据发生错误") << std::endl;
							}
							psync_rseq_diff = psync_offset - is.rseq();
							peventhandler->is_in_full_sync = false;
							{
							...
							log(lyramilk::log::debug,"sync") << D("继续同步:masterid=%s,offset=%llu",psync_replid.c_str(),psync_offset) << std::endl;
