...
class FetchKDEngine(FetchEngine):
    ...
    def __init__(self):
        ...
        stock_tables = [
            'hkshsz_hold', 'market_adj_before', 'factor_mining', 'uqer_factor',
            'uqer_fancy'
        ]
        futures_tables = [
            'market_fut', 'research', 'factor_basis', 'factor_fundamentals',
            'factor_momentum', 'factor_position', 'factor_term_structure',
            'factor_sentiment', 'factor_reversal', 'factor_tf_fundamentals'
        ]
        self._stock_factor_tables = [
            self._base.classes[s] for s in stock_tables
            if s in self._base.classes
        ]
        self._futures_factor_tables = [
            self._base.classes[s] for s in futures_tables
            if s in self._base.classes
        ]
