...
import { catalogModeHandler, createRemoteIKernelHandler } from './handler';
    ...
    if (newSession.newValue?._kernel?._name) {
      ...
      this.props.getClusterUUIDFromKernel(newSession.newValue._kernel._name).then(
        (clusterUUID) => {
          ...
          if (clusterFromKernel && clusterFromKernel.status === 'RUNNING') {
  ...
  implements DocumentRegistry.IWidgetExtension<NotebookPanel, INotebookModel>
{
  ...
  async _setCatalogMode(): Promise<void> {
    ...
    if (catalogmode === null) {
      catalogmode = await catalogModeHandler();
    }
