...
class CatalogModeHandler(APIHandler):
    """
    Determine whether catalog mode should be enabled
    """

    @web.authenticated
    async def get(self):
        logging_prefix = "[CatalogmodeHandler.get]"
        self.log.info(f"{logging_prefix} Starting...")
        error = None
        ...
        self.log.info(f"{logging_prefix} Finishing...")
        self.finish(json.dumps({"catalog_mode": catalog_mode, "e": error}))
...
def setup_handlers(web_app):
    ...
    catalog_mode_path = url_path_join(base_url, r"/bodo/catalogmode")
    ...
    handlers = [
        ...
        (catalog_mode_path, CatalogModeHandler)
    ]
