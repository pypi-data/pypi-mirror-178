...
def check_int(name, value, max_value=None, min_value=None):
    try:
        ...
        raise ValueError('SIMBA VALUE ERROR: {} should be an integer in SimBA, but is set to {}'.format(name, str(value)))
    if (min_value != None):
...
def check_float(name: str,
    ...
    try:
        ...
        print('{} {} {}'.format(name, str(value), e.as_dict()))
        print('SIMBA VALUE ERROR: {} should be a float in SimBA, but is set to {}'.format(name, str(value)))
