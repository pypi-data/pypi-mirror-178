...
def find_model_version(model_name: str, env: str, metadata: Dict) -> str:
    ...
    for c in project_settings:
        if c.model_name == model_name:
            ...
            external_id = f"{model_name}/{env}/{n_models}"
            continue
...
def promote_model(model_name: str, **kwargs) -> None:
    ...
    Keyword Args:
        confirm (bool): Whether to confirm the promotion, defaultt to True
        version (Optional[int]): Version number. Defaults to None,
