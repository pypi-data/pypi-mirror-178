...
def garbage_collection(
    ...
    latest_model_version = int(max(external_ids).split("-")[-1])
    oldest_model_version = int(min(external_ids).split("-")[-1])

    delete_from_version = latest_model_version - models_to_keep
    if models_to_keep >= len(external_ids) or c.keep_all_models is True:
        logging.info(
            f"Skipping garbage collection, will keep all alive versions of model {c.human_friendly_model_name} in {env} ({len(external_ids)})"
        )
    else:
        logging.info(
            f"Starting garbage collection of model {c.human_friendly_model_name} in {env}"
        )
        logging.info(f"The oldest model in {env} is {oldest_model_version}")
        for v in range(delete_from_version, oldest_model_version - 1, -1):
            logging.info(f"Deleting version {v} of model {c.model_name} in {env}")
            external_id_to_delete = function_name + "-" + str(v)
            cdf.delete_function(function_name=external_id_to_delete, confirm=False)
