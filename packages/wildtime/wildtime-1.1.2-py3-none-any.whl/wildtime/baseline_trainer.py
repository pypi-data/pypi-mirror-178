...
def _precipitation_init(args):
    if args.method in group_datasets:
        from .data.precipitation import PrecipitationGroup
        dataset = PrecipitationGroup(args)
    else:
        from .data.precipitation import Precipitation
        dataset = Precipitation(args)
...
def init(args):
    ...
    method_dic = {'groupdro': 'GroupDRO', 'coral': 'DeepCORAL', 'irm': 'IRM', 'ft': 'FT', 'erm': 'ERM', 'ewc': 'EWC',
                  'agem': 'AGEM', 'si': 'SI', 'simclr': 'SimCLR', 'swav': 'SwaV', 'swa': 'SWA'}
    trainer = globals()[method_dic[args.method]](args, dataset, network, criterion, optimizer, scheduler)
    return trainer
