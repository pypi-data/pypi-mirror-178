...
def chunk_data(data, block_length=None, time_interval=None, split_heights_bands=True):
    """
            ...
            if 'geometry' in dims:
                ...
                for y in data.lat.values:
                    for x in data.lon.values:
                        ...
                        ng1 = bool(calc_null_grid(new1).any())
                        if ng1:
                            new1 = data.sel(lon=[x], lat=[y]).copy()
            ...
            if 'geometry' in dims:
                ...
                for iy, y in enumerate(y_range[1:]):
                    ...
                    for ix, x in enumerate(x_range[1:]):
                        ...
                        new1 = data.isel(lon=lon_slice, lat=lat_slice).copy()
                        ng1 = calc_null_grid(new1)
...
def save_dataset_stations(file_path, block_length, compression='zstd', remove_station_data=True):
    ...
    chunks_list = chunk_data(data, block_length=block_length, time_interval=None, split_heights_bands=False)
...
def save_new_results(nc_path, metadata, version_date, system_version=4, overwrite=False, base_path=None):
    ...
    chunks_list = chunk_data(data, block_length=block_length, time_interval=time_interval, split_heights_bands=True)
