...
class BuildBallViews(ViewBuilder):
    ...
    def __call__(self, instant_key: InstantKey, instant: Instant):
        ...
        for ball in filter(predicate, instant.annotations + instant.detections):
            ...
            yield ViewDescription(c, 0, self.compute_box(keypoints, instant.calibs[c]), ball=ball, timestamp=timestamp)
