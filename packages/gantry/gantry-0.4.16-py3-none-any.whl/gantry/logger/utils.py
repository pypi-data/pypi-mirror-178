...
def _batch_success_msg(batch_id, application, log_store: Union[APILogStore, BaseLogStore]):
    if batch_id:
        ...
        logger.info(
            "Track your batch at {}/applications/{}/jobs".format(
                host, urllib.parse.quote(application)
            )
        )
...
def _put_block(url: str, part_num: int, block: dict) -> dict:
...
def _concurrent_upload_multipart_batch(
    data_batch_iterator: Iterable[dict], signed_urls: List[str]
) -> List[dict]:
