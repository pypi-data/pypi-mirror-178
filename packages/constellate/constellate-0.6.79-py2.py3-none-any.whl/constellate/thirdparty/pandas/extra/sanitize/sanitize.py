...
def fill_missing_from_schema(
    ...
    def _apply_dtype_dataframe(data: pd.DataFrame = None, col_name: str = None, col_type: Any = None) -> pd.DataFrame:
        data[col_name] = data[col_name].astype(col_type)
        return data

    def _apply_dtype_series(data: pd.Series = None, col_name: str = None, col_type: Any = None) -> pd.Series:
        data = data.astype(col_type)
        return data

    def _apply_dtype(data: PandasData = None, col_name: str = None, col_type: Any = None) -> PandasData:
    ...
    def _to_datetime(data: PandasData = None, col_name: str = None) -> PandasData:
        if isinstance(data, pd.DataFrame):
            ...
            data = pandas.to_datetime(data, errors="coerce", downcast=None)
            return data
        else:
            raise NotImplementedError()
    ...
    def _to_timedelta(data: PandasData = None, col_name: str = None) -> PandasData:
        if isinstance(data, pd.DataFrame):
            ...
            data = pandas.to_timedelta(data, errors="coerce", downcast=None)
            return data
        else:
            raise NotImplementedError()
    ...
    def fill(data=PandasData, col_name: str = None, col_info: Any = None, index:bool=False, multi_index:bool=False) -> PandasData:
    ...
    for indexed, fields in [(True, indexes.items()), (False, non_indexes.items())]:
        for col_name, col_info in itertools.chain(fields):
            if has_index and indexed:
                data.index = fill(data=data.index, col_name=col_name, col_info=col_info, index=has_index, multi_index=multi_indexed)
            else:
                data = fill(data=data, col_name=col_name, col_info=col_info, index=False, multi_index=False)
...
def _get_index_name_and_dtypes(schema: PanderaSchema = None) -> Dict[str, Any]:
    ...
    if isinstance(schema2.index, pandera.schema_components.MultiIndex):
        ...
        annotations = next(iter(filter(lambda m: m[0] == '__annotations__', inspect.getmembers(schema))), ('__annotations__', {}))[1]
        name = next(iter([k for k, v in annotations.items() if get_origin(v) == pandera.typing.pandas.Index]), None)
        if name is None:
            raise ValueError()
