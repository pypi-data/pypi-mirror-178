...
def _get_uid(token: str):
    ...
    try:
        ...
        if token_cache_redis is not None and uid:
            ...
            token_cache_redis.setex(token, ttl, json.dumps(payload, default=str))
        return uid
...
async def validate_socket_token(token: str, websocket: WebSocket):
    ...
    try:
        ...
        wallet = payload.get(f"{settings.auth0_namespace}wallet", payload.get('wallet'))
