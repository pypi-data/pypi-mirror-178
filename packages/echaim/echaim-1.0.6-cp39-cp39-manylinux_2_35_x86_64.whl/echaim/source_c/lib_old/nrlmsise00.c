...
#include "nrlmsise00.h"   /* header for nrlmsise-00.h */
...
/*      INTEGRATE CUBIC splineMSISE FUNCTION FROM XA(1) TO X
...
/*      CALCULATE CUBIC splineMSISE INTERP VALUE
...
void splineMSISE (double *x, double *y, int n, double yp1, double ypn, double *y2) {
/*       CALCULATE 2ND DERIVATIVES OF CUBIC splineMSISE INTERP FUNCTION
 *       ADAPTED FROM NUMERICAL RECIPES BY PRESS ET AL
 ...
 */
	...
	if (u==NULL) {
		printf("Out Of Memory in splineMSISE - ERROR");
		return;
	}
	if (yp1>0.99E30) {
		y2[0]=0;
...
/*      Calculate Temperature and Density Profiles for lower atmos.  */
	...
	/* calculate splineMSISE coefficients */
	splineMSISE (xs, ys, mn, yd1, yd2, y2out);
	x = zg/zgdif;
 ...
 */
	...
	/* joining altitudes of Bates and splineMSISE */
	za=zn1[0];
	if (alt>za)
		z=alt;
	else
	...
	*tz = tt;
	...
	/* integrate splineMSISE temperatures */
	splini (xs, ys, y2out, mn, x, &yi);
	expl = gamm * yi;
	if (expl>50.0)
		expl=50.0;
