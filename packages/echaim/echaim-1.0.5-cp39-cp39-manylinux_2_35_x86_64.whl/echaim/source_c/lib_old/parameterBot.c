...
//Calculate the model parameters for the bottomside model
...
double ** parametersBot (double *mlat, double *mlt, double *jd, double *slz, double *f10_81, double *dipTilt, double *ae, int l0)
{
	//define variables
	...
	for (int ai=0; ai<l0; ai++)
	{
		...
		output[ai][c+9] = ae[ai] * cos(D2R * mlat[ai]);
		output[ai][c+10] = ae[ai] * sin(D2R * mlat[ai]);
	}	
