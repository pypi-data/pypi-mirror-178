...
def ipc_forecast(store_id, reset_date, type_list, schema, db, logger):
    ...
    if run_ml_flag == 1:
        ...
        for i in range(1, 5):
            ...
            for group_name in merged_df1[slice_col].unique().tolist():
                ...
                forecast_df['AE_2'] = forecast_df['AE']*(1.11)
    ...
    # weekly_fcst['AE_2'] = weekly_fcst['AE']*(bias_corr_factor)
    weekly_fcst['model'] = 'AE'    
    weekly_fcst[[store_col, drug_col]] = weekly_fcst[key_col].str.split('_',
                                                                        expand=True)
    weekly_fcst.rename(columns={'AE_2': 'fcst'}, inplace=True)
