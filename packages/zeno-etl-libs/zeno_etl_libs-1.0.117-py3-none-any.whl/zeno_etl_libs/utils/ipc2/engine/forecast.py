...
class Forecast:
    ...
    def add_sample_weights(self, df, max_date, forecast_start):
        ...
        df.loc[
            (df[date_col]<(forecast_start-pd.Timedelta(days=70))) & 
            (df[date_col]>=(forecast_start-pd.Timedelta(days=420))),
        'sample_weights'] = 1

        df.loc[
            (df[date_col]<(forecast_start-pd.Timedelta(days=28)))&
            (df[date_col]>=(forecast_start-pd.Timedelta(days=56))),
        'sample_weights'] = 4

        df.loc[
            (df[date_col]<=(forecast_start-pd.Timedelta(days = 0))) &
            (df[date_col]>=(forecast_start-pd.Timedelta(days=28))),
        'sample_weights'] = 7

        df.loc[
            (df[date_col]<=(pd.Timestamp('2020-4-1'))) & 
            (df[date_col]>=(pd.Timestamp('2020-3-1'))),
        'sample_weights'] = 0

        df['sample_weights'] = df['sample_weights'].fillna(0.5)
    ...
    def get_STM_forecast(self, df, forecast_start, num_shift_lags=1):
        ...
        feature_cols = num_cols + flag_cols + added_one_hot_cols + [
            ...
            'cov', 'ADI', 'week_of_month', 'Month', 'MonthStart', 'MonthEnd',
            'max_week5', 'median_week5', 'min_week5', 'std_week5',
            'avg_week5', 'avg_week5_diff', 'max_week13', 'median_week13',
            'min_week13', 'std_week13', 'avg_week13', 'avg_week13_diff'
        ]
