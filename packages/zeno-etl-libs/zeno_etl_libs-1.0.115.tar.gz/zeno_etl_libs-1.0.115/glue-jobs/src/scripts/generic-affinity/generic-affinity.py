...
query = f''' DELETE FROM {table_update} m1 using (select patient_id from {table_update}) m2 where
...
logger.info(f"Postgres DB write completed")
...
s3.write_df_to_db(df=generic_affinity[table_info['column_name']], table_name=table_name, db=rs_db,
                  schema=schema)
