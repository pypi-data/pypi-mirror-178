...
from DatabaseConnectionUtility import Oracle, SAPHANA, InMemory, Dremio, MySql, ExcelFile, Postgress, SQLServer, Tally, ProteusVision, SnowFlake
import requests
import pickle
...
class IncentiveCalculation:
    ...
    def predict(self, textColumn, model, filename=None):
        ...
        if model == "sentiment":
            ...
            predictmodel = joblib.load("models/"+ model+"/"+model+".pkl")
            logger.log(f"predictmodel : {predictmodel}","0")
            # print(knn_from_joblib.predict(textColumn))
