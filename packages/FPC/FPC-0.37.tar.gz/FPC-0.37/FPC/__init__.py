...
class mysql(object):
	conn=None
	def __init__(self, host='127.0.0.1',**args):
		import pymysql
		self.pymysql=pymysql
		connect_config={'host':host,'user':'root','password':'','db':'localhost','charset':'utf8','cursorclass':pymysql.cursors.DictCursor}
		if 'table' in args:
	...
	def execute(self,strs):
		...
		return data
	def add(self,dicts,table=None,ignore=False):
		...
		return self.execute(f''' insert {ignore} `{table}`({key}) values{values[:-1]} ''')
