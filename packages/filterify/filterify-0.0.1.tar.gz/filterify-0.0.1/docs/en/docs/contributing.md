...
First, you might want to see the basic ways to [help Filterify and get help](help-filterify.md){.internal-link target=_blank}.
...
=== "Linux, macOS, Windows Bash"
    ...
    some/directory/filterify/env/bin/pip
    ```
...
**Filterify** uses <a href="https://flit.readthedocs.io/en/latest/index.html" class="external-link" target="_blank">Flit</a> to build, package and publish the project.
...
It will install all the dependencies and your local Filterify in your local environment.

#### Using your local Filterify

If you create a Python file that imports and uses Filterify, and run it with the Python from your local environment, it will use your local Filterify source code.

And if you update that local Filterify source code, as it is installed with `--symlink` (or `--pth-file` on Windows), when you run that Python file again, it will use the fresh version of Filterify you just edited.
...
For it to sort them correctly, you need to have Filterify installed locally in your environment, with the command in the section above using `--symlink` (or `--pth-file` on Windows).
...
* Check the currently <a href="https://github.com/boardpack/filterify/pulls" class="external-link" target="_blank">existing pull requests</a> for your language and add reviews requesting changes or approving them.
...
* Check in the <a href="https://github.com/boardpack/filterify/issues" class="external-link" target="_blank">issues</a> to see if there's one coordinating translations for your language.
...
If you look at the Filterify docs website, you will see that every language has all the pages. But some pages are not translated and have a notification about the missing translation.
