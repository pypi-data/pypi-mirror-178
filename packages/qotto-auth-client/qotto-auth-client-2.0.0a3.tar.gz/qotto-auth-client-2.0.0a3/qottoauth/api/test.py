...
class QottoAuthTestApi(QottoAuthApi):
    ...
    def createUser(self, name: str) -> dict:
        user = User(gen_uuid(), name, False)
        self._users[user.id] = user
        self._user_cookies[user.id] = str(uuid4()), str(uuid4())
        return dict(user=user.to_dict())

    def updateUser(self, userId: str, addIdentityId: str = None, removeIdentityId: str = None) -> dict:
