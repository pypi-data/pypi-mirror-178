        ...
        if network_interface.get("Association") is not None:
            ...
            if nic.public_ip:
                ...
                if not re.match("amazon-aws|amazon-rds|amazon-redshift|amazon-elb",
                    ...
                    nic.instance_name = f"(EC2) {instance_tags[0].get('Value')}"
                elif re.match("amazon-elb", instance_info.get("InstanceOwnerId")):
                    network_interface_desc = network_interface.get("Description").split('/')
                    nic.instance_name = f"(ELB) {network_interface_desc[1]}"
                    nic.instance_id = f"(ELB) {network_interface_desc[1]}"
                else:
