...
class PPMain:
    ...
    def load_data(self):
        ...
        ipars = np.fromiter(res['model_params'].keys(), dtype=np.int64)
        mod_par = np.c_[ipars, res['model_params']]

        imins = np.fromiter(res['mu_glmin'].keys(), dtype=np.int64)
        min_preds = np.c_[
        ...
        xnexts = np.c_[iter_labels[:-1], acqs[1:, 1:-1]]
        est_yranges = np.c_[
            iter_labels,
    ...
    def dump_data(self):
        ...
        for it in range(self.batch_tracker.num_iters):
            ...
            if self.settings["pp_truef_at_glmins"]:
                ...
                f_glmin = self.settings.f.eval(
                    np.atleast_2d(x_glmin)
                )[1].item()
                ioutils.append_write(
                    self.files["truef_glmin"],
                    ioutils.data_line([it, npts, f_glmin, f_glmin - mu_glmin], fstr="%18.10E"),
                )
    ...
    def _plot_conv(self):
        ...
        if self.settings["pp_truef_at_glmins"]:
            truef_hats = np.atleast_2d(np.loadtxt(self.files["truef_glmin"], skiprows=1))
            if len(truef_hats) > 1:
                plot.plot_truef_hat(
                    self.settings,
    ...
    def _plot_model(self):
        ...
        for it in sts["pp_iters"]:
            ...
            assert npts == model.X.shape[0], "Model recreate fail!"
