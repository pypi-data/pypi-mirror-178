...
def refresh_tokens(chatdb: 'ChatDb',
    ...
    if len(new_tokens) > 0:
        ...
        logger.debug(f'{participle} table {code(table_name)}, shape: {new_tokens.shape}', arrow='yellow')

    else:
        logger.debug(f'No new tokens to add to table {code(table_name)}', arrow='yellow')
