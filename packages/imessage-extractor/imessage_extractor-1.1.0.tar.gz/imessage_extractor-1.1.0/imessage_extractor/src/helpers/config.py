...
class WorkflowConfig(object):
            ...
            for subthing in thing:
                if subthing != self.dir.home:
                    if not isdir(subthing):
                        ...
                        self.logger.debug(f'Required directory exists {path(subthing.replace(self.dir.home + "/", ""))}')
        ...
        for item in [f for f in dir(self.file) if not f.startswith('_')]:
            ...
            for subthing in thing:
                if not isfile(subthing):
                    ...
                    self.logger.debug(f'Required file exists {path(subthing.replace(self.dir.home + "/", ""))}', arrow='black')
    ...
    def _no_duplicate_object_names(self):
        ...
        self.logger.debug('No duplicate table/view definitions found', arrow='black')
