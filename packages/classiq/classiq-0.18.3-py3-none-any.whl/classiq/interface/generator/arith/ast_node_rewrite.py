...
class AstNodeRewrite(ast.NodeTransformer):
    ...
    def extract_node_id(self, node: ast.AST) -> Optional[Union[str, float]]:
        if hasattr(node, "id"):
            ...
            return type(node.op).__name__ + next(self.count_str_gen)  # type: ignore[attr-defined]
            ...
            return type(node.ops[0]).__name__ + next(self.count_str_gen)  # type: ignore[attr-defined]
        return None
