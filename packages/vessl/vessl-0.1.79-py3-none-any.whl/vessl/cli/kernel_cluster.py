...
def create(
    name: str, mode: str, kubernetes_namespace: str, extra_helm_values: List[str]
):
...
def delete(name: str):
    ...
    print(
        f"try execute the following in the shell (note that this is NOT RECOVERABLE)."
    )
...
def print_cluster_data(cluster: ResponseKernelClusterInfo, **kwargs):
    print_data(
        {
            "Name": cluster.name,
            "Dashboard URL": Endpoint.cluster.format(
                _get_organization_name(**kwargs), cluster.id
            ),
