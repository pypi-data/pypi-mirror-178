...
def _load_cluster_kubeconfig(
    ...
    for config_file in config_files:
        try:
            ...
            logger.warn(
                f"Cluster install: load kubeconfig from {config_file} failed: {repr(e)}"
            )
...
"""
    ...
    if uname().machine == "arm64":
        logger.debug(
            "System is using CPU with arm64 architecture; adding additional kwargs"
        )
    ...
    for trial in range(trial_limit):
        logger.info(
            f"Checking for the k0s-in-docker node to be ready... ({trial+1}/{trial_limit})"
        )
    ...
    if not node_ready:
        container.kill()
        raise VesslRuntimeException(
            "Timeout while waiting for k0s-in-docker node. Please retry."
        )

    # Get kubeconfig, keep at /opt/vessl/cluster_kubeconfig.yaml for later
    logger.debug("Acquiring kubeconfig")
    res = container.exec_run(
        "cat /var/lib/k0s/pki/admin.conf", stream=False, demux=True
    )
    output, _ = res.output
...
def _check_existing_vessl_installation(**kwargs) -> ResponseKernelClusterInfo:
    ...
    logger.warn(
        "Existing VESSL cluster installation found! getting cluster information..."
    )
