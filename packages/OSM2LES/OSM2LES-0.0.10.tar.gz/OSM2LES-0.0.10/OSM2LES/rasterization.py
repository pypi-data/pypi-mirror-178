...
def process_building(lineO,lineL,domain,idxP,gdf,bHeightL,bHeightH,x_mul,y_mul,x_start,y_start,bbox_cor,bldH,area,areaH,res):
    # poly.bounds
    nx = []
    ny = []
    xy = []
    ...
    if pd.isnull(bHeightH[idxP]): # consider the actual height first
        if pd.isnull(bHeightL[idxP]): 
