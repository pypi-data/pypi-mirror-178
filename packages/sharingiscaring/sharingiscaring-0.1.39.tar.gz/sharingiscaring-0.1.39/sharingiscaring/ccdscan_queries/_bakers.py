...
class Mixin:
    # def ql_request_open_pools(self, before: str = None, after: str = None):
    #         LIMIT = self.nodes_request_limit
    #         LIMIT = 10
    #         query = "query {"
            
    #         accounts_str = 'bakers(filter: { openStatusFilter: OPEN_FOR_ALL }, '
    #         if before == 'first':
    #             accounts_str  +=  f'first: {LIMIT}) '
            
    #         elif before != 'null':
    #             accounts_str  +=  f'last: {LIMIT}, before: "{before}") '
    #         elif 'last' in after:
    #             accounts_str  +=  f'{after}) ' 
            
    #         elif after != 'null':
    #             accounts_str  +=  f'first: {LIMIT}, after: "{after}") ' 
    #         else:
    #             accounts_str += f'first: {LIMIT})'
    #         query += accounts_str + '{'
    #         query += self.pageInfo()
    ...
    #                 return  r.json()['data']['bakers']['nodes'], \
    #                         r.json()['data']['bakers']['pageInfo']
