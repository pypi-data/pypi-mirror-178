            ...
            if t.result['outcome'] == 'success':
                ...
                for event in events:
                    if event['tag'] == 'Transferred':
                        ...
                        receiver = event['to']
                        # txHash = t.hash
                        amount = self.get_sum_amount_from_scheduled_transfer(events)

                    elif event['tag'] == 'ContractInitialized':
                        # txHash = t.hash
                        contract = {'tag': event['tag'], 
                        ...
                        # # check for CNS domain
                        # if 'message' in event:
                        #     print ("'message' in event")
                        #     cns_info[event["receiveName"]] = event["message"]
                        #     print (cns_info)
                        #     if (('BictoryCnsNft.getTokenExpiry' in cns_info) and ('BictoryCns.register' in cns_info)):
                        #         print ("'BictoryCnsNft.getTokenExpiry' in cns_info")
                        #         tokenId = cns_info['BictoryCnsNft.getTokenExpiry'][2:]
                        #         print (f"{tokenId=}")

        if amount:
            amount = int(amount) / 1_000_000
        return sender, receiver, txHash, contract, amount, tokenId, self.cns_domain.domain_name, self
            ...
            if t.result['outcome'] == 'success':
                ...
                for event in events:
                    ...
                    if event['tag'] in ['DelegationStakeIncreased', 'DelegationStakeDecreased']:
                        previous_amount = self.request_account_on_previous_block(sender, self.block['blockHeight'])
                        if previous_amount:
                            if int(previous_amount) > 0:
                                if int(event['newStake']) > int(previous_amount):
                                    perc = f" ({(100*((int(event['newStake'])/int(previous_amount)) - 1)):,.2f}%)"
                                else:
                                    perc = f" ({(100*(int(previous_amount) - int(event['newStake']))/int(previous_amount)):,.2f}%)"

                    if event['tag'] == 'DelegationSetDelegationTarget':
                        if event['delegationTarget']['delegateType'] == 'Baker':
                            bakerId = event['delegationTarget']['bakerId']
                            message += f"Delegation target set to {bakerId:,.0f}. "
                    
                    elif event['tag'] == 'DelegationStakeIncreased':
                        if previous_amount:
                            message += f"Stake increased{perc} to {(int(event['newStake'])/1_000_000):,.0f} CCD. "
                        else:
                            message += f"Stake set to {(int(event['newStake'])/1_000_000):,.0f} CCD. "
                        request_target = True
                    
                    elif event['tag'] == 'DelegationStakeDecreased':
                        message += f"Stake decreased{perc} to {(int(event['newStake'])/1_000_000):,.0f} CCD. "
                        request_target = True
                    
                    elif event['tag'] == 'DelegationRemoved':
                        message += f"Delegation removed. "
                        request_target = True
                    
                    
        return sender, bakerId, txHash, message, request_target
