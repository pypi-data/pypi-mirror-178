...
from base_app.views import GenericSoapAPIView
...
class GetLmsData(APIView):
    ...
    def post(self, request):
        ...
        :return:
        """
        rq_srl = fec_dbsync_serializers.GetLmsDataRequestSerializer(data=request.data)
        if rq_srl.is_valid():
            # get objects
            try:
                general_account_obj = get_object_or_404(
                    klass=fec_dbsync_models.GeneralAccount, LOAN_ACCT_NUM=rq_srl.data.get('contract_id')
                )
            except Http404 as e:
            ...
            try:
                robo_eit_obj = get_object_or_404(
                    klass=fec_dbsync_models.RoboEitView, LOAN_ACCT_NUM=rq_srl.data.get('contract_id')
                )
            except Http404 as e:
                error = {'type': 's', 'reason': repr(e)}
                return base_app_utils.response(data=None, code=status.HTTP_400_BAD_REQUEST, error=error)
            ...
            try:
                robo_chat_object = get_object_or_404(
                    klass=fec_dbsync_models.RoboChatView, LOAN_ACCT_NUM=rq_srl.data.get('contract_id'),
                    CHARGE_TYPE="MISC2"
                )
            except Http404 as e:
                error = {'type': 's', 'reason': repr(e)}
            ...
            upcoming_installation_payment_due_date = Utils.get_upcoming_due_date(robo_lrs_obj)
            ...
            overdue_amount, months_overdue, total_installments_paid, last_inst_obj = \
                Utils.calculate_total_overdue_amount(robo_idt_objects, robo_eit_obj, robo_chat_object)
            ...
            repayment_information = Utils.get_repayments_info(robo_idt_objects, rq_srl.data.get("business_date"))
            ...
            if rorbo_clelarsh_after_business_date.count() != 0:
                ...
                current_outstanding = int(current_principle) + int(current_interest) + int(current_repayment)
            ...
            # payment history
            payment_history = fec_dbsync_serializers.RepaySerializer(robo_repay_objects, many=True)

            transaction_history = get_transaction_history(rq_srl.data.get('contract_id'))
            res = []
            for txn in transaction_history:
                ...
                d['paymentMethod'] = Utils.get_payment_mode(txn['repay_object'])
            ...
            overdue['overdueOutstanding'] = principle_overdue + interest_overdue + penal_overdue
            repayment_tab_data['overdue'] = overdue
            repayment_tab_data['total'] = {"totalOutstanding": outstanding_amount}
...
class GetRoboActiveApps(APIView):
    ...
    def post(self, request):
        try:
            rq_srl = fec_dbsync_serializers.GetActiveAppsRequestSerializer(data=request.data)
            if rq_srl.is_valid():
                # get objects
                cifgen_objects = CIFGenDetailsModel.objects.filter(IDNUMBER=rq_srl.data.get('national_id'))
                count=0
                count_closed=0
                loanset = set()
                for cifgen_object in cifgen_objects:
                    loanset.add(cifgen_object.CIF_ID)
                    general_account_obj = fec_dbsync_models.GeneralAccount.objects.filter(CIF_ID=cifgen_object.CIF_ID)
                    try :
                        outstanding_amount = abs(general_account_obj[0].CURRENT_BAL)
                ...
                response = {
                    ...
                    'totalClosedLoanAppsRobo': count_closed,
                    'totalDisbursedLoanAppsRobo': len(loanset)
                }
                return base_app_utils.response(data=response, code=status.HTTP_200_OK, error='')
            error = rq_srl.errors
            error.update({'type': 's', 'reason': 'bad request'})
            ...
            response = {
                'totalActiveLoanAppsRobo': 0,
                'totalClosedLoanAppsRobo': 0,
                'totalDisbursedLoanAppsRobo': 0,
                'exception': str(e)
            }

            return base_app_utils.response(data=response, code=status.HTTP_200_OK, error='')

class GetRepaymentScheduleOld(APIView):
    """
...
class GetRepaymentSchedule(APIView):
    ...
    def post(self, request):
        ...
        if rq_srl.is_valid():
            ...
            #    LOAN_ACCT_NUM=rq_srl.data.get('contract_id')
            #).order_by('REP_SHDL_DATE')
            robo_clelarsh = Utils.get_repayment_schedule_from_oracle(rq_srl.data.get('contract_id'));
            repayment_schedule = list()
            try:
                general_account = get_object_or_404(
                    klass=fec_dbsync_models.GeneralAccount, LOAN_ACCT_NUM=rq_srl.data.get('contract_id')
            ...
            for clelarsh in robo_clelarsh:
                outstanding_principle = outstanding_principle + clelarsh[2]
            for clelarsh in robo_clelarsh:
                try:
                    repayment_schedule_date = datetime.datetime.strptime(str(clelarsh[6]), '%Y-%m-%d') \
                        .strftime('%d-%m-%Y')
                except:
                    repayment_schedule_date = clelarsh[6]
                outstanding_principle = outstanding_principle - clelarsh[2]
                repayment_schedule.append({
                    'repayment_date': repayment_schedule_date,
                    'payable_principle': clelarsh[2],
                    'payable_interest': clelarsh[3],
                    'outstanding_principle': outstanding_principle,
                    'payable_collection_fee': clelarsh[4],
                    'total_payable_amount': clelarsh[5] + clelarsh[4],
                })
            response = {
                'repayment_schedule': repayment_schedule,
                'maturity_date': maturity_date
...
class DedupeCheck(APIView):
    def post(self, request):
        rq_srl = fec_dbsync_serializers.DedupeCheckRequestSerializer(data=request.data)
        if rq_srl.is_valid():
            try:
                response = Utils.get_data_from_oracle(rq_srl.data.get('national_id'),rq_srl.data.get('phone_number'))
...
class DedupeCheckRobo(APIView):
    def post(self, request):
        rq_srl = fec_dbsync_serializers.DedupeCheckRoboRequestSerializer(data=request.data)
        if rq_srl.is_valid():
            try:
                response = Utils.get_data_from_oracle_robo(rq_srl.data.get('p_Phone'),rq_srl.data.get('p_NID'),rq_srl.data.get('p_Product_Channel'))
                return base_app_utils.response(response[0], status.HTTP_200_OK)
            except Exception as e:
                print(e)
                return base_app_utils.response({"message": "Something went wrong"},
...
class RetrieveReferences(GenericSoapAPIView):
    def post(self, request):
        rq_srl = fec_dbsync_serializers.RetreiveReferencesRequestSerializer(data=request.data)
        if rq_srl.is_valid():
            try:
                response = Utils.get_references_from_oracle(rq_srl.data.get('national_id'),rq_srl.data.get('phone_number'))
...
class CollectionsRepayment(APIView):
    def get(self, request):
        ...
        try:
            repayments = []
            robo_clelarsh_objects = fec_dbsync_models.RoboClelarshView.objects.filter(LOAN_ACCT_NUM=loan_acct_num)
...
class CollectionsStatement(APIView):
    def get(self, request):
        ...
        try:
            ...
            robo_cxl = fec_dbsync_models.RoboCxlView.objects.filter(LOAN_ACCT_NUM=loan_acct_num, EVENT_ID='MISC2')

            robo_clelarsh = fec_dbsync_models.RoboClelarshView.objects.filter(LOAN_ACCT_NUM=loan_acct_num)
            ...
            list_schedules = fec_dbsync_models.RoboIdtView.objects.filter(LOAN_ACCT_NUM=loan_acct_num)
...
class CollectionsPayment(APIView):
    def get(self, request):
        ...
        try:
            response = {}
            payments = []
            fdv_objects = fec_dbsync_models.RoboCollectionFdv.objects.filter(APPL_ID=loan_acct_num)
...
class CollectionsSummary(APIView):
    def get(self, request):
        ...
        try:
            ...
            repayment_information = Utils.get_repayments_info(robo_idt_objects, business_date)
            ...
            charge_calc = fec_dbsync_models.RoboChatView.objects.filter(
                LOAN_ACCT_NUM=loan_acct_num).aggregate(TOTAL_SYS_CALC_CHRGE_AMT=Sum('SYS_CALC_CHRGE_AMT'),
                                                       TOTAL_USER_CALC_CHRGE_AMT=Sum('USER_CALC_CHRGE_AMT'),
            ...
            if interest_overdue_obj and interest_overdue_obj.get("DMD_AMT_SUM", -1) > -1 and interest_overdue_obj.get(
                    "TOT_ADJ_AMT_SUM", -1) > -1:
                interest_overdue = interest_overdue_obj.get("DMD_AMT_SUM") - interest_overdue_obj.get("TOT_ADJ_AMT_SUM")
            ...
            if penalty_overdue_obj and penalty_overdue_obj.get("DMD_AMT_SUM") and penalty_overdue_obj.get(
                ...
                "TOT_ADJ_AMT_SUM", -1) > -1:
                penalty_overdue = penalty_overdue_obj.get("DMD_AMT_SUM") - penalty_overdue_obj.get("TOT_ADJ_AMT_SUM")
            ...
            if charge_calc:
                ...
                if charge_calc.get("TOTAL_CHRGE_AMT_COLLECTED") and total_charges:
                    total_charges -= charge_calc.get("TOTAL_CHRGE_AMT_COLLECTED")
            payload = {
                "overdueDetails": {
...
class CollectionsForeClosure(APIView):
    def get(self, request):
        ...
        if not loan_acct_num:
            return base_app_utils.response(data=None, code=status.HTTP_400_BAD_REQUEST, error=None)
        try:
            fdv_obj = fec_dbsync_models.RoboCollectionFdv.objects.filter(APPL_ID=loan_acct_num).first()
            if fdv_obj:
                try:
...
class CollectionsStaCard(APIView):
    def get(self, request):
        ...
        try:
            ...
            fdv_obj = fec_dbsync_models.RoboCollectionFdv.objects.filter(APPL_ID=loan_acct_num).first()
            gam_obj = fec_dbsync_models.GeneralAccount.objects.filter(LOAN_ACCT_NUM=loan_acct_num).first()
            lrs_obj = fec_dbsync_models.RoboLrsView.objects.filter(LOAN_ACCT_NUM=loan_acct_num).first()
            ...
            if lrs_obj:
                payload["emiAmount"] = lrs_obj.FLOW_AMT + Utils.fec_dbsync_cons.PROCESSING_FEE
            if gam_obj:
                try:
            ...
            if payment_history.data:
                payload["latestPaymentDate"] = payment_history.data[0].get("payment_date")
...
class GetDataCustConAdd(GenericSoapAPIView):
    def post(self, request):
        rq_srl = fec_dbsync_serializers.GetDataCustConAddRequestSerializer(data=request.data)
        if rq_srl.is_valid():
            try:
                response = Utils.get_data_from_cust_con_add(rq_srl.data.get('contract_number'))
                return base_app_utils.response(response, status.HTTP_200_OK)
            except Exception as e:
                print(e)
                return base_app_utils.response({"message": "Something went wrong"},
...
class GetActiveLoanByAppIds(APIView):
    def post(self, request):
        rq_srl = fec_dbsync_serializers.GetActiveLoanByAppIdsRequestSerializer(data=request.data)
        if rq_srl.is_valid():
            try:
                response = Utils.getActiveLoanByAppIds(rq_srl.data.get('app_ids'))
                return base_app_utils.response(response, status.HTTP_200_OK)
            except Exception as e:
                print(e)
                return base_app_utils.response({"message": "Something went wrong"},
...
class FraudCheckRule(APIView):
    def post(self, request):
        ...
        if fraudSrl.is_valid():
            try:
                result ={'dataFlags':{},'fraudFlag' : False, 'softRejectFlag' : False, 'freezeFlag': False, 'dataFraudFlag': False}


                if fraudSrl.data.get('pincode') !=None and bool(fraudSrl.data.get('pinCode')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('pinCode',fraudSrl.data.get('pincode'), False), result, 'pinCodeFraud')
                if fraudSrl.data.get('tsaCode') !=None and bool(fraudSrl.data.get('tsaCode')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('salesAgentEmailId',fraudSrl.data.get('tsaCode'), False),result, 'tsaCodeFraud')
                if fraudSrl.data.get('dsaCode') !=None and bool(fraudSrl.data.get('dsaCode')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('salesAgentEmailId',fraudSrl.data.get('dsaCode'), False), result,'dsaCodeFraud')
                if fraudSrl.data.get('ccCode') !=None and bool(fraudSrl.data.get('ccCode')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('salesAgentEmailId',fraudSrl.data.get('ccCode'), False), result,'ccCodeFraud')
                if fraudSrl.data.get('temporaryProvince') !=None and bool(fraudSrl.data.get('temporaryProvince')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('temporaryProvince',fraudSrl.data.get('temporaryProvince'), False), result,'temporaryProvinceFraud')
                if fraudSrl.data.get('temporaryAddressCityDistrict') !=None and bool(fraudSrl.data.get('temporaryAddressCityDistrict')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('temporaryAddressCityDistrict',fraudSrl.data.get('temporaryAddressCityDistrict'), False), result,'temporaryAddressCityDistrictFraud')
                if fraudSrl.data.get('temporaryAddressCityDistrictWard') !=None and bool(fraudSrl.data.get('temporaryAddressCityDistrictWard')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('temporaryAddressCityDistrictWard',fraudSrl.data.get('temporaryAddressCityDistrictWard'), False), result, 'temporaryAddressCityDistrictWardFraud')
                if fraudSrl.data.get('permanentAddressCity') !=None and bool(fraudSrl.data.get('permanentAddressCity')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('permanentProvince',fraudSrl.data.get('permanentAddressCity'), False), result,'permanentAddressCityFraud')
                if fraudSrl.data.get('permanentAddressCityDistrict') !=None and bool(fraudSrl.data.get('permanentAddressCityDistrict')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('permanentDistrict',fraudSrl.data.get('permanentAddressCityDistrict'), False), result,'permanentAddressCityDistrictFraud')
                if fraudSrl.data.get('permanentAddressCityDistrictWard') !=None and bool(fraudSrl.data.get('permanentAddressCityDistrictWard')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('permanentWard',fraudSrl.data.get('permanentAddressCityDistrictWard'), False), result,'permanentAddressCityDistrictWardFraud')
                if fraudSrl.data.get('phoneNumberCustomer') !=None and bool(fraudSrl.data.get('phoneNumberCustomer')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('mobileNumber',fraudSrl.data.get('phoneNumberCustomer'), False), result,'phoneNumberCustomerFraud')
                if fraudSrl.data.get('phoneNumberReferenceA') !=None and bool(fraudSrl.data.get('phoneNumberReferenceA')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('referenceMobileNumber',fraudSrl.data.get('phoneNumberReferenceA'), False), result,'phoneNumberReferenceAFraud')
                if fraudSrl.data.get('phoneNumberReferenceB') !=None and bool(fraudSrl.data.get('phoneNumberReferenceB')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('referenceMobileNumber',fraudSrl.data.get('phoneNumberReferenceB'), False), result,'phoneNumberReferenceBFraud')
                if fraudSrl.data.get('phoneNumberReferenceC') !=None and bool(fraudSrl.data.get('phoneNumberReferenceC')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('referenceMobileNumber',fraudSrl.data.get('phoneNumberReferenceC'), False), result,'phoneNumberReferenceCFraud')
                if fraudSrl.data.get('applicationID') !=None and bool(fraudSrl.data.get('applicationID')) :
                    print("Application ==========",Utils.getFraudDataFromOracle('applicationId',fraudSrl.data.get('applicationID'), False))
                    result = Utils.check(Utils.getFraudDataFromOracle('applicationId',fraudSrl.data.get('applicationID'), False), result,'applicationIDFraud')
                if fraudSrl.data.get('bankAccount') !=None and bool(fraudSrl.data.get('bankAccount')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('beneficiaryAccountNumber',fraudSrl.data.get('bankAccount'), False), result,'bankAccountFraud')
                if fraudSrl.data.get('nationalId') !=None and bool(fraudSrl.data.get('nationalId')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('nationalId',fraudSrl.data.get('nationalId'), False), result,'nationalIdFraud')
                if fraudSrl.data.get('locationCordinate') !=None and bool(fraudSrl.data.get('locationCordinate')) :
                    result = Utils.check(Utils.getFraudDataFromOracle('locationCordinate',fraudSrl.data.get('locationCordinate'), False), result,'locationCordinateFraud')


                if fraudSrl.data.get('locationCordinate') !=None and bool(fraudSrl.data.get('locationCordinate')) :
                    dataList = Utils.getFraudDataFromOracle(None, None, True)
                    ...
                    try :
                        ...
                        result = Utils.check(fraudRuleList, result,'locationZoneFraud')
                    except Exception as e:
                        traceback.print_exc()
...
class GetBlacklistedAccounts(APIView):
    def post(self, request):
        ...
        try:
            ...
            blacklisted_bank_accounts = Utils.getBlackListedBankAccountsFromOracle(
                bank_account)
...
class getAMLTableData(APIView):
    def post(self, request):
        print("Started getAML table" + str(request.data));
        ...
        try:
            ...
            print("Ended with get response" + str(rawresponse))
            return base_app_utils.response(rawresponse, status.HTTP_200_OK)
        except Exception as e:
            print(e)
            return base_app_utils.response({"message": "Something went wrong"},
                                            status.HTTP_500_INTERNAL_SERVER_ERROR, error=str(e))