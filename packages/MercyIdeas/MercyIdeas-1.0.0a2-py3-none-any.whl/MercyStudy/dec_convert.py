from MercyIdeas.MercyStudy import get_num


def check_float(_num):
    if not isinstance(_num, float):
        raise TypeError("输入内容非浮点数或整数")
    return True
...
def bin_float(_num, _precision=8):
    if isinstance(_num, int):
        return float(str(bin(_num)).lstrip('0b'))
    check_float(_num)
...
def oct_float(_num, _precision=8):
    if isinstance(_num, int):
        return float(str(oct(_num).lstrip('0o')))
    check_float(_num)
    _integer = get_num.get_integer_from_float(_num)
