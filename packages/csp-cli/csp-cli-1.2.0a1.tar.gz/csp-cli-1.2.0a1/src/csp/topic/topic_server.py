...
# 新版list 不再有合并单元格
def topic_list(model_repository=None, classify=None, show=True):
    if model_repository:
...
def topic_upload(model_type: int, model_repository: str, dataset: str):
    if model_type not in [0, 1]:
        raise ValueError("模型类型，0-内部 1-外部")
    ...
    if image_url:
        ...
        params = {"modelType": model_type, "modelName": model_name, "modelVersion": model_tag, "datasetName": dataset}
        ...
        if res_dict['code'] == 200:
            if model_type == 0 and msg == "模型注册成功":
                print("内部模型注册成功")
            elif model_type == 1 and msg == "模型注册成功":
                print("外部模型注册成功")
            else:
