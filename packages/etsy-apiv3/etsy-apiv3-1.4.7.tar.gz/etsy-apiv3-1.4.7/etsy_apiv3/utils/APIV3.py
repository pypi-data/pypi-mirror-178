...
class EtsySession:
    ...
    def __init__(self,
                 ...
                 token_updater: Callable = __token_updater
    ):
        ...
        self.__oauth2_session = OAuth2Session(
            self.CLIENT_KEY,
            token=self.TOKEN,
    ...
    @property
    def me(self) -> dict:
        return self.__oauth2_session.get(f"{self.__base_endpoint}/users/me", headers=self.headers).json()
    ...
    def request(self, endpoint: str, method="GET", *args, **kwargs) -> dict:
        ...
        req: requests.Response = self.__oauth2_session.request(method, url, headers=self.headers, *args, **kwargs)
