...
class HourSelectionService:
    ...
    def _convert_collections(self, newobj: HourObject, index_deviation: int) -> HourObject:
        ...
        newobj.nh = _chop_list(newobj.nh)
        newobj.ch = _chop_list(newobj.ch)
        newobj.dyn_ch = _chop_dict(newobj.dyn_ch)
        newobj.offset_dict = _chop_dict(newobj.offset_dict)
        newobj.pricedict = _chop_dict(newobj.pricedict)

        return newobj

    def _update_interim_lists(self, _range: range, oldobj: HourObject, newobj: HourObject, index_devidation: int) -> HourObject:
        newobj = self._convert_collections(newobj, index_devidation)
        for i in _range:
            if i in newobj.nh:
        ...
        for r in newobj.offset_dict.keys():
            oldobj.offset_dict[r] = newobj.offset_dict[r]
            oldobj.pricedict[r] = newobj.pricedict[r]
