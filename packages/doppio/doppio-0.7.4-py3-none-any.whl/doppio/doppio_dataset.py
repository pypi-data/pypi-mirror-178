...
class DoppioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        local_json_count = len(glob(os.path.join(self.local_json_path,"*.json")))

       # print(len(self.json_files), len(os.path.join(self.local_json_path,"*.json")))

        if len(self.json_files)-1 == local_json_count:
            self.local = True
        else:
            self.data_count += 1
