...
import pandas as pd
from .DS_clust import Dawid_Skene_clust as DS
import torch
...
class WAUM_clust(CrowdModel):
    def __init__(
        ...
        **kwargs
        ...
        """
        The `tasks` must be given in the form of a torch dataset
        using  the following __getitem__ format:
            (task, target, index)
        When the true target is unknown, replace it with `None`
        """
    ...
    def run_DS(self, cut=False):
        if not cut:
            self.ds = DS(self.answers, self.n_classes, L=self.L)
            ...
            self.ds = DS(self.answers_waum, self.n_classes, L=self.L)
            self.ds.run(maxiter=self.maxiterDS)
    ...
    def make_step(self, data_j, batch):
        ...
        Batch:
            ...
            - index 1: placeholder for labels
            - index 2: ground truths if available
