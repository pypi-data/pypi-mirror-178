            ...
            if arg_type in core_types:
                ...
                if arg_type.startswith("Vector"):
                    ...
                    if sub_type in core_types:
                        ...
                        arg_type = "List of :class:`fipper.api.types.{}`".format(
                            ".".join(
                                sub_type.split(".")[:-1]
                    ...
                    arg_type = ":class:`fipper.api.types.{}`".format(
                        ".".join(
                            arg_type.split(".")[:-1]
                ...
                if c.return_type.startswith("Vector"):
                    ...
                    if sub_type in core_types:
                        ...
                        if c.section == "functions":
                            try:
                                ...
                                return_type = "List of :class:`fipper.api.types.{}`".format(
                                    ".".join(
                                        sub_type.split(".")[:-1]
                                        + [capit(sub_type.split(".")[-1])]
                                    )
                                ...
                                constructors = ["List of :class:`fipper.api.types.{}`".format(
                                    ".".join(
                                        i.split(".")[:-1]
                                        + [capit(i.split(".")[-1])]
                                    )
                    ...
                    if c.section == "functions":
                        try:
                            ...
                            return_type = ":class:`fipper.api.types.{}`".format(
                                ".".join(filter(None, [c.namespace, capit(c.name)]))
                            )
                        else:
                            constructors = [":class:`fipper.api.types.{}`".format(
                                ".".join(
                                    i.split(".")[:-1]
                                    + [capit(i.split(".")[-1])]
                                )
