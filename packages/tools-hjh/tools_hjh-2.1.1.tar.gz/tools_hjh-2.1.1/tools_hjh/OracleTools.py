...
class OracleTools:
    ...
    def get_tablespace_state(dba_conn):
        ...
        """ ip, service_name, owner, tablespace_name, file_id, file_name, all_size_m, use_size_m, user_status, lock_or_expiry_date """
        sql = '''
    ...
    def get_tablespace_file_state(dba_conn):
        ...
        """ ip, service_name, tablespace_name, file_name, file_id, all_size, use_size """
        ip = dba_conn.run("select utl_inaddr.get_host_address ip from dual").get_rows()[0][0]
        service_name = dba_conn.run("select global_name from global_name").get_rows()[0][0]
        sql = '''
        ...
        col = ('ip', 'service_name', 'tablespace_name', 'file_name', 'file_id', 'all_size', 'use_size')
        rows = []
        ...
        for rs in rss:
            ...
            rows.append((ip, service_name, tablespace_name, file_name, file_id, all_size, use_size))
    ...
    def analysis_ogg_info(host_conn):
        ...
        """ 返回 {'ext_info':[], 'dmp_info':[], 'rep_info':[], 'rep2kafka_info':[]} """
        host = host_conn.host
        ...
        # 进程信息 
        ogg_info = {'ext_info':[], 'dmp_info':[], 'rep_info':[], 'rep2kafka_info':[]}
        ...
        for ogg in ogg_status.get_rows():
            if ogg[3] != 'manager':
                ...
                if ogg_type == 'ext':
                    ...
                    ogg_info['ext_info'].append(ext_info)
                    print(ext_info)
                    ...
                    ogg_info['dmp_info'].append(dmp_info)
                    print(dmp_info)
                    ...
                    ogg_info['rep_info'].append(rep_info)
                    print(rep_info)
                    ...
                    ogg_info['rep2kafka_info'].append(rep2kafka_info)
