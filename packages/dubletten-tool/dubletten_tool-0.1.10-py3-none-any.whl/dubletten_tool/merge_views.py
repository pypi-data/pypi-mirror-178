...
from apis_core.apis_relations.tables import LabelTableBase  # , EntityDetailViewLabelTable,  get_generic_relations_table,
...
from .merge_functions import write_person_person_rels, MergeGroup
...
class GenericEntitiesDetailView(UserPassesTestMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        persons_ = [r.related_personB for r in PersonPerson.objects.filter(related_personA=instance, relation_type=merged_rel)]
...
def getRelationsHelperFunction(instance):
    rels = instance.personinstitution_set.values("id", "related_institution", "related_institution__name", "relation_type", "relation_type__name", "start_date_written", "end_date_written")
    rels = sorted(rels, key=lambda k: (k["relation_type__name"], k["related_institution__name"], ("1400" if not k["start_date_written"] else k["start_date_written"])), reverse=True)
...
class GetRelationEditor(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        merged_rel = PersonPersonRelation.objects.get(name="data merged from")
        persons = [r.related_personB for r in PersonPerson.objects.filter(related_personA=inst, relation_type=merged_rel)]
...
class CreateRelationView(View):
    ...
    def post(self, request):
        ...
        msg = True
        re_id = None
        per = get_object_or_404(Person, pk=request.POST.get("id", "-111")) # todo: check if this is best solution
        data = json.loads(request.POST.get("relation"))
        del data["id"]
        try: 
            temp = {}
            temp.update({"related_person":per})
            temp.update({"related_institution":Institution.objects.get(id=data["related_institution"])})
            temp.update({"relation_type":PersonInstitutionRelation.objects.get(id=data["relation_type"])})
