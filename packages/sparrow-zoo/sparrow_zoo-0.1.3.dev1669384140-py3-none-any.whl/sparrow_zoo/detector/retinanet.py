...
class RetinaNet(SparrowDetector):
    def __init__(
        ...
        self.bbox_xform_clip = torch.tensor(math.log(1000.0 / 16))
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        result["boxes"] = self._decode_box_offsets(
            result["bbox_regression"], self.anchors
        )
    ...
    def _decode_box_offsets(
        ...
        if box_offsets.device != self.bbox_xform_clip.device:
            self.bbox_xform_clip = self.bbox_xform_clip.to(box_offsets.device)
        ...
        dw = torch.clamp(dw, min=-torch.inf, max=self.bbox_xform_clip)
        dh = torch.clamp(dh, min=-torch.inf, max=self.bbox_xform_clip)
