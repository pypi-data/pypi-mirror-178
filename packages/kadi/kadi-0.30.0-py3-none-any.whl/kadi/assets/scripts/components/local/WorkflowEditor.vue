  ...
  methods: {
    ...
    loadWorkflow(url) {
      axios.get(url)
        .then((response) => {
          ...
          try {
            this.editor.fromFlow(response.data)
              .then((success) => {
                if (!success) {
                  kadi.alerts.warning('Could not fully reconstruct workflow.');
                }
              })
              .catch((error) => {
                console.error(error);
                kadi.alerts.danger('Error parsing workflow data.');
              })
        ...
        .catch((error) => kadi.alerts.danger('Error loading workflow.', {request: error.request}));
    },
    addTool(tool) {
