...
class Record(
    ...
    * **unit**: A unit the value corresponds to, which is only present when the type is
    ...
    * **validation**: An optional object containing additional validation instructions
      for the values of non-nested types. The following instructions are currently
      supported:

      * **required**: A boolean value indicating whether the value of the extra should
        be required, i.e. not ``null``.
      * **options**: An array containing an enumeration of possible values that the
        extra can have, which all need to match the type of the extra.
    """
