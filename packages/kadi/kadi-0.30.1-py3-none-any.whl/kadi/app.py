...
def _init_shell_context(app):
    ...
    def _shell_context():
        ...
        # Also import all database models.
        imports["Workflow"] = Workflow
...
def _check_database(app):
    ...
    if (
        app.environment == const.ENV_PRODUCTION
        and os.environ.get("KADI_APP_FROM_CLI") != "1"
    ):
        with app.app_context():
            if has_pending_revisions():
                raise KadiConfigurationError(
                    "The database schema is not up to date. Maybe you forgot to run"
                    " 'kadi db upgrade'?"
                )
