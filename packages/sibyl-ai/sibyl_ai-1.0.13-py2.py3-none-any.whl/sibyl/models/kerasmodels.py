...
class KerasDenseRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        self._build_model(X_val, y_val, out_act=self.out_act,
    ...
    def _build_model(self, X, y, out_act, loss, metrics):
        ...
        layers = []  # [InputLayer(X.shape[1] if len(X.shape) > 1 else 1)]
        for units in self.units:
            if self.dropout > 0: layers += [klrs.Dropout(self.dropout)]
            layers += [klrs.Dense(units, activation=self.activation,
                                  kernel_initializer=self.kernel_init)]
            if self.batch_norm: layers += [klrs.BatchNormalization()]
        out_shape = y.shape[1] if len(y.shape) > 1 else 1
        layers += [klrs.Dense(out_shape, activation=out_act)]
...
class KerasCNNRegressor(KerasDenseRegressor):
    ...
    def _build_model(self, X, y, out_act, loss, metrics):
        """ Build Keras model according to input parameters """
        layers = [klrs.Reshape(X.shape[1:]+(1,))] if len(X.shape[1:]) < 2 \
            else []
        for units in self.cnn_units:
            if self.dropout > 0: layers += [klrs.Dropout(self.dropout)]
            layers += [klrs.Conv1D(filters=units, kernel_size=2,
