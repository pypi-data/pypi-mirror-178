...
def main():
    ...
    if 'ls' in args or 'list' in args:
        if 'models' in args or 'model' in args:
            res = protector.get_entries(org, proj, 'models')
            listType = 'models'
        elif 'datasets' in args or 'dataset' in args:
            res = protector.get_entries(org, proj, 'datasets')
            listType = 'datasets'
        else:
    ...
    if 'create' in args and '-m' in args and '-d' in args and '-t' in args:
        ...
        if '-m' in args:
            ...
            protector.upload_model(org, proj, modelPath)
        if '-d' in args:
            datasetPath = args[args.index('-d')+1]
            protector.upload_dataset(org, proj, datasetPath)
    else:
        print(f'''
