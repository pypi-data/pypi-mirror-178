...
class WebBotMain:
    ...
    def __init__(self, port,browserName):
        ...
        if browserName=='edge':
            ...
            print('请注意')
            print('谷歌内核浏览器不能使用closepage，否则连接会断，不能往下操作, edge和360安全浏览器可以使用')
        self.port = port
    ...
    @classmethod
    def build(cls, address="127.0.0.1", port=26678, browserName='chrome', debugPort=0, userDataDir="./UserData",
              browserPath='null', argument="null") -> "WebBotMain":
        """
        ...
        return WebBotMain(port,browserName)
    ...
    #    元素操作   #
    # #############
    def clickElement(self, xpath, timeout=3.0) -> bool:
        """
        点击元素
    ...
    def setElementValue(self, xpath, value, timeout=3.0) -> bool:
        """
        设置编辑框内容
        :param xpath: 字符串型，元素路径
    ...
    def getElementText(self, xpath, timeout=3) -> str:
        """
        获取文本
        :param xpath: 字符串型，元素路径
        """
    ...
    def clickMouseByXpath(self, xpath, msg, timeout=3) -> bool:
        """
        通过xpath点击鼠标(元素中心点)
        :param xpath: 字符串型，元素路径
        :param msg: 整型，单击左键:1  单击右键:2  按下左键:3  弹起左键:4  按下右键:5  弹起右键:6  双击左键:7
    ...
    def setCookie(self, cookieParam) -> bool:
        ...
        strData = (
        "setCookie", name, value, url, domain, path, secure, httpOnly, sameSite, expires, priority, sameParty,
        sourceScheme, sourcePort, partitionKey)
