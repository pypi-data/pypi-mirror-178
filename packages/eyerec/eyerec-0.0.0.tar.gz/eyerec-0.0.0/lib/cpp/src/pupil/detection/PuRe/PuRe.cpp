...
void PuRe::estimateParameters(const int rows, const int cols)
{
    ...
    maxPupilDiameterPx = static_cast<int>(
        maxCanthiDistancePx * (maxPupilDiameterMM / meanCanthiDistanceMM));
    minPupilDiameterPx = static_cast<int>(
...
Mat PuRe::canny(const Mat& in, const bool blurImage, const bool useL2,
    const int bins, const float nonEdgePixelsRatio,
    const float lowHighThresholdRatio)
{
    (void)useL2;
    /*
    ...
    // Ratio
    int sum = 0;
    const int nonEdgePixels
        = static_cast<int>(nonEdgePixelsRatio * in.rows * in.cols);
     ...
     */
    enum { NON_EDGE = 0, POSSIBLE_EDGE = 128, EDGE = 255 };
    ...
    for (int i = 1; i < magnitude.rows - 1; i++) {
        ...
        for (int j = 1; j < magnitude.cols - 1; j++) {
            ...
            if (y < tg22_5x) {
                if (m > p_res[j - 1] && m >= p_res[j + 1]) edgeTypePtr[j] = val;
            } else {
                float tg67_5x = tg67_5 * x;
                if (y > tg67_5x) {
                    if (m > p_res_b[j] && m >= p_res_t[j]) edgeTypePtr[j] = val;
                } else {
                    if ((iy <= 0) == (ix <= 0)) {
    ...
    for (int i = 1; i < edgeType.rows - 1; i++) {
        for (int j = 1; j < edgeType.cols - 1; j++) {
            ...
            if (edgeType.data[idx + j] != EDGE
                || edge.data[idx + j] != NON_EDGE)
                continue;
            ...
            while (akt_idx < lines_idx) {
                ...
                if (akt_pos - edgeType.cols - 1 < 0
                    || akt_pos + edgeType.cols + 1 >= area)
                    continue;

                for (int k1 = -1; k1 < 2; k1++)
                    for (int k2 = -1; k2 < 2; k2++) {
                        if (edge.data[(akt_pos + (k1 * edgeType.cols)) + k2]
                                != NON_EDGE
                            || edgeType
                                    .data[(akt_pos + (k1 * edgeType.cols)) + k2]
                                == NON_EDGE)
                            continue;
...
void PuRe::filterEdges(cv::Mat& edges)
{
    ...
    for (int j = start_y; j < end_y; j++)
        for (int i = start_x; i < end_x; i++) {
            ...
            if (box[4]) {
                box[1] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i)]);
                box[3] = static_cast<uchar>(
                    edges.data[(edges.cols * (j)) + (i - 1)]);
                box[5] = static_cast<uchar>(
                    edges.data[(edges.cols * (j)) + (i + 1)]);
                box[7] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i)]);
    ...
    for (int j = start_y; j < end_y; j++)
        for (int i = start_x; i < end_x; i++) {
            ...
            if (box[4]) {
                box[0] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i - 1)]);
                box[1] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i)]);
                box[2] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i + 1)]);
                ...
                box[6] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i - 1)]);
                box[7] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i)]);
                box[8] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i + 1)]);

                // external
                box[9] = static_cast<uchar>(
                    edges.data[(edges.cols * (j)) + (i + 2)]);
                box[10] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i)]);

                box[11] = static_cast<uchar>(
                    edges.data[(edges.cols * (j)) + (i + 3)]);
                box[12] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i + 2)]);
                box[13] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i + 2)]);

                box[14] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i)]);
                box[15] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i - 1)]);
                box[16] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i + 1)]);
    ...
    for (int j = start_y; j < end_y; j++)
        for (int i = start_x; i < end_x; i++) {
            ...
            if (box[4]) {
                ...
                box[17] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 3)) + (i - 1)]);
                box[18] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 3)) + (i + 1)]);
                box[19] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i - 1)]);
                box[20] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i + 1)]);

                box[21] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i + 3)]);
                box[22] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i - 3)]);
                box[23] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i + 3)]);
                box[24] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i - 3)]);

                box[25] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 2)) + (i - 2)]);
                box[26] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i + 2)]);
                box[27] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 2)) + (i + 2)]);
                box[28] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i - 2)]);

                box[29] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 3)) + (i - 3)]);
                box[30] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i + 3)]);
                box[31] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 3)) + (i + 3)]);
                box[32] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i - 3)]);
                ...
                if (box[0] && box[13] && box[17] && box[8] && box[11]
                    && box[21])
                    edges.data[(edges.cols * (j)) + (i)] = 0;
                if (box[2] && box[14] && box[18] && box[6] && box[12]
                    && box[22])
                    edges.data[(edges.cols * (j)) + (i)] = 0;
                if (box[6] && box[15] && box[19] && box[2] && box[9] && box[23])
                    edges.data[(edges.cols * (j)) + (i)] = 0;
                if (box[8] && box[16] && box[20] && box[0] && box[10]
                    && box[24])
                    edges.data[(edges.cols * (j)) + (i)] = 0;
                ...
                uchar box2[18];
                box2[1] = static_cast<uchar>(
                    edges.data[(edges.cols * (j)) + (i - 1)]);

                box2[2] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i - 2)]);
                box2[3] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 2)) + (i - 3)]);

                box2[4] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i + 1)]);
                box2[5] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 2)) + (i + 2)]);

                box2[6] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i - 2)]);
                box2[7] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i - 3)]);

                box2[8] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i + 1)]);
                box2[9] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i + 2)]);

                box2[10] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 1)) + (i)]);

                box2[15] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 1)) + (i - 1)]);
                box2[16] = static_cast<uchar>(
                    edges.data[(edges.cols * (j - 2)) + (i - 2)]);

                box2[11] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i + 1)]);
                box2[12] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i + 2)]);

                box2[13] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 2)) + (i - 1)]);
                box2[14] = static_cast<uchar>(
                    edges.data[(edges.cols * (j + 3)) + (i - 2)]);
...
void PuRe::findPupilEdgeCandidates(
    const Mat& intensityImage, Mat& edge, vector<PupilCandidate>& candidates)
{
    /* Find all lines
    ...
    findContours(
        edge, curves, hierarchy, cv::RETR_LIST, cv::CHAIN_APPROX_TC89_KCOS);
...
//#define DBG_EDGE_COMBINATION
            ...
            if (intersection.area() >= min<int>(pc->combinationRegion.area(),
                                           pc2->combinationRegion.area()))
                continue;

            vector<Point> mergedPoints = pc->points;
            mergedPoints.insert(
                mergedPoints.end(), pc2->points.begin(), pc2->points.end());
    ...
    candidates.insert(
        candidates.end(), mergedCandidates.begin(), mergedCandidates.end());
}

void PuRe::searchInnerCandidates(
    vector<PupilCandidate>& candidates, PupilCandidate& candidate)
{
    if (candidates.size() <= 1) return;
...
Pupil PuRe::implDetect(const cv::Mat& frame, DetectionParameters params)
{
    ...
    estimateParameters(static_cast<int>(scalingRatio * frame.rows),
        static_cast<int>(scalingRatio * frame.cols));
    if (params.userMinPupilDiameterPx > 0)
        minPupilDiameterPx
            = static_cast<int>(scalingRatio * params.userMinPupilDiameterPx);
    if (params.userMaxPupilDiameterPx > 0)
        maxPupilDiameterPx
            = static_cast<int>(scalingRatio * params.userMaxPupilDiameterPx);

    // Downscaling
    Mat downscaled;
    resize(frame(params.roi), downscaled, Size(), scalingRatio, scalingRatio,
        cv::INTER_LINEAR);
...
inline bool PupilCandidate::isValid(const cv::Mat& intensityImage,
    const int& minPupilDiameterPx, const int& maxPupilDiameterPx,
    const int bias)
{
...
inline bool PupilCandidate::validateOutlineContrast(
    const Mat& intensityImage, const int& bias)
{
    outlineContrast = PupilDetectionMethod::outlineContrastConfidence(
        intensityImage, outline, bias);
    if (outlineContrast <= 0) return false;
    return true;
}
...
inline bool PupilCandidate::validateAnchorDistribution()
{
    ...
    anchorDistribution = anchorPointSlices.count()
        / static_cast<float>(anchorPointSlices.size());
    return true;
}

inline bool PupilCandidate::validityCheck(
