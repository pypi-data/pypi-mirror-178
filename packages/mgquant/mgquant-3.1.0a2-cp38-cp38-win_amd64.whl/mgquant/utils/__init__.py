...
from mgquant.utils.exception import CustomError, CustomException
from mgquant.const import EXC_TYPE, INSTRUMENT_TYPE, DEFAULT_ACCOUNT_TYPE, UNDERLYING_SYMBOL_PATTERN, NIGHT_TRADING_NS
from mgquant.utils.datetime_func import TimeRange
from mgquant.utils.default_future_info import STOCK_TRADING_PERIOD, TRADING_PERIOD_DICT
from mgquant.utils.i18n import gettext as _
from mgquant.utils.py2 import lru_cache
...
class MgAttrDict(object):
    ...
    def update(self, other):
        MgAttrDict._update_dict_recursive(self, other)
    ...
    def _update_dict_recursive(target, other):
        if isinstance(other, MgAttrDict):
            other = other.__dict__
        if isinstance(target, MgAttrDict):
            target = target.__dict__

        for k, v in six.iteritems(other):
            if isinstance(v, collections.Mapping):
                r = MgAttrDict._update_dict_recursive(target.get(k, {}), v)
                target[k] = r
            else:
                target[k] = other[k]
        return target
...
def to_sector_name(s):
    from mgquant.model.instrument import SectorCode, SectorCodeItem
...
def to_industry_code(s):
    from mgquant.model.instrument import IndustryCode, IndustryCodeItem
...
def run_when_strategy_not_hold(func):
    from mgquant.environment import Environment
    from mgquant.utils.logger import system_log
...
def run_with_user_log_disabled(disabled=True):
    from mgquant.utils.logger import user_log
