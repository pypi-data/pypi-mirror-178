...
class Api:
    ...
    def _call_api(
        ...
        try:
            ...
            headers["X-Collimator-API-Caller"] = "pycollimator"
    ...
    def simulation_results(cls, model_uuid: str, simulation_uuid: str, retries=3) -> str:
        api = f"{Api.BASE_PATH}/models/{model_uuid}/simulations/{simulation_uuid}"
        api += "/process_results?files=continuous_results.csv"

        s3_urls_batch_resp = cls._call_api(api, method="GET", retries=retries)
        urls = cls._get_result_urls(s3_urls_batch_resp)

        return cls._get_file(urls[0])

    @classmethod
    def signal_results(cls, model_uuid: str, simulation_uuid: str, paths: T.List[str], retries=3) -> T.List[str]:
        api = f"{Api.BASE_PATH}/models/{model_uuid}/simulations/{simulation_uuid}"
    ...
    @staticmethod
    def _get_result_urls(batch_resp: str) -> T.List[str]:
        errors = []
        urls = []
        ...
        for s3_url_resp in batch_resp["s3_urls"]:
            ...
            if url is None:
                errors.append(f"Error for signal '{s3_url_resp.get('name')}'\n{s3_url_resp.get('error')}")
            else:
                urls.append(url)
