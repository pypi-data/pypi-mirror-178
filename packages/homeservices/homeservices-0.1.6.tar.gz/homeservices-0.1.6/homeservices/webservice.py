...
class HomeServices(ManagedClass):
    ...
    def _read_last_measures(self, number_of_measures, sensor_name):
        sensor_table = {'jardín': 2, 'cocina': 1, 'buhardilla': 3}

        if sensor_name in sensor_table.keys():
            query = "SELECT * from DHT22 WHERE sensorid='{}' ORDER BY time DESC LIMIT {}".format(sensor_table[sensor_name], 
                                                                                                number_of_measures)
    ...
    def alexa_intent(self):
        ...
        if request.method == 'GET':
            ...
            if points and len(points) == 4:
                ...
                response_phrase = "Hace {} grados{}, y la humedad es del {:.0f} por ciento."\
                ...
                response = make_response("Ese sensor es desconocido", 404)
