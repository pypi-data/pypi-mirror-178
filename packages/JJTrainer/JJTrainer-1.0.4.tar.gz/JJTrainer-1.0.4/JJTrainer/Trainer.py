...
class Config(object):
    ...
    def to_json(self, path):
        dicts = ['一般配置', self.general_dict, '训练参数配置', self.train_dict, '模型参数配置', self.model_dict,
                 '模型检查点配置', self.checkpoint_dict]
...
class Trainer(object):
    ...
    def fit(self, model, criterion, train_dataset: Dataset, val_dataset: Dataset = None, metric=None,
        ...
        arg_dict = {
            ...
            'optimizer': optimizer, 'schedule': schedule, 'feature_dict': feature_dict,
            'train_batch_size': train_batch_size,
            'val_batch_size': val_batch_size, 'num_epochs': num_epochs, 'num_steps': num_steps,
    ...
    def fit_(self, local_rank, model, criterion, train_dataset: Dataset, val_dataset: Dataset = None, metric=None,
        ...
        if self.config.general_dict['gpu_nums'] > 1:
            ...
            self.model = nn.parallel.distributed.DistributedDataParallel(self.model.cuda(self.local_rank),
        ...
        if self.config.train_dict['use_fp16']:
            if self.config.general_dict['gpu_nums'] == 1 or (
                    self.config.general_dict['gpu_nums'] > 1 and self.local_rank == 0):
                print('Use FP16!!!')
        ...
        for i in range(self.start_epoch, num_epochs):
            ...
            if self.local_rank < 0 or self.local_rank == 0:
                self.log('Val_AvgLoss_Epoch', self.val_avg_loss, self.actual_epoch)

            if self.val_metric_value is not None:
                if self.local_rank < 0 or self.local_rank == 0:
                    self.log(f'Val_{self.metric.__name__}_Epoch', self.val_metric_value, self.actual_epoch)
            ...
            if self.config.checkpoint_dict['use_checkpoint']:
                self.save_checkpoint(self.train_avg_loss, self.val_avg_loss, self.train_metric_value,
                                     self.val_metric_value)
        ...
        if self.config.checkpoint_dict['use_checkpoint'] and self.config.checkpoint_dict[
            'save_last'] and self.local_rank <= 0:
            self.save_last()
    ...
    def train_per_epoch(self,
        ...
        for i, batch in enumerate(loader_bar):
            ...
            if self.actual_step % self.log_step_interval == 0 and (
                    self.log_step_interval != len(self.train_loader)) and self.local_rank <= 0:
                self.model.eval()
            ...
            if self.actual_step == self.num_steps:
                if self.local_rank <= 0:
                    print(
                        f"The number of steps has reached {self.actual_step}, terminate the training, save the final weight.")
                    self.val_avg_loss, self.val_metric_value = self.val_per_epoch(use_tqdm=True, use_posfix=True)
                    self.savemodel(
                        f'Final_Step{self.actual_step}_val_{self.metric.__name__}_{round(self.val_metric_value, 4)}_' +
    ...
    def accumulate_gradient(self):
        if self.config.train_dict['accumulate_num'] < 0 or self.actual_accumulate_num == self.config.train_dict[
            'accumulate_num'] - 1:
            if self.config.train_dict['use_fp16']:
    ...
    def fit_init(self):
        ...
        if self.config.general_dict['experiment_name'] == 'auto':
            ...
            self.experiment_path = os.path.join(self.config.general_dict['log_dir'],
                                                self.config.general_dict['experiment_name'])
