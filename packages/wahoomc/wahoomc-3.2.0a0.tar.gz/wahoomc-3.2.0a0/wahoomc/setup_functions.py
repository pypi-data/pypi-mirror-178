...
def check_installed_version_against_latest_pypi():
    ...
    response = requests.get('https://pypi.org/pypi/wahoomc/json')
    latest_version = response.json()['info']['version']

    if pkg_resources.parse_version(VERSION) > pkg_resources.parse_version(latest_version):
        log.info('# A new version of wahoomc is available: %s. You have version "%s" installed \
                    \nYou can upgrade with "pip install wahoomc --upgrade" \
                    and read the release notes here: \n- https://github.com/treee111/wahooMapsCreator/releases/latest',
