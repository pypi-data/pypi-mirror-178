...
<tr class="form-group${required_tr_class}">
    ...
    % elif data['type'] == 'Int':
        <input type="text" class="form-control input-sm touchspin" data-bts-min="${data.get('min') or -4294967295}" data-bts-max="${data.get('max') or 4294967295}" id="${id}" name="${data['name']}"${required_input} />
        ...
        <div id="api-doc-collapse-${service_id}" class="collapse out api-doc-service-collapse" data-parent="#api-doc-${module_id}" role="tabpanel" aria-labelledby="api-doc-heading-${service_id}">
            ...
            % endif
                    <div class="pull-right">
                        <button class="btn btn-primary" type="submit" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> ${translate('Send request')}">${translate('Send request')}</button>
                    </div>
                </form>
            </div><!-- .panel-body -->
