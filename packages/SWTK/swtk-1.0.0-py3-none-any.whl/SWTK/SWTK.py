...
The user can get the top N lines by specifying the -n flag. The user can also use this flag to get the most normal lines by using a negative number.
...
Example:
SWTK-LAD input.txt
SWTK-LAD -i input.txt -n 10
SWTK-LAD -i input.txt -n 10 -o output.txt
SWTK-LAD -i input.txt -n 10 -o output.txt -s  #Default model name is input.model
SWTK-LAD -i input.txt -n 10 -o output.txt -m input.model #Use the saved model
#Use the saved model and update the model
SWTK-LAD -i input.txt -n 10 -o output.txt -s -m input.model
'''
...
def prepare_input(input):
    ...
    # If the host is linux or mac, check if the rust program is accessible
        ...
        # Get the path of the rust program
        path = os.getcwd()
        TriePath = os.path.abspath(
            path + "/.anomaly_analysis/target/debug/Trie")
        # Check if the rust program is accessible
        if not os.path.isfile(TriePath):
            ...
            # Rename the anomaly_anoalysis folder to hidden
            os.system("mv anomaly_analysis .anomaly_analysis")
...
def main():
    ...
    parser.add_argument(
        "-n", "--top", help="Number of lines to print. Default is 10", default=None)
    parser.add_argument(
        "-o", "--output", help="Output file name. Default is output.txt", default=False)
    parser.add_argument(
