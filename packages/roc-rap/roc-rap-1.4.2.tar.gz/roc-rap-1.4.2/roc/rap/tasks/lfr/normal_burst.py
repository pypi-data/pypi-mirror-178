...
def set_asm(fdata, freq):
    ...
    asm_blks = np.empty([nblk.shape[0], np.max(nblk), 25], dtype=np.float32)
    asm_blks[:] = np.dstack(
...
def set_bp1(fdata, freq, task):
    ...
    # sign (1 bit)...
    sx_sign = 1 - (2 * bp_lib.Uint16ToUint8(fdata["PA_LFR_SC_BP1_SX_" + freq][:, :n], 15, 0x1).astype(np.float))
    ...
    # sign (1 bit)...
    vphi_sign = 1 - (2 * bp_lib.Uint16ToUint8(fdata["PA_LFR_SC_BP1_VPHI_" + freq][:, :n], 15, 0x1).astype(np.float))
