...
class SoPFunction(SoPService):
    ...
    def _wrapper(self, execute_request: SoPExecuteRequest) -> bool:
        ...
        print(f'[_hamdle_mqtt_message] recv_time: {time.time()}')
        ...
        try:
            ...
            self._running = True
            execute_request._result = func_timeout(
            ...
            # print_error(e)
            SOPLOG_DEBUG(
                f'[FUNC TIMEOUT] function {self._name} by scenario {execute_msg._scenario} was timeout!!!', 'red')
            error = SoPErrorType.TIMEOUT
        except Exception as e:
            print_error(e)
            SOPLOG_DEBUG(
                f'[FUNC ERROR] function {self._name} by scenario {execute_msg._scenario} was failed while executing!!!', 'red')
            error = SoPErrorType.FAIL
        else:
            SOPLOG_DEBUG(
                f'[FUNC FINISH] function {self._name} finish. -> return value : {execute_request._result}, duration: {execute_duration:.4f} Sec', 'green')
            error = SoPErrorType.NO_ERROR
        finally:
            self._running = False

            self._send_TM_RESULT_EXECUTE(execute_msg._function_name, execute_msg._thing_name,
                                         self._return_type, execute_request._result, execute_msg._scenario, error,
                                         middleware_name=execute_request._trigger_msg._middleware_name, request_ID=execute_request._trigger_msg._request_ID)
