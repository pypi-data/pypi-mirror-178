...
class DerivationAgent(ABC):
                ...
                for derivation in derivationAndStatusType:
                    ...
                    try:
                        ...
                        if statusType == 'REQUESTED':
                            ...
                            if DERIVATION_SPARQL_ONTODERIVATION_DERIVATIONASYN in immediateUpstreamDerivationToUpdate:
                                ...
                                if not bool(self.derivation_client.derivation_client.checkImmediateUpstreamDerivation(derivation)):
                                    ...
                                    if not progressToJob:
                                        ...
                                        agent_input_key = str(DERIVATION_CLIENT_AGENT_INPUT_KEY)
    ...
    def handle_sync_derivations(self):
        ...
        if self.validate_inputs(requestParams):
            ...
            derivationIRI = requestParams[DERIVATION_CLIENT_DERIVATION_KEY]
            derivationType = requestParams[DERIVATION_CLIENT_DERIVATION_TYPE_KEY]
            syncNewInfoFlag = requestParams[DERIVATION_CLIENT_SYNC_NEW_INFO_FLAG]

            # serialises DerivationInputs objects from JSONObject
            inputs = self.jpsBaseLib_view.DerivationInputs(requestParams[DERIVATION_CLIENT_AGENT_INPUT_KEY], derivationIRI)
            ...
            if not syncNewInfoFlag:
                outputs.setOldEntitiesMap(requestParams[DERIVATION_CLIENT_BELONGSTO_KEY])
                outputs.setOldEntitiesDownstreamDerivationMap(requestParams[DERIVATION_CLIENT_DOWNSTREAMDERIVATION_KEY])
            ...
            if syncNewInfoFlag:
                agentServiceIRI = requestParams[DERIVATION_CLIENT_AGENT_IRI_KEY]
                ...
                res[DERIVATION_OURPUTS_RETRIEVED_INPUTS_TIMESTAMP_KEY] = outputs.getRetrievedInputsAt()
                res[DERIVATION_CLIENT_AGENT_OUTPUT_KEY] = json.loads(str(outputs.getNewEntitiesJsonMap()))
            ...
            if not derivation.isDerivationAsyn() and not derivation.isDerivationWithTimeSeries():
                ...
                if triplesChangedForSure:
                    ...
                    res[DERIVATION_OURPUTS_RETRIEVED_INPUTS_TIMESTAMP_KEY] = updated.getTimestamp()
                    res[DERIVATION_CLIENT_AGENT_OUTPUT_KEY] = json.loads(str(updated.getBelongsToMap()))
            ...
            res[DERIVATION_CLIENT_AGENT_OUTPUT_KEY] = DERIVATION_AGENT_EMPTY_REQUEST_MSG
