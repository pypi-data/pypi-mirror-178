...
def gen_sample_series(number: int = 40, dst=f'{pet_dst}/generated_sample_series_{datetime.date.today()}.xlsx'):
    order = {
        '姓名.n': '',  # ""生成默认的随机名字，也可以设置姓名字符串，['赵钱孙李','微甜地平天下'],
        '成绩.i': ''
    }
...
def gen_sample_dataframe(sample_order=sample_order,number: int = 40, dst=f'{pet_dst}/generated_sample_dataframe_{datetime.date.today()}.xlsx'):
    print('订单格式*'*number)
    print(sample_order)
    print("*"*number)
