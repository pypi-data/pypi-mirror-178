...
def generator(order: dict = sample_order, number: int = 40,
              dst: str = f'd:/generated_dataset_{datetime.date.today()}.xlsx') :
    '''
    根据订单生成数据
    :param order: 订单字典
    :param number: 数据元素个数
...
def gen_sample_series(number: int = 40, dst=f'd:/generated_sample_series_{datetime.date.today()}.xlsx'):
    order = {
        '姓名.n': '',  # ""生成默认的随机名字，也可以设置姓名字符串，['赵钱孙李','微甜地平天下'],
        '成绩.i': ''
    }
...
def gen_sample_dataframe(number: int = 40, dst=f'd:/generated_sample_dataframe_{datetime.date.today()}.xlsx'):
    print('订单格式*'*number)
    print(sample_order)
    print("*"*number)
    return generator(sample_order, number, dst)
...
if __name__ == '__main__':
    ...
    df = add_noise(generator(sample_order, 10))
    print(df)
    print(gen_sample_series(30))
