...
class SMSFormMixin(object):
    ...
    def generate_token(self, device, by_call=False):
        ...
        self.request.session[settings.OTP_SMS_SESSION_KEY_BY_CALL] = by_call
    ...
    def verify_token(self, number, token):
        ...
        by_call = self.request.session.get(settings.OTP_SMS_SESSION_KEY_BY_CALL, False)
        return device and device.verify_token(token, by_call=by_call)
