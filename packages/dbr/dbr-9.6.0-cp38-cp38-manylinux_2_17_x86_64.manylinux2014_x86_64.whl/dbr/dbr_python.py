...
class FrameDecodingParameters:
    ...
    def __init__(self, parameters):
        ...
        self.duplicate_forget_time = parameters["DuplicateForgetTime"]
    ...
    def update_parameters(self, parameters):
        ...
        parameters["DuplicateForgetTime"] = self.duplicate_forget_time
        parameters["Orientation"] = self.orientation
...
class PDFDetailedResult:
    ...
    - has_left_row_indicator <int>:Whether the left row indicator of the PDF417 code exists.
    
    - has_right_row_indicator <int>:Whether the right row indicator of the PDF417 code exists.
    ...
    def __init__(self, detailed_result):
        ...
        self.columns = detailed_result["Columns"]
        self.error_correction_level = detailed_result["ErrorCorrectionLevel"]
        self.has_left_row_indicator= detailed_result["HasLeftRowIndicator"]
        self.has_right_row_indicator = detailed_result["HasRightRowIndicator"]
...
class ExtendedResult:
    ...
    def __init__(self, extended_result):
        ...
        self._barcode_format_string_2 = extended_result["BarcodeFormatString_2"]
    ...
    @property
    def barcode_format_string_2(self):
        warnings.warn("Deprecated, use barcode_format_string instead.", DeprecationWarning)
        return self._barcode_format_string_2
...
class LocalizationResult:
    ...
    def __init__(self, localization_result):
        ...
        self._barcode_format_string_2 = localization_result["BarcodeFormatString_2"]
        ...
        self.transformation_matrix = localization_result["TransformationMatrix"]
    ...
    def _convert_to_inner(self):
        ...
        localization_result["TransformationMatrix"] = self.transformation_matrix
...
class TextResult:
    ...
    def __init__(self, text_result):
        ...
        self._barcode_format_string_2 = text_result["BarcodeFormatString_2"]
...
class ImageData:
    ...
    def _convert_to_inner(self):
        ...
        image_data["Orientation"] = self.orientation
        return image_data
...
class BarcodeReader():
    ...
    def __init__(self,instancemod = False):
        ''' Init Function '''
        self.version = "dbr-python 9.6"
        self.__dbr = DynamsoftBarcodeReader()
        if instancemod == False:
    ...
    def recycle_instance(self):
         self.__dbr.RecycleInstance()
...
#region License Functions
    ...
    def init_license_from_server(self, license_server, license_key):
        ''' Initializes barcode reader license and connects to the specified server for online verification. 
            ...
            :return error         <tuple> : error_code = error[0], error_message = error[1], if error_code != EnumErrorCode.DBR_OK, 
                you can get the detailed error message by error_message.
        '''
        warnings.warn("Deprecated, use init_license instead.", DeprecationWarning)
    ...
    @staticmethod
    def set_device_friendly_name(name):
        ret = BarcodeReader.__dbr__.SetDeviceFriendlyName(name)
        return ret

    @staticmethod
    def transform_coordinates(original_point,transformation_matrix):
        ''' Transform the coordinates of a point based on the given transformation matrix.
            :param original_point <Point> : The original point that needs to be transformed.
            :param transformation_matrix <list<double> > : The 3x3 matrix used for coordinate transformation.
            :return parameters  <class Point> : The point after transformation.
        '''
        innerPt = original_point._convert_to_inner()
        ret = BarcodeReader.__dbr__.TransformCoords(innerPt,transformation_matrix)
        return ret

    @staticmethod
    def get_instance():
        ''' Gets an idle Dynamsoft Barcode Reader instance running on concurrent instance mode.
            :return instance  <class BarcodeReader> : Returns an idle Dynamsoft Barcode Reader instance running on concurrent instance mode. If failed, returns NULL.
        '''
        reader = BarcodeReader(True)
        ret = reader.__dbr.GetInstance()
        if ret == 0:
            return reader
        else:
            return None
    ...
    @staticmethod
    def set_max_concurrent_instance_count(count_for_this_device,count_for_this_process = 0):
        ''' Sets the max concurrent instance count used for current device and process.
            :param count_for_this_device <int> : The maximum number of concurrent instances that the current device can run.
            :param count_for_this_process <int> : The maximum number of concurrent instances that the current process can run.
        '''
        BarcodeReader.__dbr__.SetMaxConcurrentInstanceCount(count_for_this_device,count_for_this_process)
...
#region Template Settings Funtions
    ...
    def is_instance_valid(self):
        return self.__dbr.IsInstanceValid()
#endregion
...
#region Image Decoding Functions
    ...
    def decode_buffer(self, image, image_pixel_format=EnumImagePixelFormat.IPF_RGB_888, template_name="",orientation = 0):
        ...
        error_code = self.__dbr.DecodeBuffer(image, height, width, stride, image_pixel_format, template_name,orientation)
        if error_code == EnumErrorCode.DBR_OK or error_code == EnumErrorCode.DBRERR_LICENSE_EXPIRED or error_code == EnumErrorCode.DBRERR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_RECOGNITION_TIMEOUT or \
            error_code == EnumErrorCode.DBRERR_1D_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_QR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_PDF417_LICENSE_INVALID or \
            error_code == EnumErrorCode.DBRERR_AZTEC_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_DATAMATRIX_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_GS1_DATABAR_LICENSE_INVALID or \
            error_code == EnumErrorCode.DBRERR_GS1_COMPOSITE_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_MAXICODE_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_PATCHCODE_LICENSE_INVALID or \
    ...
    def decode_buffer_manually(self, buffer, width, height, stride, image_pixel_format, template_name="",orientation = 0):
        ''' Decodes barcodes from the memory buffer containing image pixels in defined format. 
            ...
            :exception BarcodeReaderError : If error happens, this function will throw a BarcodeReaderError exception that can report the detailed error message.
        '''
        error_code = self.__dbr.DecodeBufferManually(buffer, height, width, stride, image_pixel_format, template_name,orientation)
        if error_code == EnumErrorCode.DBR_OK or error_code == EnumErrorCode.DBRERR_LICENSE_EXPIRED or error_code == EnumErrorCode.DBRERR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_RECOGNITION_TIMEOUT or \
            error_code == EnumErrorCode.DBRERR_1D_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_QR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_PDF417_LICENSE_INVALID or \
            error_code == EnumErrorCode.DBRERR_AZTEC_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_DATAMATRIX_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_GS1_DATABAR_LICENSE_INVALID or \
            error_code == EnumErrorCode.DBRERR_GS1_COMPOSITE_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_MAXICODE_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_PATCHCODE_LICENSE_INVALID or \
    ...
    def decode_file_stream(self, file_stream, template_name=""):
        ''' Decodes barcodes from an image file in memory.
            :param file_stream <bytes or bytearray> : The image file bytes in memory.
            :param template_name(optional) <str> : The template name. 
            :return text_results <list[class TextResult]> : All text results.
            :exception BarcodeReaderError : If error happens, this function will throw a BarcodeReaderError exception that can report the detailed error message.
        '''
    ...
    def decode_base64_string(self, base64String, template_name=""):
        ''' Decodes barcodes from an image file in base64String.
            :param base64String <str> : The base64String image file bytes.
            :param template_name(optional) <str> : The template name. 
            :return text_results <list[class TextResult]> : All text results.
            :exception BarcodeReaderError : If error happens, this function will throw a BarcodeReaderError exception that can report the detailed error message.
        '''
        error_code = self.__dbr.DecodeBase64String(base64String, template_name)
        if error_code == EnumErrorCode.DBR_OK or error_code == EnumErrorCode.DBRERR_LICENSE_EXPIRED or error_code == EnumErrorCode.DBRERR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_RECOGNITION_TIMEOUT or \
            error_code == EnumErrorCode.DBRERR_1D_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_QR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_PDF417_LICENSE_INVALID or \
            error_code == EnumErrorCode.DBRERR_AZTEC_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_DATAMATRIX_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_GS1_DATABAR_LICENSE_INVALID or \
            error_code == EnumErrorCode.DBRERR_GS1_COMPOSITE_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_MAXICODE_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_PATCHCODE_LICENSE_INVALID or \
...
#region Frame Decoding Functions
    ...
    def start_video_mode(self, frame_decoding_parameters, text_result_callback_func, template_name="", intermediate_result_callback_func=None, error_callback_func=None, unique_barcode_callback_func = None, user_data=None):
        ''' Starts a new thread to decode barcodes from the inner frame queue.
            ...
            :param error_callback_func(optional) <function pointer> : Sets callback function to process errors generated during frame decoding. 
                                                                ...
                                                                If you would like to learn how to use it, please refer to the sample<https://github.com/Dynamsoft/python-barcode/blob/master/samples/test_DecodeVideoByCamera.py>.
            :param unique_barcode_callback_func <function pointer> : Sets callback function to process unique text results generated during frame decoding. 
                                                                This callback function pointer must follow the following format: callback_func_name(frameId, results, user_data); 
                                                                Or you can inherit the abstract class TextResultResultCallBack to implement the abstract method text_results_callback_func.
                                                                If you would like to learn how to use it, please refer to the sample<https://github.com/Dynamsoft/python-barcode/blob/master/samples/test_DecodeVideoByCamera.py>.
        ...
        try:
            ...
            print(key)
        error_code = self.__dbr.StartVideoMode(cp_frame_decoding_parameters, text_result_callback_func, template_name, intermediate_result_callback_func, error_callback_func,unique_barcode_callback_func, user_data)
