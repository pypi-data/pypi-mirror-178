...
class VelocityContainer():
    ...
    def __init__(self, parameters, Met, x):
        ...
        for k in range(0, chebIts):
            ...
            self.U[:N, 0, k] = Met.wind_U(lower_z[:N], U_scale)
            self.V[:N, 0, k] = Met.wind_V(lower_z[:N], U_scale)
            self.Ws[:N, 0, k, :] = Met.settling_speed(
                parameters, lower_z[:N], Ws_scale)
            upper_z = np.zeros((N-1), dtype=np.float64)
            upper_z[:N-1] = 2/(1 - x[:N-1, k])*H
            self.z[:N-1, 1, k] = upper_z[:N-1]
            self.U[:N-1, 1, k] = Met.wind_U(upper_z[:N-1], U_scale)
            self.V[:N-1, 1, k] = Met.wind_V(upper_z[:N-1], U_scale)
            self.Ws[:N-1, 1, k, :] = Met.settling_speed(parameters,
                                                        upper_z[:N-1],
                                                        Ws_scale)
            self.fd_Ws(k, H)
