...
def verify_cisco_support_api_data(
    ...
    if no_error:
        ...
        print(f"'Verify Cisco support {api.upper()} API data' -> {success(True)}")
    ...
    error = (
        ...
        f"'Verify Cisco support {api.upper()} API data' -> {success(False)}\n"
    )
...
def get_sni_owner_coverage_by_serial_number(serial_dict: dict, api_creds: tuple) -> dict:
    ...
    for index in range(0, len(list(serial_dict.keys())), MAX_SR_NO):
        ...
        # Ending for loop as iterable exhausted
            ...
            verify_cisco_support_api_data(api="sni", iterable=response, verbose=True)
...
def get_sni_coverage_summary_by_serial_numbers(serial_dict: dict, api_creds: tuple) -> dict:
    ...
    for index in range(0, len(list(serial_dict.keys())), MAX_SR_NO):
        ...
        for _ in range(RETRY_ATTEMPTS):
            ...
            # Break out of the range() loop if ErrorResponse is not present in the API response
            if (
                "pagination_response_record" in response
                and response["pagination_response_record"] is not None
            ):
...
def get_eox_by_serial_numbers(serial_dict: dict, api_creds: tuple) -> dict:
    ...
    for index in range(0, len(list(serial_dict.keys())), MAX_SR_NO):
        ...
        for _ in range(RETRY_ATTEMPTS):
            ...
            # If the pagination details are present
            # Break out of the range() loop if ErrorResponse is not present in the API response
            if "PaginationResponseRecord" in response and response["PaginationResponseRecord"] is not None:
                # Get the total number of pages to create API calls for all pages
        ...
        # Ending for loop as iterable exhausted
            ...
            verify_cisco_support_api_data(api="eox", iterable=response, verbose=True)
        ...
        for page in range(1, num_pages + 1):
            ...
            for _ in range(RETRY_ATTEMPTS):
                ...
                # Break out of the range() loop if ErrorResponse is not present in the API response
                if (
                    "PaginationResponseRecord" in response
                    and response["PaginationResponseRecord"] is not None
                ):
                    # Update the owner_coverage_status dict
...
def get_ss_suggested_release_by_pid(serial_dict: dict, api_creds: tuple, pid_list: list = False) -> dict:
    ...
    for index in range(0, len(list(pid_list)), MAX_PID):
        ...
        # Ending for loop as iterable exhausted
            ...
            verify_cisco_support_api_data(api="ss", iterable=response, verbose=True)
        ...
        for page in range(1, int(num_pages) + 1):
            ...
            for _ in range(RETRY_ATTEMPTS):
                ...
                if (
                    ...
                    # Check if the initial list and the response list have the same length. This verifies
                    # that there is a response for each pid of the initial list
