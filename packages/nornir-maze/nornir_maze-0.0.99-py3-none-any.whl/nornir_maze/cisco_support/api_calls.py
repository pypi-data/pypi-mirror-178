...
def _get_total_num_pages(**kwargs) -> int:
    ...
    for _ in range(kwargs["RETRY_ATTEMPTS"]):
        ...
        try:
            if api.lower() in "sni":
                response = api_obj.getCoverageSummaryBySerialNumbers(sr_no=chunk, page_index=1)
            elif api.lower() in "eox":
                response = api_obj.getBySerialNumbers(serialNumber=chunk, pageIndex=1)
            elif api.lower() in "ss":
                response = api_obj.getSuggestedReleasesByProductIDs(productIds=chunk, pageIndex=1)
        ...
        if api.lower() in "sni":
            key_to_verify = "pagination_response_record"
            key_to_get = "last_index"
        elif api.lower() in "eox":
            key_to_verify = "PaginationResponseRecord"
            key_to_get = "LastIndex"
        elif api.lower() in "ss":
            key_to_verify = "paginationResponseRecord"
            key_to_get = "lastIndex"

        if key_to_verify in response and response[key_to_verify] is not None:
            # Return the total number of pages to create API calls for all pages
            return response[key_to_verify][key_to_get]
    ...
    return verify_cisco_support_api_data(api=api.lower(), iterable=response, verbose=True)
...
def _get_data_all_pages(**kwargs) -> list[dict]:
    ...
    api_obj = kwargs["api_obj"]
    api = kwargs["api_string"]
    api_sub = kwargs["api_string_sub"] if kwargs["api_string_sub"] else api
    chunk = kwargs["chunk"]
    sleep = kwargs["SLEEP"]

    # Get the API data for each page of the chunk list
    # Call the Cisco support API for each page of the chunk list
    for page in range(1, kwargs["num_pages"] + 1):
        # Re-try the Cisco support API call with a backoff again in case of an error
        for _ in range(kwargs["RETRY_ATTEMPTS"]):
            # Call the Cisco support API for the chunk list
            try:
                if api_sub.lower() in "sni_owner":
                    response = api_obj.getOwnerCoverageStatusBySerialNumbers(sr_no=chunk)
                if api_sub.lower() in "sni_summary":
                    response = api_obj.getCoverageSummaryBySerialNumbers(sr_no=chunk, page_index=page)
                if api_sub.lower() in "eox":
                    response = api_obj.getBySerialNumbers(serialNumber=chunk, pageIndex=page)
                if api_sub.lower() in "ss":
                    response = api_obj.getSuggestedReleasesByProductIDs(productIds=chunk, pageIndex=page)
            except requests.exceptions.JSONDecodeError:
                continue
            ...
            # Break out of the range() loop if ErrorResponse is not present in the API response
            if api_sub.lower() in "sni_owner":
                key_to_verify = "serial_numbers"
                key_to_get = "serial_numbers"
            elif api_sub.lower() in "sni_summary":
                key_to_verify = "pagination_response_record"
                key_to_get = "serial_numbers"
            elif api_sub.lower() in "eox":
                key_to_verify = "PaginationResponseRecord"
                key_to_get = "EOXRecord"
            elif api_sub.lower() in "ss":
                key_to_verify = "paginationResponseRecord"
                key_to_get = "productList"

            if key_to_verify in response and response[key_to_verify] is not None:
                # Update the api_data list
                for item in response[key_to_get]:
                ...
                if api_sub.lower() in "ss":
                    ...
                    if len(chunk) != len(chunk_response):
                        verify_cisco_support_api_data(api=api.lower(), iterable=response, verbose=True)
...
def verify_cisco_support_api_data(
    api: Literal["sni", "eox", "ss"], iterable: dict, verbose: bool = False
) -> bool:
    """
    ...
    for value in iterate_all(iterable=iterable, returned="value"):
        if value is None:
            continue
        if [error for error in (SNI_ERRORS + EOX_ERRORS + SS_ERRORS) if error in value]:
            no_error = False
...
def get_sni_owner_coverage_by_serial_number(serial_dict: dict, api_creds: tuple) -> dict:
    ...
    for index in range(0, len(list(serial_dict.keys())), MAX_SR_NO):
        ...
        # Get the API data for each page of the chunk list
        api_data.extend(_get_data_all_pages(**sni_vars))

    # Add all records to the serial_dict dictionary
    for record in api_data:
        sr_no = record["sr_no"]
        serial_dict[sr_no]["SNIgetOwnerCoverageStatusBySerialNumbers"] = record
...
def get_eox_by_serial_numbers(serial_dict: dict, api_creds: tuple) -> dict:
    ...
    for index in range(0, len(list(serial_dict.keys())), MAX_SR_NO):
        ...
        # Part 2: Get the API data for each page of the chunk list
        api_data.extend(_get_data_all_pages(**eox_vars))

    for record in api_data:
        # The response value of "EOXInputValue" can be a single serial number or a comma separated string
        # of serial numbers as the API response can collect multiple same EoX response together
...
def get_ss_suggested_release_by_pid(serial_dict: dict, api_creds: tuple, pid_list: list = False) -> dict:
    ...
    for item in iterate_all(iterable=serial_dict, returned="key-value"):
        if "orderable_pid" in item[0]:
            pid_list.append(item[1])
        elif "base_pid" in item[0]:
            pid_list.append(item[1])
