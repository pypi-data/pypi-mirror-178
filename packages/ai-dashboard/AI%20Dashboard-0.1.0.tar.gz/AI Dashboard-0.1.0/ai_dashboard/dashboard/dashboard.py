...
class Dashboard:
    ...
    def _init_tabs(self):
        ...
        for deployable_id, config in self.config["tabs"].items():
            if config["type"] == "REPORT":
                tab = data_report.DataReport(self._endpoints, self._dataset_id, config)
    ...
    def DataReport(
        ...
        return data_report.DataReport(
            self._endpoints, self._dataset_id, title, name, colour, config
        )

    def CategoryView(self, config: Optional[Dict[str, Any]] = None):
        return category_view.CategoryView(config)

    def ChartView(self, config: Optional[Dict[str, Any]] = None):
        return chart_view.ChartView(config)

    def DocumentView(self, config: Optional[Dict[str, Any]] = None):
        return document_view.DocumentView(config)
