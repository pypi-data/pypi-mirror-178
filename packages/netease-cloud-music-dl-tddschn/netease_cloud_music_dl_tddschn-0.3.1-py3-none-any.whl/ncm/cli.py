...
def get_parse_id(song_id: str) -> str:
    # Parse the url
    if song_id.startswith('http'):
        try:
            ...
            path = urlparse(song_id).path
            if m := re.search(r'/song/(?P<song_id>\d+)/?', path):
                return m.group('song_id')
            else:
                print(f'Cannot parse the song_id / url: {song_id}')
                sys.exit(1)
    return song_id
