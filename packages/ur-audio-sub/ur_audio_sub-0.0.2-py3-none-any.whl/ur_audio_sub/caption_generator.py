...
def ytSub(link, model='medium', language='', translate=False, lmt=LMT, basepath=ROOT_DIR):
  ...
  if language == '':
    if translate == False:
      os.system(f'whisper "{file_path}" --model {model}')
    else:
      os.system(f'whisper "{file_path}" --model {model} --task translate')
  else:
    if translate == False:
      os.system(f'whisper "{file_path}" --model {model} --language {language}')
    else:
      os.system(f'whisper "{file_path}" --model {model} --language {language} --task translate')
