...
class MultistartNlp(Nlp):
    ...
    def _symbols(
        ...
        if vars:
            S.update(self._vars
                     if i is None else
                     {k: self._multi_nlp._vars[_n(k, i)] for k in self._vars})
        if pars:
            S.update(self._pars
                     if i is None else
                     {k: self._multi_nlp._pars[_n(k, i)] for k in self._pars})
        if dual:
            S.update(self._dual_vars
                     if i is None else
                     {k: self._multi_nlp._dual_vars[_n(k, i)]
    ...
    def constraint(
        ...
        for i in range(self._starts):
            ...
            self._multi_nlp.constraint(
                _n(name, i), expr, op, 0, soft, simplify=False)
        return out
    ...
    def minimize(self, objective: Union[cs.SX, cs.MX]) -> None:
        ...
        self._fs: Union[List[cs.SX], List[cs.MX]] = [
            subsevalf(
                objective,
                symbols,
                self._symbols(i, vars=True, pars=True), eval=False
            ) for i in range(self._starts)
        ]
    ...
    def solve_multi(
        self,
        pars: Union[
            None, Iterable[DMStruct], Iterable[Dict[str, np.ndarray]]] = None,
        vals0: Union[
            None, Iterable[DMStruct], Iterable[Dict[str, np.ndarray]]] = None,
