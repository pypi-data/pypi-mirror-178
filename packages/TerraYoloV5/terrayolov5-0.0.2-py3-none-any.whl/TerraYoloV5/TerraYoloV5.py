...
class TerraYoloV5:
    ...
    def do_experiment(self, command, param_dict, project_dir):
        ...
        if os.path.isdir(project_dir) == True:
            ...
            if 'exist-ok' in param_dict.keys():
                ...
                print("last_train_subdir = param_dict.get('name', 'exp')")
            else:
                last_train_subdir = sorted(list(set(os.listdir(project_dir)) -
                                         exp_dir_set))
                last_train_subdir = last_train_subdir[-1] if len(
                                                last_train_subdir)>0 else ''
        else:
            project_dir = ''
    ...
    def run(self, param_dict, exp_type='val'):
        ...
        # print(exp_dict)
        exp_dict['param_dict'] = param_dict
