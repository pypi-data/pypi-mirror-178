...
class ApiTest(APILiveServerTestCase):
	...
	def __make_api_request(self, method='post', endpoint: str = '', data: dict = None, _func=None, **opt) -> ApiTestResult:
		...
		try:
			...
			request_url = self.config.make_url(api_version=self._api_version, endpoint=endpoint)
	...
	def _authenticate(self, auth_credentials: dict = None) -> None:
	...
	def api_get(self, endpoint: str = '', payload: dict = None, **opt) -> ApiTestResult:
		return self.__make_api_request('get', endpoint, payload, _func=self.client.get, **opt)

	def api_post(self, endpoint: str = '', payload: dict = None, **opt) -> ApiTestResult:
		return self.__make_api_request('post', endpoint, payload, _func=self.client.post, **opt)

	def api_put(self, endpoint: str = '', payload: dict = None, **opt) -> ApiTestResult:
