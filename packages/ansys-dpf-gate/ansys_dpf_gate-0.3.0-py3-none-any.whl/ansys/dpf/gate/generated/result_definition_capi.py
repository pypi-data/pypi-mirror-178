...
class ResultDefinitionCAPI(result_definition_abstract_api.ResultDefinitionAbstractAPI):
	...
	def result_definition_set_mesh_scoping(resDef, scoping):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.ResultDefinition_SetMeshScoping(resDef, scoping._internal_obj, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def result_definition_set_cyclic_sectors_scoping(resDef, scoping, stageNum):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.ResultDefinition_SetCyclicSectorsScoping(resDef, scoping._internal_obj, utils.to_int32(stageNum), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
