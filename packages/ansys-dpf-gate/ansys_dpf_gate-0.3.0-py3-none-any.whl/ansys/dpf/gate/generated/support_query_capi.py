...
class SupportQueryCAPI(support_query_abstract_api.SupportQueryAbstractAPI):
	...
	def support_query_rescoping_by_property(supportQuery, scoping, prop_name, prop_number):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.SupportQuery_RescopingByProperty(supportQuery, scoping._internal_obj, utils.to_char_ptr(prop_name), utils.to_int32(prop_number), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def support_query_transpose_scoping(supportQuery, scoping, bInclusive):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.SupportQuery_TransposeScoping(supportQuery, scoping._internal_obj, bInclusive, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def support_query_topology_by_scoping(supportQuery, scoping, topologyRequest):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.SupportQuery_TopologyByScoping(supportQuery, scoping._internal_obj, utils.to_char_ptr(topologyRequest), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def support_query_data_by_scoping(supportQuery, scoping, domainsDataRequest):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.SupportQuery_DataByScoping(supportQuery, scoping._internal_obj, utils.to_char_ptr(domainsDataRequest), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
