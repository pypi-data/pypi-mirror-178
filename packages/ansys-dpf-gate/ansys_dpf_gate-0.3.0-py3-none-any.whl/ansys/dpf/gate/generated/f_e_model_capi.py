...
class FEModelCAPI(f_e_model_abstract_api.FEModelAbstractAPI):
	...
	def femodel_add_result_with_scoping(feModel, res, scoping):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.FEModel_AddResultWithScoping(feModel, utils.to_char_ptr(res), scoping._internal_obj, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
