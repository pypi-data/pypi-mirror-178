...
class DataBaseCAPI(data_base_abstract_api.DataBaseAbstractAPI):
	...
	def data_base_record_entity_by_db_id(dbid, obj):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.DataBase_recordEntityByDbId(utils.to_char_ptr(dbid), obj._internal_obj, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def data_base_record_entity(db, obj):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.DataBase_recordEntity(db, obj._internal_obj, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
