...
class SessionCAPI(session_abstract_api.SessionAbstractAPI):
	...
	def add_external_event_handler(session, event_handler, cb):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.addExternalEventHandler(session._internal_obj, utils.to_void_ptr(event_handler), cb, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
