...
class DataProcessingCAPI(data_processing_abstract_api.DataProcessingAbstractAPI):
	...
	def data_processing_release_on_client(client, context):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.DataProcessing_release_on_client(client._internal_obj, utils.to_int32(context), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def data_processing_initialize_with_context_on_client(client, context, dataProcessingCore_xml_path):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.dataProcessing_initializeWithContext_on_client(client._internal_obj, utils.to_int32(context), utils.to_char_ptr(dataProcessingCore_xml_path), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
	...
	def data_processing_apply_context_on_client(client, context, dataProcessingCore_xml_path):
		errorSize = ctypes.c_int(0)
		sError = ctypes.c_wchar_p()
		res = capi.dll.dataProcessing_applyContext_on_client(client._internal_obj, utils.to_int32(context), utils.to_char_ptr(dataProcessingCore_xml_path), ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError))
		if errorSize.value != 0:
			raise errors.DPFServerException(sError.value)
		return res
