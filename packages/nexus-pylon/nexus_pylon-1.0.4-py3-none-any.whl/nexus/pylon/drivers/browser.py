...
def wait_for_window_handle(index):
    def wait(driver):
            ...
            for action in self.actions:
                match action['type']:
                    ...
                    case 'switch_to_window':
                        new_window = WebDriverWait(chrome, action.get('timeout', 15.0)).until(wait_for_window_handle(action['index']))
                        previous_window = current_window
                    ...
                    case 'wait_css_selector':
                        last_element = WebDriverWait(chrome, action.get('timeout', 15.0)).until(
                            wait_functions[action.get('wait_function', 'presence')]((
                                By.CSS_SELECTOR,
                                action['selector'],
                            ))
                        )
                    case 'wait_id':
                        last_element = WebDriverWait(chrome, action.get('timeout', 120.0)).until(
                            wait_functions[action.get('wait_function', 'presence')]((
                                By.ID,
                                action['id'],
                            ))
