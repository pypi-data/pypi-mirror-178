...
def rate_filter(time, rate, drate, level_down=100, level_up = 0, quantile=0.8, make_plot=True, 
                far=1e-4):
    help = '''
sas_cr_filter(self, typical_level=0.5, quantile=0.7,  make_plot=True, far=1e-3,
                    tmin=0,tmax=1e12):
...
- a Gaussian is fitted on LC data within the lower quantile (default=0.8) of the light curve
- a limit is set on FAR, but it can be overrided by using the typical_level parameter, which is the lowest possible limit.
    ...
    ind = rate <= np.quantile(rate, quantile)
    ...
    limit_up = np.max([level_up, tmp ])
    limit_down = np.min([level_down, 2*mu-tmp])
