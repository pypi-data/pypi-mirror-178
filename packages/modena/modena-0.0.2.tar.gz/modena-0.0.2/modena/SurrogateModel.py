...
class SurrogateModel(DynamicDocument):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if '_cls' in kwargs:
            ...
            if not 'surrogateFunction' in kwargs:
    ...
    def loadFromModule(cls):
        ...
        doc = collection.find_one({ '_cls': { '$exists': True}})
        ...
        try:
            ...
            return next(iter(m for m in modena.SurrogateModel.BackwardMappingModel.get_instances() if m._id == modName))
        except ImportError:
