...
class Waveform(object):
    ...
    def __init__(
        ...
        edges : list
            The edges (adjecencies) of the flowsheet.
        var_map : list 
    ...
    def compute(
            ...
            min_frame_length=0, max_frame_length=0, max_iter_frame=7, 
            points_per_frame=1001, max_iter=5000, iter_threshold=1, 
            alpha=1.5, beta=0.2,epsilon=0.0001, step_type="gauss-seidel",
            verbose = False):
        """
        ...
        if self.frame_length > t_end - t_start or self.frame_length <= 0:
            self.frame_length = 0.05 * (t_end - t_start)
        # Set minimum length of frame
        if self.min_frame_length <= 0:
            self.min_frame_length = 0.05 * self.frame_length
        # Set maximum length of frame
        if self.max_frame_length <= 0:
            self.max_frame_length = 5.0 * self.frame_length
