...
def circleScar(mesh, index, path, list_face_of_vertice, limitY_min, limitY_max, min_num_point_ratio=50, max_num_point_ratio=10, range_random_inc_Z=(20,30), range_random_llZ=(0,1)):
    mesh_pre = mesh.copy()
    ...
    min_num_point = int(len(list_index_face)/min_num_point_ratio)
    max_num_point = int(len(list_index_face)/max_num_point_ratio)
    ...
    _inc_Z = np.random.randint(range_random_inc_Z[0], range_random_inc_Z[1])
    llZ = np.random.randint(range_random_llZ[0],range_random_llZ[1])
    sm = len(_tmp_ver_pkd)
    if llZ == 0:
        _inc_Z = -_inc_Z
    
    colors = [ [100,100,100,100] for i in range(len(mesh_pre.vertices))]

    for i in range(sm):
        for idx_ver in _tmp_ver_pkd[i]:
            mesh_pre.vertices[idx_ver][2] =  mesh_pre.vertices[idx_ver][2] + _inc_Z*(sm - i)/sm
            colors[idx_ver] = [100,200,10,100]

    mesh_pre = trimesh.Trimesh(vertices = mesh_pre.vertices, faces = mesh_pre.faces, vertex_colors = colors)
...
def longScar(mesh, index, path, list_face_of_vertice, limitY_min, limitY_max, min_num_point_ratio=200, max_num_point_ratio=20, range_random_inc_Z=(20,30), range_num_ver_c=(10,30), range_random_llZ=(0,1)):
    mesh_pre = mesh.copy()
    ...
    while(len(lst_vertices_picked) < num_vertices_pick):
        ...
        num_ver_c = np.random.randint(range_num_ver_c[0],range_num_ver_c[1])
        bg_get = num_ver_c
...
def scarCreatingV2(
    mesh, path, numGenScar=10, mesh_return=True,
    circle_scar_min_num_point_ratio=50, circle_scar_max_num_point_ratio=10, circle_scar_range_random_inc_Z=(20,30), circle_scar_range_random_llZ=(0,1),
    long_scar_min_num_point_ratio=200, long_scar_max_num_point_ratio=20, long_scar_range_random_inc_Z=(20,30), long_scar_range_num_ver_c=(10,30), long_scar_range_random_llZ=(0,1)
    ):
    """
    ...
    circleScarMeshList = [
        circleScar(mesh, index, path, list_face_of_vertice, limitY_min, limitY_max,
            circle_scar_min_num_point_ratio, circle_scar_max_num_point_ratio, circle_scar_range_random_inc_Z, circle_scar_range_random_llZ
        )
    ...
    longScarMeshList = [
        longScar(mesh, index, path, list_face_of_vertice, limitY_min, limitY_max,
            long_scar_min_num_point_ratio, long_scar_max_num_point_ratio, long_scar_range_random_inc_Z, long_scar_range_num_ver_c, long_scar_range_random_llZ
        )
