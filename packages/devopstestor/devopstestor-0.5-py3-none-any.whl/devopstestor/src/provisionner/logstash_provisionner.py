...
class LogstashProvisionner(AbstractProvisionner):
    ...
    def initialise(self, machine_controller, machine_name):
        ...
        # self.instance_base_path = self.global_config.get('source_manager::sources::logstash_configs::path::machine')


    ########### Lancement de logstash ###############
    def test_config(self, path_settings, **others):
        """
        Lancement oneshot
        """
        ...
        arguments= [
            ...
            "--path.settings {}".format(path_settings)
        ]
    ...
    def logstash_service_control(self, action):
        # Lancement Logstash
        ret, out = self.machine_controller.run_cmd(
            "systemctl {} logstash".format(action)
        )
        # Interpretation resultats
        result = True if ret == 0  else False
        return result, out
