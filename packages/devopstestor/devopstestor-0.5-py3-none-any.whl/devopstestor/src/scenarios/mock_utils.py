...
"""
from utils import grep, copy_merge_recursive_dict
import yaml
import json
...
def replace_in_file(machine_provisionner, init_contexte, file_path, search, replace):
    mc = machine_provisionner.get_machine_controller()
    ret, file_content = mc.get_file_content(file_path=file_path)
    if str(ret) != '0':
...
def delete_path(machine_provisionner, init_contexte, path):
    mc = machine_provisionner.get_machine_controller()
    ret, res = mc.run_cmd('rm -rf "{}"'.format(path))
