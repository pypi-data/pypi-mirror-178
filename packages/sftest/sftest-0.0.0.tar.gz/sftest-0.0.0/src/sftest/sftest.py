...
class sftest:
    ...
    def __init__(self, p_project_name, p_dir, p_parent_log_name='', p_verbose=True):
        ...
        if p_parent_log_name:
            self.log_name = '{}.{}'.format(p_parent_log_name, sftest)
...
def do_examples(p_cls=True):
    '''A collection of implementation examples for sftest.

    A collection of implementation examples for sftest. The examples
...
def do_example1(p_cls=True):
    '''A working example of the implementation of sftest.
    ...
    t_sftest = sftest("sftest", t_dir)
    t_sftest.method_1('This is do_example1')
...
def do_example2(p_cls=True):
    '''Another working example of the implementation of sftest.
    ...
    t_dir = beeutils.get_tmp_dir()
    t_sftest = sftest("sftest", t_dir)
    t_sftest.method_1('This is do_example2')
    beeutils.rm_tree(t_dir)
    archiver.print_footer()
    return success
