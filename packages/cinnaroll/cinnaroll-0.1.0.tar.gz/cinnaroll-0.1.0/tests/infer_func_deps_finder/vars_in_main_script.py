...
class Config(RolloutConfig):
    ...
    def infer(model_object: Any, input_data: str) -> str:
        ...
        return x + y + z  # everything from outer scope
...
def infer(model_object: Any, input_data: str) -> str:
    ...
    return user_globals['x'] + user_globals['y'] + user_globals['z']
"""
...
"""
    expect = "\n".join(
        [required_imports, infer_code, loading_global_vars_dict, functions_and_classes]
    )
