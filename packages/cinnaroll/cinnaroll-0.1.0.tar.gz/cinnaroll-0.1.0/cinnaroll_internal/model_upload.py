...
from cinnaroll_internal import environment_info
from cinnaroll_internal import rollout_config
from cinnaroll_internal.infer_func_deps_finder import infer_func_file
...
def prepare_artifact(
    ...
    def _process_requirements(r: List[str]) -> str:
        """In requirements replace tensorflow-macos by tensorflow. Return a string which can be written to a file."""
        for idx, x in enumerate(r):
            if "tensorflow-macos" in x:
                r[idx] = x.replace("tensorflow-macos", "tensorflow")
    ...
    files = {
        ...
        infer_func_file.GLOBAL_VARS_DICT_FILENAME: (
...
def send_rollout_data(
    ...
    res = requests.post(url, headers=headers, data=rollout_data)
    return res
...
def notify_server(upload_id: str, status: str) -> requests.Response:
    ...
    res = requests.put(url, data={"status": status})
    return res
