...
from cinnaroll_internal.infer_func_deps_finder import types, ast_utils
...
class UsedGlobalVariables:
    ...
    def add_global_vars(
        ...
        for child_node in asttokens.util.walk(node):
    ...
    def add_node_if_is_unshadowed_name(
            self, node, shadowed_names_in_current_scope: Set[str]  # type: ignore
    ) -> None:
