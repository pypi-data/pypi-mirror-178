...
def get_cache_dir_path(working_environment: WorkingEnvironment, notebook: Optional[Notebook]) -> Path:
    ...
    if working_environment is working_environment.NOTEBOOK:
        return Path(notebook.path).parent / CACHE_DIR_NAME
...
def rollout(config: rollout_config.RolloutConfig) -> None:
    ...
    working_environment, notebook = environment_info.get_working_environment_and_notebook_if_appropriate()
    cache_dir_path = get_cache_dir_path(working_environment, notebook)
    ...
    deps_finder = build_deps_finder(config.infer, notebook)
