...
GLOBAL_VARS_DICT_NAME = "user_globals"
GLOBAL_VARS_DICT_FILENAME = "user_defined_globals"
...
user_globals_file = open("{GLOBAL_VARS_DICT_FILENAME}.pickle", "rb")
{GLOBAL_VARS_DICT_NAME} = pickle.load(user_globals_file)
...
def build_infer_py_file_content(
    infer_code: str,
    non_local_import_statements: str,
    used_global_variables_locations: Optional[List[List[VariableWithPositionInLine]]],
) -> str:
    if used_global_variables_locations:
        non_local_import_statements = "\n".join(
            [non_local_import_statements, "import pickle"]
        )
    ...
    return (
        non_local_import_statements
        + "\n\n"
        + (LOAD_PICKLED_GLOBALS_STATEMENT if used_global_variables_locations else "")
        + code
    )
...
def replace_vars_with_globals_dict_usage(
    ...
    added_chars_count = len(f"{GLOBAL_VARS_DICT_NAME}['']")
    ...
    for line_number in range(lines_count):
        ...
        for i in range(vars_count):
            ...
            offset = added_chars_count + var.name_length - 1

            code_lines[line_number] = (
                code_lines[line_number][: var.start_column]
                + f"{GLOBAL_VARS_DICT_NAME}['{var.name}']"
