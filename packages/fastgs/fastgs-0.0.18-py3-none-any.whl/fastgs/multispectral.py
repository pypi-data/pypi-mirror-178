...
@patch
def create_unet_learner(self: MSPipeline,model,pretrained=True,loss_func=CrossEntropyLossFlat(axis=1),metrics=Dice(axis=1)):
    return unet_learner(
        self.create_data_block(),model,normalize=False,n_in=len(self.ms_data.bands.ids),n_out=len(self.mask_data.mask_codes),
