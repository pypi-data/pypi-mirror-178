...
class PowerschoolPy:
    ...
    def __init__(self, base_url, api_username="pearson", api_password="m0bApP5"):
        ...
        try:
            ...
            raise PowerschoolPyError(f"Could not connect to {base_url}.")
        except requests.exceptions.HTTPError:
            raise PowerschoolPyError(f"Incorrect api credentials {api_username}, {api_password}")


    def getStudent(self, username, password, studentIDToFind = None):
        studentCounter = 0
        ...
        if result["userId"] == None:
            raise PowerschoolPyError("Could not log in to ({}, {})".format(username, password))
        ...
        for studentID in result["studentIDs"]:
            if studentIDToFind == None:
                student = Student.from_dict(service.getStudentData(userSessionVO, result["studentIDs"][0], {"includes": "1"})["studentDataVOs"][0])
                # sections = result["studentIDs"][0]["sections"]
                # for course in sections:
                #     student.courses.append(Course.from_dict(course))
            elif studentID == studentIDToFind:
                student = service.getStudentData(userSessionVO, result["studentIDs"][studentCounter], {"includes": "1"})["studentDataVOs"][0]
    ...
    def getStudents(self, username, password):
        students = []
        studentCounter = 0
        ...
        for studentID in result["studentIDs"]:
            ...
            student = Student.from_dict(service.getStudentData(userSessionVO, result["studentIDs"][studentCounter], {"includes": "1"})["studentDataVOs"][0])
