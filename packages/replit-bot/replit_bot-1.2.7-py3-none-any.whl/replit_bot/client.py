...
class Repl:
    ...
    def likes(self) -> List[str]:
        def fetch_once(after=None):
            if (after is not None): vars = {"after": after, "url": self.url}
            else: vars = {"url": self.url}
            res = post(self.c.sid, "usersWhoLikedRepl", vars=vars)
            if "repl" not in res or not res["repl"]: return None
            current = list(map(lambda x: x["votes"], res["repl"]["posts"]["items"]))
            output = []
            for i in current:
                for j in i["items"]:
                    output.append({"voters": list(map(lambda x: x["user"]["username"], i["items"])), "cursor": i["pageInfo"]["nextCursor"]})
            output.reverse()
            return output
        res = fetch_once()
