...
from .ipv4.nat import NattedPort  # for type hinting
...
class FrameBlastingFlow(Flow):
    ...
    def apply(self, duration: Optional[timedelta] = None, **kwargs) -> None:
        # if initial_time_to_wait is set, subtract this wait time from the scenario duration
        ...
        if self.number_of_frames == INFINITE_NUMBER_OF_FRAMES:
            self._number_of_frames = int(duration.total_seconds() * self.frame_rate)
