...
from typing import Any, DefaultDict, Dict, NoReturn, Optional, Union, cast
...
class RedisHealthChecker:
    def __init__(
        self, redis_conn: Redis[Union[str, bytes]], monitoring_server: MonitoringServer
    ):
    ...
    def check_write_read_redis_service(self) -> None:
        ...
        if result:
            try:
                redis_value_b = self.redis_conn.get(self.redis_health_check_key)
                if redis_value_b is not None:
                    if isinstance(redis_value_b, bytes):
                        redis_value = redis_value_b.decode("utf-8")
                    else:
                        redis_value = redis_value_b
            except ConnectionError as exc:
                self.on_unhealthy(
                    health_check_name=health_check_name,
