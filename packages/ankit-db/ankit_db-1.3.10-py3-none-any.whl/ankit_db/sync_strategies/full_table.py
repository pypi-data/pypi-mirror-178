...
def sync(config, state, stream):
    ...
    for result in scan_table(table_name, projection, expression, last_evaluated_key, config):
        LOGGER.info("here is the result from full table sync : %s\n last evaluated key : %s ",result,last_evaluated_key)
        for item in result.get('Items', []):
            ...
            LOGGER.info("Record Message : %s",record_message)
    ...
    singer.write_state(state)
    LOGGER.info("state after full table sync",state)
