...
class TransitionApiView(APIView):
    ...
    def post(self,request):
        ...
        if type and t_id  is not None:
            transitions = FinFlotransition(action = action.upper() , type = type.capitalize() , t_id = t_id , source = source ,
...
class ActionListApi(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if (action and model) is not None:
            queryset = Action.objects.filter(description__icontains = action, model__description__icontains = model)
        elif action :
            queryset = Action.objects.filter(description__icontains = action)
        else:
            queryset = Action.objects.all()
        return queryset
