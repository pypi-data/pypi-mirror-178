...
class FinFlotransition():
    ...
    def return_transition(self):
        ...
        if self.action == self.gets_default_return_action().description.upper():
            obj , created  = workflowitems.objects.update_or_create( transitionmanager=overall_model[0] or overall_model[0].id, defaults= {"initial_state" : overall_model[3].initial_state, "interim_state" : overall_model[1].to_state.description ,
                    "final_state" : overall_model[1].to_state.description, "action" : self.action, "subaction" : self.action , "next_available_transitions" : None, "model_type" : self.type.upper(), "event_user" : get_current_user() , "current_from_party" : overall_model[1].from_party if overall_model[1].from_party else self.from_party , "current_to_party" : overall_model[1].to_party if overall_model[1].to_party else self.to_party})
