...
# This part is designed by @UnknownUser1837: 
# LIB IMPORTS
...
def toggle():
     global toggle
     toggle.source = "{appdir}/{main:base_lib}.scripts/toggle"
     toggle.possiblemodes = "isolated, live, main"
     toggle.chosenmodes.src = "main: {appdir}/{main:program:exe}; live: {appdir}/{live:program:any}; isolated: {appdir:isolated}/env"
     toggle.mainsrcode = "{appdir}/{mainsrcode:function:toggle}"
     toggle.chosenmodesdef = "main"
...
def activate_pycor():
  ...
  defer = 0
activate_pycor()
# While the rest is made by  @UnknownUser2222
# BEFORE START
...
# SOURCE CODE
print("Welcome to PyCor 1.18.0.1009 Alpha")
...
while x != "0":
    ...
    if x == str("start_debug"):
        print("Restarting PyCor...")
        print("pycor info: OCCW is loading...")
        print("pycor info: Running pycor.py...")
        print("pycor info: Building OEM scripts...")
        print("pycor info: Loading CLI tool...")
        ...
        print("pycor info: Daemon is loading...")
        print("pycor info: Running program...")
        print("pycor warn: WSL is not installed, Linux features will not work without it.")
        print("pycor info: Downloading WSL | [00:00:01] [0 KB/s] [2 KB / 127MB] | (Download failed, Stopped)")
        print("pycor err!: Download failed for WSL, Process finished with exit code 1")
        print("pycor err!: 2 applications are not found. Search queries will not work!")
        print("pycor warn: Inside input. Typing 0 or 1 may exit the program by keyboard interrupts")
        print("--------------------------------------------------------------------------------------------")
        print("github info: Connected to UnknownUser2222/pycor, success")
        print("github info: Connected to Dependabot, success")
        print("github err!: Failed to load metadata for GitHub Actions!")
        print("github warn: Some package versions are maybe invalid!")
        print("--------------------------------------------------------------------------------------------")
        print("npm info: Load package 'pycor'")
        print("npm info: 6 dependencies, no devDependencies!")
        print("npm info: Load package 'fastify'")
        print("npm info: Load file package.json")
        print("npm info: Load package null, Exiting...")
        print("--------------------------------------------------------------------------------------------")
        print("Output folder: reavsecurenableics")
        print("Extract: tpm1module.py")
        print("Extract: tpm1_2module.py")
        print("Extract: tpm2module.py")
        print("Extract: extraSearchvolume.guid")
        print("Extract: hookconfig.pyc")
        print("Extract: ipctr.pyc")
        print("Extract: pathcond.dll")
        print("Extract: isomodules.pyc")
        print("Extract: cppconfig")
        print("Extract: sst.exe")
        print("10 / 10 files extracted, Time elapsed: 00:00:01")
        ...
        input("Snakesite: snake://")
        print("Error while loading: 403 Forbidden!")
    elif x == str("other_vbc"):
    ...
    "pycor_readinfo": {
       "epc": [{
        ...
        "endColumn": 2
                     ...
                     """)
    elif x == str("deployment"):
        # Created on the launch of Artemis I
        SHADOWROOT = 1
        ALWAYSDEACTIVATE = 1
        RUNFILE = "deploymentenv"
        
        ISOLATEDENVSRC = "./env/isolatedenv"
        #Toggle mode
        IFTOGGLE = "isolated"
        Flags = "toggle(isolated)._instances_1"
        # NOTE: UNDER HERE IS A ISOLATED ENVIRONMENT! IF FLAGS CONTAIN ISOLATED, THEN THE MODE IS SET.
        ...
        print("deployment - Deployment program.")
        print("switch_mode - Switches the mode you are using (PCR files included)")
        print("start_debug - Starts a startup debug of PyCor")
    elif x == str("switch_mode"):
      print("Press any key to exit")
      print("Chosen modes:")
      print("[CHOSEN] PyCor Program Environment")
      print("Available modes:")
      print(" Programs:")
      print("  [1] PyCor Bash")
      print("  [2] PyCor Interactive Shell")
      print("  [4] Localhost Environment")
      print(" PCR files:")
      print("  [3] PyCorMgr.pcr")
      modechosen = input("Option: ")
      if modechosen == "1":
        print("Switching to PyCor Bash...")
        print("PyCor Bash (0.01.0.1000)")
        print("Type help bash for help.")
        while x != "0":
            x = input("""
Admin@PyCor MSWINGW6432
  root/~: """) 
            if x == "help bash":
                print("reavsecur /enable /pth - Enables the security feature in PyCor")
                print("reavsecur /disable - Disables the security feature in PyCor")
                print("reavsecur /info - Shows the status of the feature.")
                print("dir /contents - Shows the files and folders in the root folder.")
                print("/ppd ac - {Tarball not recognised: '/ppd ac' Not found in commands.json and commands.py, If you are the owner of the file, please configure it use server port 11} Code: ERR_NAME_NOT_RESOLVED, UNKNOWN_GLOBAL_VARIABLE. If the issue persists, please add a description or change the port.")
            if x == "dir /contents":
                print("""Contents of root:
app | Type: file folder | Size: null
1 DIRS, 0 FILES, SIZE: 24.2MB
                """)
            if x == "reavsecur /info":
                print("Status: Enabled by default, Safe mode disabled")
            if x == "reavsecur /disable":
                print("Disable complete")
            if x == "reavsecur /enable /pth":
                print("Output folder: reavsecurenableics")
                print("Extract: tpm1module.py")
                print("Extract: tpm1_2module.py")
                print("Extract: tpm2module.py")
                ...
                print("Extract complete")
      if modechosen == "2":
        print("Switching to PyCor Interactive Shell...")
        print("Welcome to PyCor Interactive Shell")
        print("Type help_pycor for help")
      if modechosen == "3":
        print("Switching to PyCorMgr.pcr...")
        print("Please use the command PyCorMgr to switch.")
      if modechosen == "4":
        print("Switching to Localhost environment...")
        print("............................................")
