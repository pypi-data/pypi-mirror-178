...
class SQL_Helper:
    ...
    def write_row(self, row, table_name):
        ...
        cols = ", ".join([k for k, v in row.items() if v])
        values = ", ".join([str(v) if not isinstance(v, str) else f"'{v}'" for k, v in row.items() if v])
    ...
    def update_row(self, row, keys, table_name):
        ...
        update = ", ".join(
            [
                f"""{k} = {str(v) if not isinstance(v,str) else f"'{v}'"}"""
                for k, v in row.items()
                if v and (k not in keys)
            ]
        )
        condition = "AND ".join(
            [f"""{k} = {str(v) if not isinstance(v,str) else f"'{v}'"}""" for k, v in row.items() if v and (k in keys)]
        )
