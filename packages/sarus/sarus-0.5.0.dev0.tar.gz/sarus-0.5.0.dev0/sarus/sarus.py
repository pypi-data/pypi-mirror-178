...
class Dataset(DataSpecWrapper[t.Iterator[pa.RecordBatch]]):
    ...
    def _from_dict(cls, data: dict, client: Client) -> Dataset:
        ...
        Args:
            data (dict): JSON data returned by calling GET /datasets/<id>
            client (Client): client used to get information from the server

        Returns:
            Dataset
        """
        dataspec_id = data.get("id")
        serialized_dataspec = data.get("dataspec")
        ...
        dataspec = (
            client.context()
            .factory()
            .create(dict_deserialize(serialized_dataspec["dataset"]))
        )
        client.context().manager().set_remote(dataspec)

        if len(serialized_dataspec["synthetic_dataset"]) == 0:
            raise ValueError("Synthetic Dataspec not ready")
        _ = (
            client.context()
            .factory()
            .create(
                dict_deserialize(serialized_dataspec["synthetic_transform"])
            )
        )
        syn_dataset = (
            client.context()
            .factory()
            .create(dict_deserialize(serialized_dataspec["synthetic_dataset"]))
        )
        variant_constraint(
        ...
        _ = (
            client.context()
            .factory()
            .create(dict_deserialize(serialized_dataspec["schema"]))
        )
        _ = (
            client.context()
            .factory()
            .create(dict_deserialize(serialized_dataspec["synthetic_schema"]))
        )
        ...
        return cls(
            ...
            is_bigdata=data.get("is_bigdata"),
        )
