...
from low_code_assistant.assistant import drawer, mixpanel
...
@solara.component
def CrossFilterWidgetsPanel(edit_data: Optional[WidgetState], set_code, dfs, open, edit, on_close):
    ...
    with drawer.RightDrawer(
        ...
        with solara.Div():
            with solara.Div(class_="lca-transform-header", style_="margin-left: -24px; margin-right: -24px; margin-top: -4px; padding: 8px 80px;"):
                ...
                if state.widget:
                    if state.widget != "Report":
                        ...
                        if state.widget == "Slider":
                            ...
                            with solara.ToggleButtonsSingle(value=state.mode, on_value=lambda v: set_state(replace(state, mode=v))):
...
def _generate_select_widget(state: WidgetState):
    return textwrap.dedent(
        f"""\
        from low_code_assistant.widgets import CrossFilterSelect
...
def _generate_slider_widget(state: WidgetState):
    return textwrap.dedent(
        f"""\
        from low_code_assistant.widgets import CrossFilterSlider
...
def _generate_report_widget(state: WidgetState):
    return textwrap.dedent(
        f"""\
        from low_code_assistant.widgets import CrossFilterReport
