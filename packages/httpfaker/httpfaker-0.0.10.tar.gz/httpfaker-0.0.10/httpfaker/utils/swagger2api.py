...
class Swagger2Api(object):
    ...
    def __init__(self, host, url, type='.yml', api_path='apis', headers=None):
        interface = Interface(url, host, headers=headers)
...
class Interface(object):
    def __init__(self, url, host=None, **kwargs):
        ...
        itfs = session.get(interface_url, **kwargs).text
        try:
        ...
        self.definitions = itfs.get('definitions') if itfs.get('definitions') else itfs.get('components')
