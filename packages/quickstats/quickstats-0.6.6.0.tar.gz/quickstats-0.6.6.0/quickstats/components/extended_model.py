    ...
    def save(self, fname:str, recreate:bool=True, rebuild:bool=False):
        ...
        if rebuild:
            ...
            if (quickstats.root_version >= (6, 26, 0)):
                snapshots = [i.GetName() for i in self.workspace.getSnapshots()]
            else:
                ...
                snapshots = []
            modifier = XMLWSModifier({"data_name": None, "snapshot": snapshots}, verbosity="ERROR")
            modifier.create_modified_workspace(self.workspace, fname)
        else:
