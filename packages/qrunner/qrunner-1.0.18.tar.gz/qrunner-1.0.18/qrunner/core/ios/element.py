...
class IosElement(object):
    ...
    def _find_element(self, retry=3, timeout=3, alert_list=None):
        ...
        element = driver.d.xpath(self._xpath) if \
            self._xpath else driver.d(**self._kwargs)[self._index]
        ...
        while not element.wait(timeout=timeout):
            if cur_retry > 0:
                ...
                logger.warning(f'【{caller.function}:{caller.lineno}】未找到元素 {self._kwargs}')
                return None
        elements = driver.d.xpath(self._xpath).find_elements() if \
            self._xpath else driver.d(**self._kwargs)
        return elements
    ...
    def rect(self):
        ...
        return [item * self.get_driver().d.scale for item in list(self.get_element().bounds)]
