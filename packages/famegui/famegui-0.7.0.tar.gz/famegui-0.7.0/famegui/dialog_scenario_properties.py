...
from .time_utils import convert_gui_datetime_to_fame_time, convert_fame_time_to_gui_datetime
...
class DialogScenarioProperties(QDialog):
    ...
    def __init__(
        ...
        # simulation params
        self._ui.lineEditStartTime.setDateTime(convert_fame_time_to_gui_datetime(self._props.simulation_start_time))
        ...
        self._ui.lineEditStopTime.setDateTime(convert_fame_time_to_gui_datetime(self._props.simulation_stop_time))
    ...
    def _connect_slots(self):
        self._ui.lineEditStartTime.dateTimeChanged.connect(self._update_ok_button_status)
    ...
    def make_properties(self) -> models.GeneralProperties:
        ...
        self._props.set_simulation_start_time(convert_gui_datetime_to_fame_time(datetime_string_start_time))

        self._props.set_simulation_stop_time(convert_gui_datetime_to_fame_time(datetime_string_stop_time))
        self._props.set_simulation_random_seed(int(self._ui.lineEditRandomSeed.text()))
        # output
        self._props.set_output_interval(self._ui.spinBoxInterval.value())
        self._props.set_output_process(self._ui.spinBoxProcess.value())
        return self._props
    ...
    def _update_fame_time_text_areas(self):
        if self.fame_to_datetime_conversion_triggered:
            ...
            return
        fame_start_time: int = 0
        self._ui.fame_stop_time.setStyleSheet("QLabel {color: black}")
        self._ui.fame_start_time.setStyleSheet("QLabel {color: black}")

        try:

            fame_start_time = convert_gui_datetime_to_fame_time(self._ui.lineEditStartTime.text())
            self._ui.fame_start_time.setText(
                str(fame_start_time))

        except ConversionException as c_ex:

            self._ui.fame_start_time.setText(str(c_ex))
            self._ui.fame_start_time.setStyleSheet("QLabel {color: red}")

        fame_stop_time: int = 0

        try:
            fame_stop_time = convert_gui_datetime_to_fame_time(self._ui.lineEditStopTime.text())
            self._ui.fame_stop_time.setText(
                str(fame_stop_time))
