...
Item {
    ...
    property real opacityPie: 1.0        // opacity a pie
    property bool addCircleToPie: false  // add а circle to the pie in a radius
    ...
    Canvas {
        ...
        onPaint: {
            ...
            if (root.isPie) {
                ...
                if (root.colorCircle != "transparent") {
                    const hexToRgb = hex => hex.substring(1).match(/.{2}/g).map(x => parseInt(x, 16)/255)
                    let rgb = hexToRgb(root.colorCircle)
                    ctx.fillStyle = Qt.rgba(...rgb, opacityPie)
                }

                if (addCircleToPie) {
                    ctx.moveTo(x, y)
                    ctx.arc(x, y, width / 2 - 0.5, start, end, false)
                    ctx.lineTo(x, y)
                    ctx.fill()
