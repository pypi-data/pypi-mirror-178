...
Rectangle {
    ...
    RowLayout {
            ...
            Image {
                ...
                Image {
                    ...
                    source: {
                        ...
                        // new
                        if (object.security_state == "NO_STATE_WITH_GROUPS_INFO") return ""  // iconsPath + "icon-a-hub-status-icon-disarmed.svg"
        ...
        Item {
            ...
            Custom.FontText {
                id: sleepUntil
                color: sleepUntil.date ? ui.colors.green1 : ui.colors.middle1
                width: parent.width - 16
                font.pixelSize: 14
                wrapMode: Text.WordWrap
                elide: Text.ElideRight
                ...
                property var date: {
                    ...
                    if (!object.sleep_until) return null
                    if (!object.sleep_until.seconds) return null

                    return util.get_timedelta_by_timestamp(object.sleep_until)
                }

                text: {
                    if (!sleepUntil.date) return ui.empty

                    var trueDate = ""
                    if (sleepUntil.date.days) {
                        trueDate += sleepUntil.date.days + " " + tr.days + " "
                    }
                    if (sleepUntil.date.hours) {
                        trueDate += sleepUntil.date.hours + " " + tr.hrs + " "
                    }
                    if (sleepUntil.date.minutes) {
                        trueDate += sleepUntil.date.minutes + " " + tr.mins + " "
                    }
                    return trueDate
                }
            }
