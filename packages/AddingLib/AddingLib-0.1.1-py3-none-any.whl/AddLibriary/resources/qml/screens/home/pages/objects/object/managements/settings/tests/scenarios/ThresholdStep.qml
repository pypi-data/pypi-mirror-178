...
//  Selected air quality feature index
    property int selectedIndicatorIndex: !!scenario ? scenario.threshold.source_param - 68 : 0
//  Parameters of slider for each indicator
    property var slidersParameters: [
        {
            from: Math.round(temperature_converter.new(0, "metric").convert(
                settings.measure_system
            ).value / 2) * 2,
            to: Math.round(temperature_converter.new(50, "metric").convert(
                settings.measure_system
            ).value / 2) * 2,
            value: Math.round(temperature_converter.new(25, "metric").convert(
                settings.measure_system
            ).value / 2) * 2,
...
//  Threshold value for selected condition
    ...
    DS3.SettingsContainer {
        ...
        Repeater {
            ...
            model: [
                tr.temperature,
                tr.humidity_lq_value,
                tr.co_scenario,
            ]

            DS3.SettingsSingleSelection {
                atomTitle.title: modelData
                checked: selectedIndicatorIndex == index
                switchChecked: () => selectedIndicatorIndex = index
            }
        }
    }
    ...
    DS3.SettingsContainer {
        Repeater {
            ...
            Column {
                ...
                spacing: 1
                visible: index == selectedIndicatorIndex
        ...
        Repeater {
            ...
            Column {
                function selectValue(value) {
                    const newValue = selectedIndicatorIndex == 0
                        ? temperature_converter.new(
                            value, settings.measure_system
                        ).convert("metric").value
                        : value
                    selectedValue = [0, 1].includes(selectedIndicatorIndex) ? newValue * 10 : newValue
                }

                function loadValue() {
                    return selectedIndicatorIndex == 0 && settings.measure_system == "imperial"
                        ? Math.round(
                ...
                DS3.SettingsValueSlider {
                    ...
                    onValueChanged: if (visible && index == selectedIndicatorIndex) selectValue(value)
                }
