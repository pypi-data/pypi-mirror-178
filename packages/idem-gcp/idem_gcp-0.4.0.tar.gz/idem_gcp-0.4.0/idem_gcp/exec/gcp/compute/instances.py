...
async def list_(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.instances.list
              - kwargs:
    ...
    if zone:
        ret = await hub.exec.gcp_api.client.compute.instances.list(
            ctx,
            project=project,
            zone=zone,
            filter=filter,
        ...
        ret = await hub.exec.gcp_api.client.compute.instances.aggregatedList(
            ctx,
            project=project,
            filter=filter,
            orderBy=order_by,
...
async def get(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.instances.get
              - kwargs:
                  project: project-name
                  zone: zone-name
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.instances.get(
            ctx, resource_id=resource_id
        )
    else:
        ret = await hub.exec.gcp_api.client.compute.instances.get(
            ctx, project=project, zone=zone, instance=instance
        )
...
async def set_disk_auto_delete(
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.instances.setDiskAutoDelete(
            ctx,
    ...
    elif project and zone and instance:
        ret = await hub.exec.gcp_api.client.compute.instances.setDiskAutoDelete(
            ctx,
            project=project,
            zone=zone,
        ...
        result["result"] = False
        result["comment"] = [
            f"gcp.compute.instances#set_disk_auto_delete(): either resource_id or project, zone and instance should be specified."
        ]
        return result
    ...
    if "compute#operation" in ret["ret"].get("kind"):
        ...
        op_ret = await hub.exec.gcp.compute.instances.get(
            ctx, resource_id=op_ret["resource_id"]
        )
        if not op_ret["result"]:
            result["result"] = False
            result["comment"] += op_ret["comment"]
...
async def attach_disk(
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.instances.attachDisk(
            ctx,
            resource_id=resource_id,
            body=request_body,
        ...
        result["result"] = False
        result["comment"] = [
            f"gcp.compute.instances#attach_disk(): either resource_id or project, zone and instance should be specified."
        ]
        return result
...
async def detach_disk(
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.instances.detachDisk(
            ctx,
            resource_id=resource_id,
            deviceName=device_name,
            requestId=request_id,
