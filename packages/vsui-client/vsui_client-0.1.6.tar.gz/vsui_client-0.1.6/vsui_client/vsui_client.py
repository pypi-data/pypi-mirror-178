...
def safe_emit(event, data):
    ...
    #global timeout_count, timeout_target
    try:
        sio.emit(event, data)
        #timeout_count = 0
    except:
        #logging.warning(f'timeout {timeout_count} / {timeout_target}')
        #timeout_count += 1
        #if timeout_count >= timeout_target:
        logging.error(f'socketio timeout, exiting!')
        disconnect()
        sys.exit(1)
