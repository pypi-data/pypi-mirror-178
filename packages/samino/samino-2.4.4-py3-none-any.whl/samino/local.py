...
class Local(Session):
    ...
    def accept_promotion(self, requestId: str):
        if not isinstance(requestId, str):
            raise Exception(f"Please use a string not {type(requestId)}")
    ...
    def get_recent_blogs(self, pageToken: str = None, size: int = 25):
        req = self.getRequest(
            f"/x{self.comId}/s/feed/blog-all?pagingType=t&{f'pageToken={pageToken}' if pageToken else ''}&size={size}"
        )
        return RecentBlogs(req["BlogList"]).RecentBlogs
