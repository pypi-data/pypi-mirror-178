...
from .lib.sessions import sessionClass

class Local(sessionClass):
    def __init__(self, comId: str, proxies: dict = None):
        self.proxies = proxies
        self.comId = comId

        sessionClass.__init__(self, proxies = self.proxies)
    ...
    def get_chat_messages(self, chatId: str, size: int = 25):
        req = self.getRequest(f"/x{self.comId}/s/chat/thread/{chatId}/message?v=2&pagingType=t&size={size}")
    ...
    def get_all_users(self, type: str = "recent", start: int = 0, size: int = 25):
        req = self.getRequest(f"/x{self.comId}/s/user-profile?type={type}&start={start}&size={size}")
        return UserProfileList(req["userProfileList"]).UserProfileList
    ...
    def get_public_chats(self, type: str = "recommended", start: int = 0, size: int = 50):
        req = self.getRequest(f"/x{self.comId}/s/chat/thread?type=public-all&filterType={type}&start={start}&size={size}")
        return ThreadList(req["threadList"]).ThreadList
    ...
    def send_web_message(self, chatId: str, message: str = None, messageType: int = 0, icon: str = None, comId: str = None):
        ...
        req = self.postRequest("/add-chat-message", data, web = True)
        return Json(req)
    ...
    def follow(self, userId: Union[str, list]):
        ...
        if type(userId) is str: link = f"/x{self.comId}/s/user-profile/{userId}/member"
    ...
    def start_chat(self, userId: Union[str, list], title: str = None, message: str = None, content: str = None,chatType: int = 0):    
        if type(userId) is str: userIds = userId
        elif type(userId) is list: userIds = [userId]
    ...
    def invite_to_chat(self, userId: Union[str, list], chatId: str = None):
        ...
        data = {"uids": userIds, "timestamp": int(timestamp() * 1000)}
    ...
    def edit_chat(self, chatId: str, title: str = None, content: str = None, icon: str = None, background: str = None,
        ...
        if background:
            data = {"media": [100, background, None], "timestamp": int(timestamp() * 1000)}
    ...
    def chat_settings(self, chatId: str, viewOnly: bool = False, doNotDisturb: bool = True, canInvite: bool = False,
        ...
        if viewOnly:
            ...
            req = self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/view-only/{viewOnly}")
            res.append(Json(req))
        ...
        if canInvite:
            ...
            req = self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/members-can-invite/{canInvite}")
            res.append(Json(req))

        if canTip:
            if canTip: canTip = "enable"
            else:canTip = "disable"
            req = self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/tipping-perm-status/{canTip}")
            res.append(Json(req))
    ...
    def like_blog(self, blogId: str = None, wikiId: str = None):
        ...
        if blogId: link = f"/x{self.comId}/s/blog/{blogId}/vote?cv=1.2&value=4"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/vote?cv=1.2&value=4"
    ...
    def unlike_blog(self, blogId: str = None, wikiId: str = None):
        if blogId: link = f"/x{self.comId}/s/blog/{blogId}/vote?eventSource=FeedList"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/vote?eventSource=FeedList"
    ...
    def change_titles(self, userId: str, titles: list, colors: list):
        ...
        data = {"adminOpName": 207, "adminOpValue": {"titles": t}, "timestamp": int(timestamp() * 1000)}
    ...
    def unlike_comment(self, commentId: str, blogId: str = None, wikiId: str = None, userId: str = None):
        if blogId: link = f"/x{self.comId}/s/blog/{blogId}/comment/{commentId}/g-vote?eventSource=PostDetailView"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/comment/{commentId}/g-vote?eventSource=PostDetailView"
        elif userId: link = f"/x{self.comId}/s/user-profile/{userId}/comment/{commentId}/g-vote?eventSource=UserProfileView"
        else: raise TypeError("Please put a wiki or user or blog Id")
    ...
    def comment(self, comment: str, userId: str = None, blogId: str = None, wikiId: str = None, replyTo: str = None, isGuest: bool = False):
        ...
        if userId: link = f"/x{self.comId}/s/user-profile/{userId}/{comType}"
        elif blogId: link = f"/x{self.comId}/s/blog/{blogId}/{comType}"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/{comType}"
        else: raise TypeError("Please put a wiki or user or blog Id")
    ...
    def delete_comment(self, commentId: str, userId: str = None, blogId: str = None, wikiId: str = None):
        if userId: link = f"/x{self.comId}/s/user-profile/{userId}/comment/{commentId}"
        elif blogId: link = f"/x{self.comId}/s/blog/{blogId}/comment/{commentId}"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/comment/{commentId}"
        else: raise TypeError("Please put blog or wiki or user Id")
    ...
    def edit_comment(self, commentId: str, comment: str, userId: str = None, blogId: str = None, wikiId: str = None, replyTo: str = None, isGuest: bool = False):
        ...
        if userId: link = f"/x{self.comId}/s/user-profile/{userId}/{comType}/{commentId}"
        elif blogId: link = f"/x{self.comId}/s/blog/{blogId}/{comType}/{commentId}"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/{comType}/{commentId}"
        else: raise TypeError("Please put blog or wiki or user Id")
        
        req = self.postRequest(link, data)
        return Comment(req).Comments

    def get_comment_info(self, commentId: str, userId: str = None, blogId: str = None, wikiId: str = None, isGuest: bool = False):
        if isGuest: comType = "g-comment"
        else: comType = "comment"
    ...
    def get_blog_comments(self, wikiId: str = None, blogId: str = None, quizId: str = None, sorting: str = 'newest', size: int = 25, start: int = 0):
        ...
        if blogId: link = f"/x{self.comId}/s/blog/{blogId}/comment?sort={sorting}&start={start}&size={size}"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/comment?sort={sorting}&start={start}&size={size}"
    ...
    def get_blog_info(self, blogId: str = None, wikiId: str = None, folderId: str = None):
        if blogId: link = f"/x{self.comId}/s/blog/{blogId}"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}"
        elif folderId: link = f'/x{self.comId}/s/shared-folder/files/{folderId}'
    ...
    def tip_coins(self, coins: int, chatId: str = None, blogId: str = None, wikiId: str = None, transactionId: str = None):
        ...
        if chatId: link = f"/x{self.comId}/s/chat/thread/{chatId}/tipping"
        elif blogId: link = f"/x{self.comId}/s/blog/{blogId}/tipping"
        elif wikiId:
    ...
    def delete_message(self, chatId: str, messageId: str, asStaff: bool = False, reason: str = None):
        ...
        else: req = self.deleteRequest(f"/x{self.comId}/s/chat/thread/{chatId}/message/{messageId}")
    ...
    def hide(self, note: str = None, blogId: str = None, userId: str = None, wikiId: str = None, chatId: str = None):
        ...
        if userId: link = f"/x{self.comId}/s/user-profile/{userId}/admin"
        elif blogId: link = f"/x{self.comId}/s/blog/{blogId}/admin"
        elif wikiId: link = f"/x{self.comId}/s/item/{wikiId}/admin"
        elif chatId: link = f"/x{self.comId}/s/chat/thread/{chatId}/admin"
    ...
    def invite_to_voice_chat(self, userId: str = None, chatId: str = None):
        ...
        req = self.postRequest(f"/g/x{self.comId}/chat/thread/{chatId}/vvchat-presenter/invite", data)
        return Json(req)
    ...
    def activate_status(self, status: int = 1):
        data = {"onlineStatus": status, "duration": 86400, "timestamp": int(timestamp() * 1000)}
    ...
    def kick(self, chatId: str, userId: str, rejoin: bool = True):
        ...
        req = self.deleteRequest(f"/x{self.comId}/s/chat/thread/{chatId}/member/{userId}?allowRejoin={rejoin}")
        return Json(req)
    ...
    def flag(self, reason: str, type: str = "spam", userId: str = None, wikiId: str = None, blogId: str = None):
        ...
        data["flagType"] = types.get(type, 106)
    ...
    def get_notifications(self, size: int = 25, pagingType: str = "t"):
        req = self.getRequest(f"/x{self.comId}/s/notification?pagingType={pagingType}&size={size}")
        return NotificationList(req).NotificationList
    
    def get_notices(self, start: int = 0, size: int = 25, type: str = "usersV2", status: int = 1):
        req = self.getRequest(f"/x{self.comId}/s/notice?type={type}&status={status}&start={start}&size={size}")
    ...
    def sendWebActive(self):
        ...
        req = self.postRequest("/community/stats/web-user-active-time", data, web = True)
        return req

    def get_recent_blogs(self, pageToken: str = None, start: int = 0, size: int = 25):
        req = self.getRequest(f"/x{self.comId}/s/feed/blog-all?pagingType=t&{f'pageToken={pageToken}' if pageToken else ''}&size={size}")
        return RecentBlogs(req['BlogList']).RecentBlogs
        
    def publish_to_featured(self, time: int, userId: str = None, chatId: str = None, blogId: str = None, wikiId: str = None):
        time = {
            1: 3600,
            2: 7200,
            3: 10800
    ...
    def remove_from_featured(self, userId: str = None, chatId: str = None, blogId: str = None, wikiId: str = None):
        data = {
            "adminOpName": 114,
            "adminOpValue": {
                "featuredType": 0
