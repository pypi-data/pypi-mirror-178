...
class Cache(SeveralFilesCache):
    ...
    def write_cache(cls, data, **kwargs):
        with open(
            cls.fichier(cls.cachename.format(**kwargs)), mode="w", encoding="utf8"
        ) as csvfile:
    ...
    def read_cache(cls, **kwargs):
        ...
        effectifs = {}
        with open(
            cls.fichier(cls.cachename.format(**kwargs)), encoding="utf8"
        ) as csvfile:
