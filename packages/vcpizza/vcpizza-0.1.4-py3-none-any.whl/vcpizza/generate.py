...
from vcpizza.find import find_template
from vcpizza.utils import make_sure_path_exists, rmtree, work_in
...
def is_copy_only_path(path, context):
    ...
    :param context: pizza context.
    """
    try:
        for dont_render in context["vcpizza"]["_copy_without_render"]:
    ...
    for variable, overwrite in overwrite_context.items():
        ...
        if isinstance(context_value, list):
            ...
            if overwrite in context_value:
                ...
                # see ``pizza.prompt.prompt_choice_for_config``
...
def generate_context(context_file="vcpizza.json", default_context=None, extra_context=None):
    """Generate the context for a Pizza project template.

    Loads the JSON file as a Python object, with key being the JSON filename.

    :param context_file: JSON file containing key/value pairs for populating
        the pizza's variables.
...
def generate_file(project_dir, infile, context, env, skip_if_file_exists=False):
    ...
    :param infile: Input file to generate the file from. Relative to the root
        template dir.
    :param context: Dict for populating the pizza's variables.
    ...
    if is_binary(infile):
        ...
        with open(infile, encoding="utf-8", newline="") as rd:
            ...
            newline = rd.newlines
            if context["pizza"].get("_new_lines", False):
                newline = context["pizza"]["_new_lines"]
...
def generate_files(
    ...
    envvars = context.get("pizza", {}).get("_jinja2_env_vars", {})
