...
class ModelMetaClass(type):
    ...
    def __new__(cls, name, bases, attr_set):
        ...
        for k, v in attr_set.items():
            ...
            if isinstance(v, IntegerField) or isinstance(v,AutoField) or isinstance(v,TimeField) \
                    or isinstance(v,VarcharField) or isinstance(v,BigIntegerField)\
                    or isinstance(v,TextField) or isinstance(v,FloatField) or isinstance(v,BlobField) \
                    or isinstance(v,DoubleField) or isinstance(v,DateTimeField) or isinstance(v,DateField)\
                    :
                # 将key进行设置成类属性
