...
class Plugin(PluginBase):
    ...
    def on_train_epoch_start(self, trainer, model, epoch_index):
        if self._freeze_targets:
            freeze_modules(model, self._freeze_targets)
        return True

    def on_train_batch_start(self, trainer, model, batch, batch_index):
        if self._freeze_targets is None:
            self._freeze_targets = get_module_names_upto(model, self._module_name_pattern, batch[0])
            freeze_modules(model, self._freeze_targets, verbose=True)
        return batch
...
def get_module_names_upto(module, module_name_pattern, example_inputs):
    ...
    for n, m in module.named_modules():
        m.register_forward_hook(functools.partial(log_module_forward, module_name=n))

    logger.debug("Running the model once to get the order of modules.")
    with torch.no_grad():
...
def freeze_modules(module, module_names, verbose=False):
    for n, m in module.named_modules():
