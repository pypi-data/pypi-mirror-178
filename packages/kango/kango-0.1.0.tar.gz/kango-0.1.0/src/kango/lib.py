...
class ConfigLoader(dict):
    """
    Allow to load config

    Example
    config = Config("config.DEV")
    ex: config.NAME also config["NAME"]
    """
    ...
    def __getattr__(self, key):
        """
        Add ability to retrieve data as dot notation
        """
        return self[key]
...
def gen_otp(size=6) -> str:
    """
    Generate an OTP number that can be used
...
def parse_dsn(dsn: str) -> dict:
    """
    Parse a Database Source Name (DSN) that can be used breakdown DB  uri
    Format: [ scheme://username:password@hostname:port/dbname ]
    At a minimum, `scheme://hostname` should be there
...
def build_uri_from_dsn(dsn:str, login: bool = False, port: bool = True, dbname: bool = False) -> str:
    """
    Build back a parsed URI
    
    Returns:
        str
    """
    parsed_uri = parse_dsn(dsn)
    url = ""
    if parsed_uri["scheme"]:
        url += parsed_uri["scheme"] + "://"
    if login and parsed_uri["username"]:
        url += parsed_uri["username"]
        if parsed_uri["password"]:
            url += ":" + parsed_uri["password"]
        url += "@"
    if parsed_uri["hostname"]:
        url += parsed_uri["hostname"]
    if port and parsed_uri["port"]:
        url += ":%s" % parsed_uri["port"]
    if dbname and parsed_uri["dbname"]:
        url += "/" + parsed_uri["dbname"]
    return url
...
def gen_secret_key(length: int = 60) -> str:
    """
    Generate a secret key with a set length
    Params:
...
def hash_password(password: str) -> str:
    ...
    Returns: str
    """
    return generate_password_hash(password=password, salt_length=12)
...
def verify_password(pwhash: str, password: str) -> bool:
    ...
    Args:
      pwhash:str - the stored hashed password
