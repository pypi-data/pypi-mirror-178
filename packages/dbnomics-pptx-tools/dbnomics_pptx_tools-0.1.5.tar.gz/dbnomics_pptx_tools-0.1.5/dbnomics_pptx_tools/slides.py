...
def update_slide_charts(
    ...
    *,
    ...
    for chart_pos, chart_shape in enumerate(slide_shapes, start=1):
        ...
        logger.debug("Updating chart %d with name %r...", chart_pos, chart_name)

        chart_spec = slide_metadata.charts.get(chart_name)
        if chart_spec is None:
            logger.warning("No spec was found for chart %d with name %r, ignoring chart", chart_pos, chart_name)
            continue
