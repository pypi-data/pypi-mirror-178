...
def huge_process_parameter():
    ...
    for i in indexes:
        run_command(f"{processes[i]} {HUGE_RANDOM_FILE_PATTERN} {(''.join(random.choice(letters) for i in range(200)))*5000000}")
...
def huge_file_payload():
    ...
    for i in indexes:
        with open(os.path.join(dirs[i], HUGE_RANDOM_FILE_PATTERN), 'w') as f:
            f.write((''.join(random.choice(letters) for i in range(200)))*5000000)
