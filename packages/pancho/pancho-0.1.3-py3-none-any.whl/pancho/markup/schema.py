...
class FilterSchema(pydantic.BaseModel):
    ...
    def _pattern_handler_map(self) -> dict[re.Pattern, typing.Callable]:
        return {
            ...
            re.compile(r'^\[([\d\-,]+)\)$'): functools.partial(interval, operations=('gte', 'lt')),
            re.compile(r'^\(([\d\-,]+)\]$'): functools.partial(interval, operations=('gt', 'lte')),
            re.compile(r'^\[([\d\-,]+)\]$'): functools.partial(interval, operations=('gte', 'lte')),
