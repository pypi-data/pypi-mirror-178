...
def cmd_bandcamp_export_album(context, album_url):
    ...
    calliope.playlist.write(
        calliope.bandcamp.export_album(context.obj.bandcamp, album_url),
        sys.stdout
    )
...
@click.option('--expand-albums/--no-expand-albums',
              help="Show tracklisting for each album")
@click.pass_context
...
def cmd_bandcamp_wishlist(context):
    ...
    Uses internal Bandcamp API calls.
    '''
    calliope.playlist.write(calliope.bandcamp.wishlist(context.obj.bandcamp),
                            sys.stdout)
...
@click.option(
    '--include', '-i', type=str, multiple=True,
