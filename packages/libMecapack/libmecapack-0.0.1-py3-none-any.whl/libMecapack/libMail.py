...
class ListEmail(object):
    ...
    def __init__(self, pval=""):
        ...
        valTemp = pval
        if not isinstance(pval, string_types):
    ...
    def __sub__(self, pval):
        ...
        if not len(pval) == 0 and not len(valTemp) == 0:
            ...
            valTemp = self.SEPARATOR.join(
                [
                    itemTemp
                    for itemTemp in valTemp.split(self.SEPARATOR)
                    if itemTemp not in pval
                ]
            )
        return ListEmail(valTemp)
    ...
    def __isub__(self, pval):
        ...
        if not len(pval) == 0 and not len(self.__val) == 0:
            ...
            self.__val = self.SEPARATOR.join(
                [
                    itemTemp
                    for itemTemp in self.__val.split(self.SEPARATOR)
                    if itemTemp not in pval
                ]
            )
        return self
...
class Mails(UserList):
    ...
    def Send_all(self, pserver="", pport=0, puser="", ppassword=""):
        ...
        SMTP_management = SMTP(
            self.hshParam["server"], self.hshParam["port"], timeout=300
        )
        ...
        for msg_temp in self.data:
            if isinstance(msg_temp, Mail):
                ...
                try:
                    SMTP_management.sendmail(
                        mailSender, mailReceivers, msg_temp.msg.as_string()
                    )
                    mailSentOk += 1
                    ...
                    self.log.Warning(
                        "Mail not sent to %s : %s", mailReceivers, e, exc_info=1
                    )
                    mailSentKo += 1
            else:
    ...
    def TLS_Connect(self):
        ...
        SMTP_management = SMTP(
            self.hshParam["server"], self.hshParam["portTLS"], timeout=300
        )
            ...
            if os.path.isfile(pAttachFile_path):
                ...
                if not len(os.path.splitext(pAttachFile_newname)[0].strip()) == 0:
                    if len(os.path.splitext(pAttachFile_newname)[1].strip()) == 0:
                        fileTemp_name = (
                            os.path.splitext(pAttachFile_newname)[0].strip()
                            + fileTemp_extension
                        )
                    else:
                ...
                msg_attach_temp.add_header(
                    "Content-Disposition", "attachment", filename=fileTemp_name
                )
                # Attach the file to the message and save reference
                self.msg.attach(msg_attach_temp)
                self.__Attachments.append(
                    [fileTemp_name, pAttachFile_path, msg_attach_temp]
                )
