...
import libMecapack.libTool as Tools
import datetime
...
class Excel:
    ...
    def TemplateWB_Load(self, ptemplateWB_path):
        ...
        if os.path.isfile(ptemplateWB_path):
            ...
            raise self.log.CustomException(
                "Template file does not exist : %s", ptemplateWB_path
            )
            ...
            for n in range(1, c + 1):
                ...
                for row in pcursrows[s:e]:
                    ...
                    for index, item in enumerate(row):
                        if (
                            type(item) is datetime.date
                            or type(item) is datetime.datetime
                        ):
    ...
    def TemplateWB_CopyRows(
        ...
        # Select sheet
        self.log.Debug(
            "EXCEL - Copy source {} = {} to {} = {}".format(
                psheet_source, prange_source, psheet_dest, prange_dest
            )
        )
...
class Xlsx_read:
    ...
    def read_data(self, psheet_name):
        ...
        datas = []
        for is_first_elt, cells in Tools.signal_first(wb_sheet.values):
