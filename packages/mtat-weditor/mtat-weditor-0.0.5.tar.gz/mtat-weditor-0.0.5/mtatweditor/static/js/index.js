...
window.vm = new Vue({
  ...
  watch: {
    ...
    serial: function (newval) {
      // if (this.platform.toLowerCase() == "ios" && newval.indexOf("http") == 0) {
  ...
  methods: {
    checkVersion: function () {
      $.ajax({
        ...
        .fail((ret) => {
          this.showError("<p>Local server not started, start with</p><pre>$ python -m mtat-mtatweditor</pre>");
        })
        .always(() => {
          this.loading = false;
        })
    ...
    filterAttributeKeys(elem) {
      return Object.keys(elem).filter(k => {
        if (['children', 'rect', 'index'].includes(k)) {
          return false;
        }
    ...
    initDevicesList: function() {
      $.ajax({
        url: LOCAL_URL + "api/v1/devices/" + this.platform.toLowerCase(),
        type: "GET",
      })
        .done((ret) => {
          this.deviceList = ret.devices;
          if (this.deviceList.length > 0) {
            this.serial = this.deviceList[0].value;
          } else {
            this.serial = "";
          }
        })
    ...
    selectMapping() {
      this.$refs.mappingFile.dispatchEvent(new MouseEvent('click'));
    },
    mappingChange(e) {
      const _that = this;
      ...
      reader.onload = function () {
        _that.initMapping(this.result);
        _that.reloadNodeMappingID();
      }
    },
    initMapping(result) {
      this.mappingMaps = {};
      ...
      if (mappingList != undefined) {
          for (let mappingInfo of mappingList) {
              ...
              this.mappingMaps[String(info[1])] = info[0].substring(info[0].indexOf('/') + 1);
          }
      } else {
          ...
          for (let mappingInfo of mappingList) {
              ...
              if (regList.length > 2) {
                  this.mappingMaps[String(regList[2])] = regList[1];
              }
          }
      }
    },
    reloadNodeMappingID() {
      if (Object.keys(this.mappingMaps).length > 0) {
        for (let nodeIndex in this.originNodeMaps) {
          let resourceId = this.originNodeMaps[nodeIndex]["resourceId"];
          if (resourceId === undefined) {
            continue;
          }
          let searchKey = resourceId.substring(resourceId.indexOf('/') + 1)
          this.originNodeMaps[nodeIndex]["mappingId"] = this.mappingMaps[searchKey];
          if (this.nodeSelected) {
            let id = this.nodeSelected._id;
            this.nodeSelected = null;
            this.nodeSelected = this.originNodeMaps[id];
          }
        }
      }
    },
    generatePreloadCode() {
      ...
      let codeLines;
      if (m[1] == "ios") {
        if (deviceUrl.indexOf("http") == 0) {
          initStr = `c = d = wda.Client("${deviceUrl}")`
        } else {
          initStr = `c = d = wda.USBClient("${deviceUrl}")`
        }
        codeLines = [
    ...
    getIndex(elemType) {
      ...
      $.ajax({
        ...
        var indexStr = ret.index + " / " + ret.maxIndex;
        this.elem.index = indexStr;
        this.originNodeMaps[this.elem._id].index = indexStr;
      }).always(() => {
        this.dumping = false;
      })
    },
