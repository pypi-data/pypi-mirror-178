...
class install_ku_Thread (threading.Thread):
    def __init__(self, chat,name,yuan):
        threading.Thread.__init__(self)
        self.chat = chat
        self.read_stop=False
        self.name=name
    ...
    def run(self):
        ...
        elif (platform.uname().system=='Liunx' and platform.uname().machine=='aarch64'):
...
class ChatConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if data['msg']=='run':
            ...
            self.ku=install_ku_Thread(self,data['name'],yuan[int(yuan_id)])
            self.ku.start()
            self.send(json.dumps({'msg':'pip_msg','data':'开始安装'}))
