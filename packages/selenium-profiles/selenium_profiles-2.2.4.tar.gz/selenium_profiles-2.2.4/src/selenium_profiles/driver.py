...
class driver(object):
    ...
    def start(self, profile: Dict[str, dict or list], uc_driver: bool = False):
        ...
        if uc_driver:
            ...
            self.options.arguments.extend(["--no-default-browser-check", "--no-first-run"])  # suppress welcome
            self.options.arguments.extend(["--no-default-browser-check", "--no-first-run"])
            self.options.arguments.extend(["--disable-blink-features=AutomationControlled", "--disable-blink-features"])
