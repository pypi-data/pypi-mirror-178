...
class SAI(object):
    ...
    def __preprocess(self, df_dict: dict, y_dict: dict, q: int) -> List[List[str]]:
        """process input data dict and append discretized monthly pandas dataframe for training SAI

        Args:
            df_dict (dict): Input data dictionary with monthly pandas dataframe
            y_dict (dict): response dictionary monthly the security success indicator, label {0,1} whether the security in key mth i is successul (1)
            q (int): quantile for discretization
        ...
        No Longer Raises:
            ValueError: Raise error when not enough input for both training and validation
        ...
        for idx, mth in enumerate(sorted(df_dict.keys())):
            if idx == 0:
                Xsize = df_dict[mth].shape[1]
            else:
                if df_dict[mth].shape[1] != Xsize:
                    raise ValueError(f'period {mth} X data has different variables')
            if df_dict[mth].shape[0] != y_dict[mth].shape[0]:
                raise ValueError(
                    f"Number of rows for both X and y must be the same: {mth}")
            if y_dict[mth].shape[1] != 1:
                raise ValueError(
                    f"y cannot be more than one dimension; users can combine multiple objectives into one dimention before input")

            x_discretized, self.cutpoints = self.__discretize_mixed_type(
                df=df_dict[mth], q=q, disc_func=self.__discretize)
            y = y_dict[mth]
            y.iloc[:, 0] = y.iloc[:, 0].map({1: 'success', 0: 'fail'})
            Xy = pd.concat(
                [x_discretized, y.set_index(x_discretized.index)], axis=1)
            train_data.append(Xy)
        return pd.concat(train_data, axis=0).values.tolist()

    def fit(self, X: dict, y: dict) -> None:
        """Train SAI with input data. Build Rules and assign probabilites to each rule

        Args:
            X (dict): Input data dictionary each (key, item) represent data items for all securities for oen month
            y (dict): response dictionary suggest the security from a particular month is considered successful (1)
            metric (str, optional): metric used to sort output rules i.e. support, confidence, lift, leverage, conviction
        """
        if not isinstance(X, dict):
            raise ValueError(
                "X must be a dictionary with key as period and value as the corresponding pandas DataFrame n x m (i.e. n is number of sercurities and m is the number of variables")
        if not isinstance(y, dict):
            raise ValueError(
                "y must be a dictionary with key as period and value as the label indentifying whether the security at period {key} is considered as successful")
        if set(X.keys()) != set(y.keys()):
            raise ValueError(
                "The keys(months) for both X and y must be the same")
