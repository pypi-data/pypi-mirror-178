...
class QuantumMachinesManager(object):
    def __init__(self, host=None, port=None, **kwargs):
        ...
        if "log_level" in kwargs:
            new_level = kwargs.get("log_level")
            try:
    ...
    def perform_healthcheck(self, strict=True):
        ...
        with _grpc_context():
            ...
            for msg in res.warningMessages:
                self._log.warning("Health check warning: " + msg)
            ...
            if res.ok:
                ...
                for msg in res.message:
                    self._log.error("Health check error: " + msg)
    ...
    def _open_qm_pb(self, pb_config, close_other_machines=False):
        with _grpc_context():
            ...
            for warning in result.openQmWarnings:
                self._log.warning(
                    "Open QM ended with warning %s: %s", warning.code, warning.message
                )
