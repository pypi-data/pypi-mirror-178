...
from qm._QmJobErrors import (
    ...
    InvalidDigitalInputDeadtimeError,
...
from qm.pb.job_manager_pb2 import (
    ...
    SetDigitalInputDeadtimeRequest,
    GetDigitalInputDeadtimeRequest,
...
class QmJob:
    ...
    def set_digital_input_deadtime(self, port: Tuple[str, int], deadtime: int) -> int:
        request = SetDigitalInputDeadtimeRequest()
        request.jobId = self._id
        request.digitalInputPort.SetInParent()
        request.digitalInputPort.controllerName = port[0]
        request.digitalInputPort.portNumber = port[1]
        request.deadtime = deadtime

        response = self._job_manager_client.SetDigitalInputDeadtime(request)
        valid_errors = (MissingDigitalInputError, InvalidDigitalInputDeadtimeError)
        _handle_job_manager_error(request, response, valid_errors)
        return response.deadtime

    def get_digital_input_deadtime(self, port: Tuple[str, int]) -> int:
        request = GetDigitalInputDeadtimeRequest()
        request.jobId = self._id
        request.digitalInputPort.SetInParent()
        request.digitalInputPort.controllerName = port[0]
        request.digitalInputPort.portNumber = port[1]

        response = self._job_manager_client.GetDigitalInputDeadtime(request)
        valid_errors = (MissingDigitalInputError,)
        _handle_job_manager_error(request, response, valid_errors)
        return response.deadtime
    ...
    def set_digital_input_polarity(self, port: Tuple[str, int], polarity: str) -> str:
        ...
        request.digitalInputPort.controllerName = port[0]
        request.digitalInputPort.portNumber = port[1]
        request.polarity = RISING
        if polarity == "RISING":
            request.polarity = RISING
        elif polarity == "FALLING":
            request.polarity = FALLING
        else:
    ...
    def get_digital_input_polarity(self, port: Tuple[str, int]) -> str:
        ...
        valid_errors = (MissingDigitalInputError,)
        _handle_job_manager_error(request, response, valid_errors)
        if response.polarity == RISING:
            return "RISING"
        elif response.polarity == FALLING:
            return "FALLING"
        else:
            raise Exception(
