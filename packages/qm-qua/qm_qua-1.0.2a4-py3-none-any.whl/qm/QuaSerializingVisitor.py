    ...
    def _search_adc_trace(values):
        ...
        for value in values:
            if isinstance(value, str):
                ...
                found, name = QuaSerializingVisitor._search_adc_trace(value.list_value)
                if found is False or name is not None:
                    return found, name
                else:
                    return found, QuaSerializingVisitor._get_result_name(value)
        return False

    @staticmethod
    def _search_auto_added_stream(values):
        is_auto_added_result = values[2].string_value == "auto"
        if is_auto_added_result:
            index = values[1].string_value.rfind("_")
            name = values[1].string_value[:index]
        else:
            name = None
        return is_auto_added_result, name

    def _fix_legacy_save_and_adc_trace(self, name, node):
        is_adc_trace_true, trace_name = self._search_adc_trace(node.values)
        is_auto_added_stream, save_name = self._search_auto_added_stream(node.values)
        ...
        elif is_adc_trace_true:
            for i in range(len(self._lines)):
                self._lines[i] = self._lines[i].replace(
                    f"{trace_name} = declare_stream()",
