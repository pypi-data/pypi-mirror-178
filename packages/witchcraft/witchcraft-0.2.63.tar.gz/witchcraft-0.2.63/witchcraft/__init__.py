...
def append_history(connection, schema_name, table_name, data_points, primary_keys=None, version_column='version', batch_size=None):
    ...
    prepare_table(connection, schema_name, table_name, first_batch, primary_keys, None, version_column=version_column)
    ...
    while True:
        ...
        if len(batch) > 0:
            prepare_table(connection, schema_name, table_name, batch, primary_keys, None, version_column=version_column)
            inserted_total += insert_data(connection, schema_name, table_name, batch)
            read_total += len(batch)
