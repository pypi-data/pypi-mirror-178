...
class H2OExtendedIsolationForestMOJOModel(H2OTreeBasedUnsupervisedMOJOModelParams, HasIgnoredColsOnMOJO):

    @staticmethod
    def createFromMojo(pathToMojo, settings=H2OMOJOSettings.default()):
        # We need to make sure that Sparkling Water classes are available on the Spark driver and executor paths
        Initializer.load_sparkling_jar()
        javaModel = _jvm().ai.h2o.sparkling.ml.models.H2OExtendedIsolationForestMOJOModel.createFromMojo(pathToMojo, settings.toJavaObject())
    ...
    def getCrossValidationModels(self):
        ...
        if cvModels is None:
            ...
            return [H2OExtendedIsolationForestMOJOModel(v) for v in cvModels]
        else:
    ...
    def getExtensionLevel(self):
        value = self._java_obj.getExtensionLevel()
        return value
