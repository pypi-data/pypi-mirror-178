...
def make_table(array,header ='',align = '',latex=True, transpose = False, error_mode = 'plus-minus'):
    try:
    ...
    for i,j in enumerate(array):             # gibt k = 2, wenn as Array 2-dim ist, k = 1 für 1-dim arrays
        try:
            k[i] = np.shape(j)[1]
            k[i] = 2
            # if isinstance(j[1], (type([1,2,3]), np.ndarray)):
            #     print('liste')
            # else:
            #     print('zahl')
        except IndexError:
        ...
        for i in range(length):
            ...
            for l,m in enumerate(k):
                if m == 1:
                    ...
                    while int(error) != error:
                        error = error*1e1
                    list[l] = f'${array[l][0][i]}({int(error)})$'
                elif m == 2 and error_mode == 'scientific':
                    value = float(array[l][0][i])
                    error = array[l][1][i]
                    val_str = array[l][0][i] # wert als string
                    dec_places = 0
                    ...
                    if e != 0:
                        list[l] = f'${result}({error})\\times 10^\u007b{e}\u007d$'
                    else:
                        list[l] = f'${result}({error})$'
