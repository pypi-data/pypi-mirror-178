...
def make_pr_excel(filename, language):
    ...
    # merge forms
    try:
        es = [Excel(temp_dir / f"{e}.xlsx") for e in excels]
...
def output(args, actions):
    if args.to:
        with open(args.to, "w") as f:
            ...
            show_success(f"{args.to} is created")
    else:
...
def main():
    ...
    parser.add_argument(
        "-m",
        "--make",
        help="make an Excel for PR data input, with or without .xlsx is ok",
    )
    parser.add_argument(
        "-check",
        "--check",
        help="Check an Excel for PR data, with or without .xlsx is ok",
    )
    parser.add_argument(
