...
class ArtifactEntry:
    ...
    def __init__(self, path: str, local_path: str, size: int, digest: str, status: str):
    ...
    def download(self, local_root_path: str, artifact_id: int, team_name: str, exp_name: str, artifact_name: str, artifact_type: str, file_index: int, total_file_count:int ):
        ...
        interface.download_file(self.path, artifact_id, local_path, team_name, exp_name, artifact_name, artifact_type, file_index, total_file_count, self.size)

        # 다운로드 완료 후 local_path 설정
        self.local_path = local_path

    # ArtifactEntry 객체를 기반으로 파일 업로드 수행
    def upload(self, upload_type: str, root_path: str, run_id: int, artifact_id: int, last_file_yn: str, teamName: str, expName: str, artifact_name: str, artifact_type: str, artifact_digest: str, entry_list: List[grpc_ArtifactEntry], file_index:int) -> bool:
        ...
        return interface.upload_file( upload_type, self.local_path, root_path, self.path, run_id, artifact_id, last_file_yn, teamName, expName, artifact_name, artifact_type, artifact_digest, self.digest, entry_list, file_index=file_index)
...
class Artifacts:
        ...
        if isinstance(data, str):
            if not os.path.isfile(data):
                ...
                if entry:
                    ...
                    self.entry_list.append(ArtifactEntry(path, local_path, size, digest, status="ADD"))  # 파일이 추가된 경우
    ...
    def upload(self) -> bool:
        ...
        for i, entry in enumerate(self.entry_list):
            ...
            succeeded = entry.upload(upload_type, root_path, self.run.run_id, self.id, last_file_yn,  self.run.team_name, self.run.exp_name, self.name, self.type, artifact_digest, entry_list, file_index=i)
            if not succeeded:
                return False
        return True
    ...
    def download(self) -> str:
        ...
        for idx, entry in enumerate(self.entry_list):
            entry.download(local_root_path, self.id, self.run.team_name, self.run.exp_name, self.name, self.type, idx, total_file_count=len(self.entry_list))
