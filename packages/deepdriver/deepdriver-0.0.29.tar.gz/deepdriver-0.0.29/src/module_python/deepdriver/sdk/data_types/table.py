...
class Table(Media):
    ...
    def upload_file(self, run: Run, key_name: str) -> None:
        ...
        # interface.upload_file("RUN", local_path, root_path, path, run.run_id)
        interface.upload_file(upload_type="RUN", local_path=local_path, root_path=root_path, path=path, run_id=run.run_id, artifact_id=0, last_file_yn="Y",teamName="", expName="", artifact_name="", artifact_type="", artifact_digest="", entry_digest="", entry_list=[], file_index=0)
