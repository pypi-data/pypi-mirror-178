...
def first_run_switch_optin_rasa_telemetry():
    models_path = print_paths(models_path=f"models/{I18N}/NLU")
    if not Path(f"{ASSISTANT_PATH}/{models_path}").exists() or not glob.glob(f"{ASSISTANT_PATH}/{models_path}/*.tar.gz"):
