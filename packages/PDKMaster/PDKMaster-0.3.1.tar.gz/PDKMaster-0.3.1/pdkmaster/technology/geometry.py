...
from typing import (
    Any, Dict, Iterable, Iterator, Generator, Collection, Tuple, List,
    Optional, Union, TypeVar, cast, overload
)

from .. import _util
from ..typing import IntFloat, SingleOrMulti
...
__all__ = [
    ...
    "Start", "SetWidth", "GoLeft", "GoDown", "GoRight", "GoUp", "NoStart",
]
...
class Rotation(enum.Enum):
    ...
    def __mul__(self, shape: "MaskShapes") -> "MaskShapes":
        ... # pragma: no cover
    def __mul__(self, shape) -> Union["Rotation", "_Shape", "MaskShape", "MaskShapes"]:
...
class Rect(Polygon, _Rectangular):
    ...
    def from_rect(
        *, rect: "_Rectangular",
        ...
        bias: Union[float, prp.Enclosure]=0.0,
    ) -> "Rect":
        if not isinstance(bias, prp.Enclosure):
            bias = prp.Enclosure(bias)
        hbias = bias.first
        vbias = bias.second
        if left is None:
            left = rect.left
...
class MultiPath(Polygon):
    ...
    manhattan connections of verying width between points.
    """
    class _Instruction:
        pass
    ...
    class Start(_Instruction):
        ...
        def __init__(self, *, point: Point, width: IntFloat):
            if width < -epsilon:
                raise ValueError(
                    f"width has to be a positive value not '{width}'"
                )
            self._point = point
            self._width = _util.i2f(width)
    ...
    class SetWidth(_Instruction):
        ...
        def __init__(self, width: IntFloat):
            if width < -epsilon:
                raise ValueError(
                    f"width has to be a positive value not '{width}'"
                )
    ...
    class _Go(_Instruction):
        ...
        def __init__(self, dist: IntFloat):
            dist = _util.i2f(dist)
            if dist < -epsilon:
                raise ValueError(
    ...
    class GoLeft(_Go):
        ...
        API Notes:
            This class ig final, subclassing may cause backwards compatiblity problems.
        """
        pass
    class GoDown(_Go):
    ...
    # All instructions except start; for typinh only
    NoStart = Union[SetWidth, _Go]
    ...
    def __init__(self, first: Start, *instrs: "MultiPath.NoStart"):
        ...
        point: Point = first._point
        width: float = first._width
        prevwidth: float = width
        previnstr: MultiPath._Instruction = first
        prevdir: type = MultiPath.Start
        clkwcoords: List[Point] = []
        cclkwcoords: List[Point] = []
        for instr in instrs:
            prevtype: type = type(previnstr)
            ...
            # Add coordinates
            if prevdir == MultiPath.Start:
                if instrtype == MultiPath.SetWidth:
                    raise ValueError("First instruction after 'Start' should not be 'SetWidth'")
                elif instrtype == MultiPath.GoLeft:
                    dxy = Point(x=0.0, y=0.5*width)
                    clkwcoords.append(point - dxy)
                    cclkwcoords.append(point - dxy)
                    cclkwcoords.append(point + dxy)
                ...
                prevwidth = width
            elif instrtype == MultiPath.SetWidth:
                newwidth = cast(MultiPath.SetWidth, instr)._width
                prevwidth = width
                width = newwidth
            else:
                if prevdir == MultiPath.GoLeft:
                    if instrtype == MultiPath.GoLeft:
                        dxy1 = Point(x=0.0, y=0.5*prevwidth)
                        dxy2 = Point(x=0.0, y=0.5*width)
                        clkwcoords.extend((point - dxy1, point - dxy2))
                        cclkwcoords.extend((point + dxy1, point + dxy2))
                    elif instrtype == MultiPath.GoDown:
                ...
                elif prevdir == MultiPath.GoRight:
                    if instrtype == MultiPath.GoLeft:
                        raise ValueError(
            ...
            # Move the point
            if instrtype == MultiPath.GoLeft:
                point += Point(x=-cast(MultiPath.GoLeft, instr)._dist, y=0.0)
            elif instrtype == MultiPath.GoDown:
                point += Point(x=0.0, y=-cast(MultiPath.GoDown, instr)._dist)
            elif instrtype == MultiPath.GoRight:
                point += Point(x=cast(MultiPath.GoRight, instr)._dist, y=0.0)
            elif instrtype == MultiPath.GoUp:
                point += Point(x=0.0, y=cast(MultiPath.GoDown, instr)._dist)
            elif instrtype != MultiPath.SetWidth: # pragma: no cover
                raise RuntimeError(
                    f"Internal error: unknown instruction type '{instrtype}'"
                )
        ...
        # Complere the last instruction
        prevtype = type(previnstr)
        if prevtype == MultiPath.SetWidth:
            raise ValueError(
        ...
        super().__init__(points=(*clkwcoords, *reversed(cclkwcoords)))
...
class Ring(Polygon):
    ...
    """
    def __init__(self, *, outer_bound: Rect, ring_width: IntFloat):
        ...
        ileft = oleft + ring_width
        ibottom = obottom + ring_width
        iright = oright - ring_width
        itop = otop - ring_width
...
class RectRing(_Shape):
    ...
    def __init__(self, *,
        outer_bound: Rect,
        rect_width: IntFloat, rect_height: Optional[IntFloat]=None,
        min_rect_space: IntFloat,
    ):
...
class MultiPartShape(Polygon):
    ...
    class _Part(Polygon):
        ...
        def moved(self, *, dxy: Point, context: Optional[MoveContext]=None):
            if context is None:
