...
from napalm_ros_di_di.utils import (
    to_seconds,
    iface_addresses,
    rtt,
)
from napalm_ros_di_di.query import (
    bgp_instances,
    bgp_advertisments,
    bgp_peers,
    lldp_neighbors,
...
class ROSDriver(NetworkDriver):
    ...
    def get_environment(self):
        ...
        for entry in self.api('/system/health/print'):
            if 'temperature' in entry:
                ...
                environment['temperature']['temperature'] = {'temperature': temperature, 'is_alert': False, 'is_critical': False}

            if 'fan1-speed' in entry:
                status = (int(entry['fan1-speed']) > 50)
                environment['fans']['fan1-speed'] = {'status': status}
            if 'fan2-speed' in entry:
                status = (int(entry['fan2-speed']) > 50)
                environment['fans']['fan2-speed'] = {'status': status}

            if 'psu1-state' in entry:
                status = (entry['psu1-state'] == 'ok')
                environment['power']['psu1-state'] = {'status': status, 'capacity': 0.0, 'output': 0.0}

            if 'psu2-state' in entry:
                status = (entry['psu2-state'] == 'ok')
                environment['power']['psu2-state'] = {'status': status, 'capacity': 0.0, 'output': 0.0}
    ...
    def get_facts(self):
        ...
        # try:
        #     query = f"select * from dcim_device join ipam_ipaddress on dcim_device.primary_ip4_id = ipam_ipaddress.id where TEXT(ipam_ipaddress.address) like '%{self.hostname}%'"
        #     print(query)
        #     select_device = Device.objects.raw(query)
        #     print(select_device)
        ...
        if not select_device:
            raise Exception('Устройство не найдено для извлечения серийного номера')
        ...
        if select_device.serial != serial_number:
            serial_number += '  !!!Отличается от указанного серийного номера в нетбоксе!!!'
        ...
        return {
            ...
            'interface_list': napalm.base.utils.string_parsers.sorted_nicely(
                tuple(str(iface['name']) for iface in interfaces),
            ),
        }
