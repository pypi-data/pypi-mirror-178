...
class AddonsLoader:
    ...
    def __init__(self, addons_dir_path: Path, files_repo: FilesResource, store: DataStore):
    ...
    async def discover_addons(self):
        for filepath in await self.files_repo.list(self.addons_path):
            await self._discover_addon(filepath.name)
    ...
    async def find_addon_by_name(self, name: str) -> List[ApiAddon]:
        middlewares = await self.store.get_item("addon")
        if not middlewares:
            return []
        else:
