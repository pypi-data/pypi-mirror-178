...
def generate_datastore_field_paths(data_store: types.DataStore) -> List[str]:
    paths = []
    if type(data_store) is not dict:
        data_store = data_store.to_dict()
    for path in generate_field_mask_paths(data_store):
...
def should_enable_databricks_registration(data_store: types.DataStore) -> bool:
    if data_store.type == "databricks" and data_store.databricks:
        params = data_store.databricks.params or dict()
        return params.get("enable_registration") == "true"
    return False


def extract_limit_from_query(statement: TokenList) -> Optional[int]:
    """
    Credit: https://github.com/apache/superset/blob/master/superset/sql_parse.py#L60
...
def create_limit_query(query: str, limit: Optional[int]) -> str:
    formatted_query = sqlparse.format(query, strip_comments=True)
    formatted_query = formatted_query.strip(" \t\n;")
    statements = sqlparse.parse(formatted_query)
    if not statements:
        return query
    else:
        existing_limit = extract_limit_from_query(statements[0])
        if existing_limit is None and limit and limit > 0:
            formatted_query = sqlparse.format(query)
            formatted_query = formatted_query.strip(" \t\n;")
            formatted_query += " limit {}".format(limit)
...
def is_select_query(query: str) -> bool:
    """
    Credit: https://github.com/apache/superset/blob/master/superset/sql_parse.py#L146
    License: Apache 2.0
    """
    ...
    return any(token.ttype == DML and token.value == "SELECT" for token in parsed[0])
