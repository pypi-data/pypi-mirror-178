...
public:
    ...
    float scale = 1.0F;
    float get_scale() override { return scale; }

    void swap() override { glfwSwapBuffers(window); }
...
class GLFWSystem : public System
{
    ...
    static uint32_t glfw2key(uint32_t code)
    {
        auto it = glfw_map.find(code);
        if (it != glfw_map.end()) { return static_cast<int>(it->second); }
        return code;
    }
...
public:
    ...
    std::shared_ptr<Screen> init_screen(Screen::Settings const& settings) override
    {
    ...
    void char_was_pressed(int codepoint)
    {
    ...
    void key_was_pressed(int key, int scancode, int action, int mods)
    {
        ...
        if (key >= 0x20 && key <= 0x7f) {
            ...
            if (down) {
                event_queue.emplace_back(KeyEvent{c, 0, 0});
                pressed.insert(c);
            }
        } else {
            ...
            if (it != glfw_map.end()) {
                ...
                if (down) {
                    pressed.insert(k32);
                    event_queue.emplace_back(KeyEvent{k32, 0, 0});
                }
            }
        }
    }
    ...
    bool is_pressed(uint32_t code, int device) override
    {
        ...
        if (code >= 0x20 && code <= 0x7f) {
            return glfwGetKey(window, std::toupper(code)) != GLFW_RELEASE;
        }
