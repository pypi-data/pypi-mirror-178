...
class  MorphabelModel(object):
    ...
    def fit(self, x, X_ind, max_iter = 4, isShow = False):
        ...
        if isShow:
            ...
            X_ind_gpu = cupy.asarray(X_ind)
            fitted_sp, fitted_ep, s, R, t = fit.fit_points(x_gpu, X_ind_gpu, self.model, n_sp = self.n_shape_para, n_ep = self.n_exp_para, max_iter = max_iter)
        ...
        return cupy.asnumpy(fitted_sp), cupy.asnumpy(fitted_ep), cupy.asnumpy(s), angles, cupy.asnumpy(t)
