...
from cubicweb.predicates import is_instance, has_related_entities, on_fire_transition
from cubicweb.server.hook import (
    Hook,
    Operation,
    LateOperation,
    DataOperationMixIn,
...
class RunChainCreation(Hook):
    ...
    def __call__(self):
        ...
        if "name" not in runchain.cw_edited:
            runchain.cw_edited["name"] = "<unnamed runchain %s>" % runchain.eid
    ...
    def precommit_event(self):
        ...
        for st in self.get_data():
            ...
            if st.wlang:
                ...
                self.cnx.create_entity("WiringLanguage", reverse_wlang=st, **kwargs)
...
class InputParameterValueAddUpdateHook(Hook):
    ...
    def __call__(self):
        ...
        if ival.pdef.param_type == "input":
            InputParameterValueOrFromOutputRequiredOp.get_instance(self._cw).add_data(
                ival
            )
...
class FromOutputDeleteHook(Hook):
    ...
    def __call__(self):
        ...
        InputParameterValueOrFromOutputRequiredOp.get_instance(self._cw).add_data(pval)
...
class ValueFileDeleteHook(Hook):
    ...
    def __call__(self):
        ...
        if entity.ptype == "input":
            ...
            RemoveValueFileIfLastReferenceOp.get_instance(self._cw).add_data(fval)
    ...
    def precommit_event(self):
        ...
        for runchain in self.get_data():
            if runchain.eid in pendingeids or not runchain.wiring:
                continue
            self.cnx.execute(
                "DELETE Run R WHERE S has_runs R, S eid %(s)s", {"s": runchain.eid}
            )
...
class RunChainAddWiringHook(Hook):
    ...
    __select__ = Hook.__select__ & match_rtype(
        "wiring",
    )
...
class OutputValueCopyOp(Operation):
    def precommit_event(self):
        ...
        rql = (
            "SET IV %(rtype)s V WHERE IV from_run R, IV from_output OD, "
...
class RunChainCheckCompleteOp(DataOperationMixIn, Operation):
    def precommit_event(self):
        for runchain in self.get_data():
            ...
            if all(
                run.cw_adapt_to("IWorkflowable").state == "wfs_run_completed"
