...
class UsesExecutableHookTC(ProcessingTCMixin, CubicWebTC):
    ...
    def test_wiring_language(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            # test edition
            cnx.execute(
                "DELETE S uses_executable E WHERE S eid %(s)s, " "E eid %(e)s",
                {"s": runchain.eid, "e": self.exe1eid},
            )
            cnx.commit()
            self.assertEqual(len(self.runchain_wlang_def(runchain)["modules"]), 1)
            # test deletion
            cnx.execute("DELETE RunChain S WHERE S eid %(s)s", {"s": runchain.eid})
            cnx.commit()
...
class ParameterOfHookTC(ProcessingTCMixin, CubicWebTC):
    def setup_database(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            fields = self.wlang_fields_by_module(self.runchain_wlang_def(runchain))
...
class InputParameterValueOrFromOutputRequiredTC(ChainingTestMixin, CubicWebTC):
    ...
    def test_value_required(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            with self.assertRaises(ValidationError) as cm:
                ce("ParameterValueFloat", param_def=self.idef1_feid, value_of_run=run1)
                cnx.commit()
    ...
    def _file_pvalue(self, cnx, with_value=True):
        ce = cnx.create_entity
        exe = ce("Executable", name="e")
        pdef = exe.add_input("fp", "File")
        cnx.commit()
