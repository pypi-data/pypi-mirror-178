...
class RunExecutableTraceabilityTC(CubicWebTC):
    ...
    def setup_database(self):
        ...
        with self.new_access("toto").client_cnx() as cnx:
            exe = cnx.create_entity("Executable", name="exe")
            i1 = cnx.create_entity(
                "ParameterDefinition",
                value_type="Float",
                param_type="input",
                name="i1",
    ...
    def test_exe_modification(self):
        ...
        with self.new_access("toto").client_cnx() as cnx:
            ...
            with self.assertRaises(Unauthorized):
                exe.cw_set(name="boom")
                cnx.commit()
        with self.admin_access.repo_cnx() as cnx:
            exe = cnx.entity_from_eid(self.exe_eid)
            exe.cw_set(name="moob")
            cnx.commit()

    def change_executable(self, cnx):
        e1 = cnx.create_entity("Executable", name="exe1")
        cnx.execute(
            "SET R executable E WHERE R is Run, R eid %(r)s, " "E eid %(e)s",
            {"e": e1.eid, "r": self.run_eid},
        )
        cnx.commit()
    ...
    def delete_executable(self, cnx):
        cnx.execute("DELETE Executable E WHERE E eid %(e)s", {"e": self.exe_eid})
        cnx.commit()
    ...
    def test_pdef_add(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            with self.assertRaises(ValidationError) as ctm:
                exe.add_input(name="p", value_type="Float")
                cnx.commit()
            self.assertExcMsg(
                ctm,
                "can't modify parameters of an executable "
    ...
    def remove_parameter_of_relation(self, cnx):
        rql = (
            "DELETE I parameter_of E WHERE E is Executable, E eid %(e)s, "
    ...
    def delete_pval(self, cnx):
        cnx.execute(
            "DELETE ParameterValueFloat V WHERE V eid %(v)s", {"v": self.i1v_eid}
        )
        cnx.commit()
    ...
    def change_param_def(self, cnx):
        cnx.execute(
            "SET V1 param_def D2, V2 param_def D1 WHERE "
    ...
    def test_pdef_update_no_pval(self):
        ...
        with self.new_access("toto").client_cnx() as cnx:
            ...
            with self.assertRaises(Unauthorized):
                cnx.execute(
                    "DELETE I parameter_of E WHERE E is Executable, "
            ...
            with self.assertRaises(Unauthorized):
                cnx.execute(
                    "DELETE ParameterDefinition D WHERE D eid %(d)s", {"d": self.i1_eid}
                )
                cnx.commit()
