...
in_same_study = RQLConstraint(
    "NOT EXISTS(O in_study ST2, S in_study ST1, NOT ST1 identity ST2)"
)
...
class RunChain(WorkflowableEntityType):
    ...
    uses_executable = SubjectRelation(
        "Executable", cardinality="**", __permissions__=rel_perms_of_subject
    )
    has_runs = SubjectRelation(
        "Run",
        cardinality="*?",
        composite="subject",
        __permissions__=rel_perms_of_subject,
        constraints=[in_same_study],
    )
    wlang = SubjectRelation(
        "WiringLanguage",
        cardinality="??",
        inlined=True,
        composite="subject",
    ...
    wiring = SubjectRelation(
        "Wiring",
        cardinality="??",
        inlined=True,
        composite="subject",
        constraints=[same_lang_cstr],
        __permissions__=rel_perms_of_subject,
    )
...
class Run(WorkflowableEntityType):
    ...
    executable = SubjectRelation(
        ...
        constraints=[
            RQLConstraint(
                'NOT EXISTS(PD parameter_of O, PD param_type "input",'
...
class run_in_study(_in_study):
    subject = "Run"
    constraints = [
        RQLConstraint("NOT EXISTS(RC has_runs S, RC in_study ST, NOT ST identity O)")
    ]
...
class runchain_in_study(_in_study):
    subject = "RunChain"
    constraints = [
        RQLConstraint("NOT EXISTS(S has_runs R, R in_study ST, NOT ST identity O)")
    ]
...
class ParameterDefinition(EntityType):
    __permissions__ = {
        ...
        "update": (
            "managers",
            ERQLExpression("U has_update_permission E, X parameter_of E"),
        ),
        "delete": (
            "managers",
            ERQLExpression("U has_update_permission E, X parameter_of E"),
    ...
    name = String(
        maxsize=256, required=True, constraints=[RegexpConstraint("^[a-zA-Z0-9_]+$")]
    )
    value_type = String(
        required=True,
    ...
    param_type = String(
        required=True, internationalizable=True, vocabulary=(_("input"), _("output"))
    )
    description = String()
    parameter_of = SubjectRelation(
        "Executable",
        cardinality="1*",
        inlined=True,
        composite="object",
...
class _ParameterValue(EntityType):
    ...
    __permissions__ = {
        ...
        "update": (
            "managers",
            ERQLExpression("U has_update_permission R, X value_of_run R"),
        ),
        "delete": ("managers",),
    }
    ...
    from_run = SubjectRelation(
        ...
        inlined=True,
        description=_(
            "run from which to copy into this input parameter value "
            "when chaining runs"
        ),
        constraints=[
            RQLConstraint(
                "O executable E, OD parameter_of E, "
                'OD value_type T, OD param_type "output", '
                "S param_def PD, PD value_type T",
                msg=_("specified run has no output " "parameter of the right type"),
            )
        ],
        __permissions__=rel_perms_of_subject,
    )
    ...
    from_output = SubjectRelation(
        ...
        inlined=True,
        description=_(
            "output parameter definition to copy into this input parameter "
            "value when chaining runs"
        ),
        constraints=[
            RQLConstraint(
                "O value_type T, S param_def PD, PD value_type T",
                msg=_("copied output parameter has wrong value type"),
            ),
            RQLConstraint(
                "S from_run R, R executable E, O parameter_of E",
                msg=_("copied output parameter is unrelated to source run"),
            ),
            RQLConstraint(
                'O param_type "output", S param_def PD, PD param_type "input"',
...
class ParameterValueFile(_ParameterValue):
    value_file = SubjectRelation(
        "File", cardinality="?*", inlined=True, __permissions__=rel_perms_of_subject
    )
    param_def = SubjectRelation(
        "ParameterDefinition",
        cardinality="1*",
