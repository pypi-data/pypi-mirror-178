...
def get_main_parser(resources, action=None, target=None):
    ...
    login_parser.add_argument('endpoint', type=str, help='metadata endpoint, http[s]://datahub.*.com')
    ...
    if action == 'describe' or action is None:
        ...
        if action is None:
            ...
            describe_parser.add_argument('-o', '--output', choices=('json', 'yaml'), action='store', dest='output',
                                     help='output format')
...
def _main():
    ...
    if main_args.action == 'login':
        ...
        if main_args.statement:
            ...
            print(do_eval(main_args.statement, main_args.workspace))
        elif main_args.command:
            from metadata.utils.ipython import do_exec
            do_exec(main_args.command, main_args.workspace)
        else:
            from metadata.utils.ipython import main as ipython_main
            ipython_main(main_args.workspace)
