...
def set_user_context(name, endpoint, token, project, user_email, *, bohrium_user, bohrium_password, bohrium_project):
    config = load_config()
    ...
    config.set(context_section_name, 'bohrium_project', bohrium_project)
...
def get_context_from_conf(conf):
    ...
    bohrium_project = conf.get('bohrium_project')
    ...
    bohrium.config["username"] = bohrium_user
    bohrium.config["password"] = bohrium_password
    bohrium.config["project_id"] = bohrium_project
    s3_config["repo_key"] = "oss-bohrium"
    storage_client = TiefblueClient()
    s3_config["storage_client"] = storage_client
