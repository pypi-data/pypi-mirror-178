...
def vectorField(par, location, isNode = False, nodeNumber = -1):
    ...
    if isinstance(location, int):
        if shapes[location].shape == 'point':
            ...
            return(vectorField(par, np.array(shapes[location].position), True, nodeNumber))
        ...
        if isNode:
            ...
            for element in neiElements:
                ...
                A = abs(area(x[tri[element][0]], y[tri[element][0]], x[tri[element][1]], y[tri[element][1]], x[tri[element][2]], y[tri[element][2]]))
                Ex = (-1/(2*A))*((y[tri[element][1]]-y[tri[element][2]])*u[tri[element][0]]+(y[tri[element][2]]-y[tri[element][0]])*u[tri[element][1]]+(y[tri[element][0]]-y[tri[element][1]])*u[tri[element][2]])                
                Ey = (-1/(2*A))*((x[tri[element][2]]-x[tri[element][1]])*u[tri[element][0]]+(x[tri[element][0]]-x[tri[element][2]])*u[tri[element][1]]+(x[tri[element][1]]-x[tri[element][0]])*u[tri[element][2]])                
                elementfield = [Ex, Ey]
            ...
            for j in range(noOfRows):
                ...
                if isInsideGlobalBoundary:
                    ...
                    # X = np.linalg.lstsq(A, b)[0]
                    # field[j] = [-X[1][0], -X[2][0]]
                    A = abs(area(x[tri[theElement][0]], y[tri[theElement][0]], x[tri[theElement][1]], y[tri[theElement][1]], x[tri[theElement][2]], y[tri[theElement][2]]))
                    Ex = (-1/(2*A))*((y[tri[theElement][1]]-y[tri[theElement][2]])*u[tri[theElement][0]]+(y[tri[theElement][2]]-y[tri[theElement][0]])*u[tri[theElement][1]]+(y[tri[theElement][0]]-y[tri[theElement][1]])*u[tri[theElement][2]])                
                    Ey = (-1/(2*A))*((x[tri[theElement][2]]-x[tri[theElement][1]])*u[tri[theElement][0]]+(x[tri[theElement][0]]-x[tri[theElement][2]])*u[tri[theElement][1]]+(x[tri[theElement][1]]-x[tri[theElement][0]])*u[tri[theElement][2]])                
                    field[j] = [Ex, Ey]
...
def force(par, shapeNumber):
    ...
    nodeForce = []
    for node in nodes:
        nodeVectorField = vectorField(par, np.array([x[node], y[node]]), True, node)
