...
from typing import List, Optional, overload, Union, NamedTuple, Tuple
...
class DataApp:
    ...
    def _check_dataApp(self):
        ...
        if output.returncode!=0:
            print(f"DataApp {self.name} has been registered but may not work as expected. Please check errorLog_register.txt \n")
        else:
            print(f"DataApp {self.name} has been registered succesfully. \n")
        
    def _print_url(self, dataApp_svc, response: str):
        """
        Print an URL to the DataApp just been registered.
        """
    ...
    def selector(self,
        ...
        return Selector
        """
        return SelectorWidget(options, title, placeholder, label_by, value_by, default, allow_multi_selection,
                              parent_data_app=self, **additional)
    ...
    def radio_group(self,
        ...
        return RadioGroup
        """
        return RadioGroupWidget(options, title, label_by, value_by, value, allow_multi_selection,
                                parent_data_app=self, **additional)
