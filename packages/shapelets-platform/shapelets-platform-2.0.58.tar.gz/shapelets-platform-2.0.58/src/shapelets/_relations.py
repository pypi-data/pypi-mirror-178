...
class SandBox:
    ...
    def _rewrite_col(self, idx: int, col: str) -> dict:
        r""" 
        It takes idx (index) and col (colname) as params and returns a dictionary item when a colon char (':') is found in col:
            index: idx
            value: col but replacing (':') by ('_')
        If colon char (':') is not found in col, it returns None
        """
        tuple = col.split(':')
        item = (idx, tuple[0] + '_' + tuple[1]) if len(tuple)>1 else None
        return item
    ...
    def map(self, *genExpr: types.GeneratorType) -> DataSet:
        query = mustang.make_query(genExpr, frame_depth=1, left_join=False)
        sql = mustang.construct_sql_from_query(query)
        ds = DataSet(self.__factory.relation_from_query(sql), self.__map_fn)
       
        col_dict = [self._rewrite_col(idx,c) for idx, c in enumerate(ds.columns)]
        col_dict = dict(filter(lambda item: item is not None, col_dict))
        
        return ds.rename_columns(col_dict)
