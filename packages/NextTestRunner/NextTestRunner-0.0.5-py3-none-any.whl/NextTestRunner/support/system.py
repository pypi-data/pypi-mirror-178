...
class StFile:
    ...
    def copy_all(self,src_path, target_path):
        """
    ...
    def dir_copy(self,src_path, target_path):
        """copy
        all
        files
        of
    ...
    def del_type_file(self,path, filename):
        """
...
class PDFObject:
    ...
    def __init__(self,path=None):
        ...
        self.PDFTOPPMPATH = os.path.join(self.BASE_DIR, 'default/conf/tanktools/bin/pdfImg.exe').replace('\\', '/')
    ...
    def png(self, pdf_file, path):

        # 获取错误信息
        subprocess.Popen('"%s" -png "%s" "%s"\image' % (self.PDFTOPPMPATH, pdf_file, path)).communicate()[0]

    def jpeg(self, pdf_file, path):
        subprocess.Popen('"%s" -jpeg "%s" "%s"\image' % (self.PDFTOPPMPATH, pdf_file, path)).communicate()[0]

    # def tiff(self,pdf_file,path):
    #
    #     subprocess.Popen('"%s" -tiff "%s" "%s"\math_hw' % (self.PDFTOPPMPATH, pdf_file,path)).communicate()[0]
    #
    #
    # def svg(self,pdf_file,path):
    #
    #     # 获取错误信息
    #     p = subprocess.Popen('"%s" -svg "%s" "%s"\math_hw' % (self.PDFTOPPMPATH, pdf_file,path)).communicate()[0]
...
#   -png : 生成一个 PNG 文件
#   -jpeg : 生成 JPEG 文件
#   -jpegopt <string> : jpeg 选项，格式为 <opt1>=<val1>[,<optN>=<valN>]*
#   -tiff : 生成一个 TIFF 文件
#   -tiffcompression <string>: 设置 TIFF 压缩：无、packbits、jpeg、lzw、deflate
#   -ps : 生成 PostScript 文件
#   -eps : 生成封装的 PostScript (EPS)
#   -pdf : 生成PDF文件
#   -svg ：生成可缩放矢量图形 (SVG) 文件
#   -print : 打印到 Windows 打印机
#   -printdlg : 显示 Windows 打印对话框并打印
#   -printer <string> : 打印机名称或如果未指定此选项则使用默认值
#   -printopt <string> : 打印机选项，格式为 <opt1>=<val1>[,<optN>=<valN>]*
#   -setupdlg：打印前显示打印机设置对话框
#   -f <int> : 要打印的第一页
#   -l <int> : 最后一页打印
#   -o : 只打印奇数页
#   -e : 只打印偶数页
#   -singlefile : 只写第一页，不添加数字
#   -r <fp> : 分辨率，以 PPI 为单位（默认为 150）
#   -rx <fp> : X 分辨率，以 PPI 为单位（默认为 150）
#   -ry <fp> : Y 分辨率，以 PPI 为单位（默认为 150）
#   -scale-to <int> ：缩放每个页面以适应缩放到*缩放到像素框
#   -scale-to-x <int> ：水平缩放每个页面以适应缩放到 x 像素
#   -scale-to-y <int> ：垂直缩放每个页面以适应缩放到 y 的像素
#   -x <int> : 裁剪区域左上角的 x 坐标
#   -y <int> : 裁剪区域左上角的 y 坐标
#   -W <int> ：以像素为单位的裁剪区域宽度（默认为 0）
#   -H <int> : 裁剪区域的高度（以像素为单位）（默认为 0）
#   -sz <int> : 裁剪方块的像素大小（设置 W 和 H）
#   -cropbox : 使用裁剪框而不是媒体框
#   -mono : 生成单色图像文件（PNG、JPEG）
#   -gray : 生成灰度图像文件（PNG、JPEG）
#   -transp : 使用透明背景而不是白色 (PNG)
#   -antialias <string> : 设置 cairo antialias 选项
#   -icc <string> : 要使用的 ICC 颜色配置文件
#   -level2 : 生成 Level 2 PostScript (PS, EPS)
#   -level3 : 生成 Level 3 PostScript (PS, EPS)
#   -origpagesizes ：保留原始页面大小（PS、PDF、SVG）
#   -paper <string> : 纸张尺寸（letter、legal、A4、A3、匹配）
#   -paperw <int> ：纸张宽度，以磅为单位
#   -paperh <int> ：纸张高度，以磅为单位
#   -nocrop : 不要将页面裁剪为 CropBox
#   -expand : 展开小于纸张尺寸的页面
#   -noshrink : 不要收缩大于纸张尺寸的页面
#   -nocenter ：不要将小于纸张尺寸的页面居中
#   -duplex : 启用双面打印
#   -opw <string> ：所有者密码（用于加密文件）
#   -upw <string> : 用户密码（用于加密文件）
#   -q : 不打印任何消息或错误
