...
class FindMethod:
    def __init__(self,_json, path="") -> None:
        self.json = _json

        path = "" if not path else path

        try:
            exec("global ____; ____ = self.json{0}".format(path))
            global ____
            self.json = ____
        except Exception as e:
            raise KeyError("{}".format(e))
        ...
        if isinstance(val,dict):
            for k,v in val.items():
                if isinstance(v,dict):
                    ...
                    if all(isinstance(i, int) for i in v) or all(isinstance(i, str) for i in v):
                        yield pre+[v]
                    else:
                        for i in v:
                            yield pre+[i]
...
class Exist:
    def __init__(self, _json, obj, path = None) -> None:
        self.obj = obj
        self.json = _json
    ...
    def func(self,obj):
        if isinstance(obj,type(self.obj)):
            return True if obj == self.obj else False
        else:
            return False
    ...
    def find(self) -> bool:
        ...
        for i in self.Walk_(self.json):
            ...
            if func(i[-1]) and isinstance(func(i[-1]),bool):
                ...
                if not isinstance(func(i[-1]),bool):
                    raise ValueError("The function dont return a boolean")
        return True if _.__len__() > 0 else False