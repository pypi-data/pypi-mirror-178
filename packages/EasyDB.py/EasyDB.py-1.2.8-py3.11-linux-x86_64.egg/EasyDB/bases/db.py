...
from EasyDB.bases import DataBase_Functions, FindMethod, Exist
class DB:
    ...
    def exist(self,obj, path=None) -> bool:
        return Exist(self.json, obj, self.getPath(path) if path else None).find()
    ...
    def filter(self,func, path=""):
        path = self.getPath(path) if not path == "" else path
        return FindMethod(self.json,path).find(func)

    def find(self,func, path=""):
        f_ = self.filter(func,path)
        return f_[0] if f_.__len__() > 0 else None
