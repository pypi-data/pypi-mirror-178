class TesoroError(Exception):
    """Base exception class for tesoro errors."""


class ApiError(TesoroError):
    """Base exception class for Tesoro API errors. API errors will result
    ...
    def _exception_for_error_type(cls, error_type, errors=[]):
        ...
        elif error_type == 'tesoro':
            return TesoroInternalError
        else:
            return None
    ...
    def exception_for(cls, status, error_type, errors=[]):
        ...
        if exception is not None:
            return exception
        raise TesoroError('Invalid error type "{}"'.format(error_type))
...
class TesoroInternalError(ApiError):
    pass
...
class MalformedResponseError(TesoroError):
...
class InvalidSignatureError(TesoroError):
    pass
