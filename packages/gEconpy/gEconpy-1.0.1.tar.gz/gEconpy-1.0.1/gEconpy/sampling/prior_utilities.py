...
from gEconpy.estimation.estimate import build_Z_matrix, build_Q_and_H, split_param_dict
...
def prior_solvability_check(model, n_samples, seed=None, param_subset=None, pert_solver='cycle_reduction'):
    data = pd.DataFrame(model.sample_param_dict_from_prior(n_samples, seed, param_subset, sample_shock_sigma=True))
...
def kalman_filter_from_prior(model, data, n_samples, filter_type='univariate'):
    ...
    results = []
    param_dicts = pd.DataFrame(model.sample_param_dict_from_prior(n_samples, sample_shock_sigma=True)).T.to_dict()
