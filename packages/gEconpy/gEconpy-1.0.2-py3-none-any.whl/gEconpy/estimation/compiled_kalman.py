...
cc = CC('compiled_kalman_filters')


@cc.export('build_mask_matrix', 'float64[:, ::1](boolean[::1])')
@njit
...
@cc.export('standard_kalman_filter',
...
@cc.export('kalman_step', 'Tuple((float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[::1]))'
...
@cc.export('filter_step', 'Tuple((float64[:, ::1], float64[:, ::1], float64[::1]))'
                          '(float64[::1], float64[:, ::1], float64[:, ::1], float64[::1], float64[:, ::1])')
@njit
def filter_step(y, Z, H, a, P):
    v = np.atleast_2d(y - Z @ a).reshape((-1, 1))
    ...
    inner_term = linalg.solve_triangular(F_chol, linalg.solve_triangular(F_chol, v, lower=True),
                                         lower=True, trans=1)
    n = y.shape[0]
...
@cc.export('predict', 'Tuple((float64[::1], float64[:, ::1]))'
                      '(float64[::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[:, ::1])')
@njit
...
@cc.export('univariate_kalman_filter',
           'Tuple((float64[:, ::1], float64[:, ::1], float64[:, :, ::1], float64[:, :, ::1], float64[::1]))'
           '(float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], '
           'float64[:, ::1], float64[:, ::1])')
@njit
def univariate_kalman_filter(data: ArrayLike,
    ...
    for i in range(n_steps):
        a_filtered, a_hat, P_filtered, P_hat, ll = univariate_kalman_step(data[i].copy(), a, P, T, Z, R, H, Q)
...
@cc.export('univariate_kalman_step', 'Tuple((float64[:, ::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], float64))'
                                     '(float64[::1], float64[::1], float64[:, ::1], float64[:, ::1], float64[:, ::1], '
                                     'float64[:, ::1], float64[:, ::1], float64[:, ::1])')
@njit
...
@cc.export('univariate_filter_step', 'Tuple((float64[:, ::1], float64[:, ::1], float64))'
...
def univariate_filter_step(y_masked, Z_masked, H_masked, a, P):
    ...
    a_filtered = np.atleast_2d(a.copy()).reshape((-1, 1))
    P_filtered = P.copy()
    ll_row = np.zeros(n_states)

    for i in range(n_states):
        a_filtered, P_filtered, ll = univariate_inner_step(y_masked[i], Z_masked[i, :], H_masked[i, i],
...
@cc.export('univariate_inner_step', 'Tuple((float64[:, ::1], float64[:, ::1], float64[::1]))'
...
@cc.export('make_initial_conditions', 'Tuple((float64[:, ::1], float64[:, ::1]))(float64[:, ::1], float64[:, ::1], '
                                      'float64[:, ::1], optional(float64[:, ::1]), optional(float64[:, ::1]))')
@njit
