...
def standard_kalman_filter(data: ArrayLike,
    ...
    filtered_states = pt.zeros((n_steps, k_states))
    predicted_states = pt.zeros((n_steps + 1, k_states))
    filtered_cov = pt.zeros((n_steps, k_states, k_states))
    predicted_cov = pt.zeros((n_steps + 1, k_states, k_states))
    log_likelihood = pt.zeros(n_steps)
...
def filter_step(y, Z, H, a, P):
    ...
    inner_term = linalg.solve_triangular(F_chol, linalg.solve_triangular(F_chol, v, lower=True),
                                         lower=True, trans=1)
    n = y.shape[0]
    ll = -0.5 * (n * MVN_CONST + (v.T @ inner_term).ravel()) - pt.log(pt.diag(F_chol)).sum()
...
def univariate_kalman_filter(data: ArrayLike,
    ...
    for i in range(n_steps):
        a_filtered, a_hat, P_filtered, P_hat, ll = univariate_kalman_step(data[i], a, P, T, Z, R, H, Q)
...
def univariate_inner_step(y, Z_row, sigma_H, a, P):
    ...
    K = PZT / F
    a_filtered = a + K * v
    P_filtered = P - pt.outer(K.squeeze(), K.squeeze()) * F
