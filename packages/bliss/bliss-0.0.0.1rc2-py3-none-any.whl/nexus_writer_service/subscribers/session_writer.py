...
class NexusSessionWriter(base_subscriber.BaseSubscriber):
    ...
    def _event_loop_initialize(self, **kwargs):
        ...
        self.logger.info("Session writer started with options %s", self.saveoptions)
        self._fds = {}
    ...
    def _event_loop_finalize(self, **kwargs):
        ...
        try:
            ...
            self.logger.exception("Exception while stopping scan writers")
        finally:
    ...
    def _event_new_node(self, node):
        ...
        name = repr(node.name)
        if node.type == "scan":
            self.logger.debug("Start writer for scan %s", name)
            self._event_start_scan(node)
        elif node.type == "scan_group":
            self.logger.debug("Start writer for group scan %s", name)
            self._event_start_scan(node)
        else:
    ...
    def purge_scan_writers(self, delay=True):
        ...
        for name, writer in list(self.writers.items()):
            if writer.done(delay):
                self.logger.info("Purge scan writer %s", writer)
    ...
    def stop_scan_writers(self, kill=False):
        ...
        if self.writers:
            ...
            if greenlets:
                ...
                self.logger.info("%s scan writers %s ...", action, lst)
            else:
                self.logger.info("No scan writers to %s", action)
    ...
    def log_progress(self, msg=None):
        ...
        nactive = sum(w.active for w in list(self.writers.values()))
        if msg:
            self.logger.info("%s (%d scan writers, %d active)", msg, n, nactive)
        else:
            self.logger.info("%d scan writers (%d active)", n, nactive)
    ...
    def scan_has_required_disk_space(self, name: str) -> bool:
        ...
        warnings.warn("Use scan_disk_space_error instead", FutureWarning)
    ...
    def scan_disk_space_error(self, name: str) -> str:
        ...
        if writer is None:
            ...
            return writer.disk_space_error
    ...
    def scan_disk_space_warning(self, name: str) -> str:
        ...
        if writer is None:
            ...
            return writer.disk_space_warning
