...
class NexusScanWriterBase(base_subscriber.BaseSubscriber):
    ...
    def _event_loop_initialize(self, **kwargs):
        ...
        self.logger.info(
            "Start writing to '%s' with options (%s) %s",
            self.filename,
    ...
    def _finalize_hdf5(self):
        ...
        if self._exception_is_fatal:
            ...
            self.logger.info("Finalize writing to %s", self.filename)
        ...
        self.logger.info("Save dataset metadata")
        skip = {None}
        for node in self._nodes:
            with self._capture_finalize_exceptions():
                self._fetch_node_metadata(node, skip)

        self.logger.info("Save device metadata")
        for subscan in self._enabled_subscans:
            self._fetch_device_metadata(subscan)

        self.logger.info("Finalize saving")
        for subscan in self._enabled_subscans:
            try:
    ...
    def _capture_finalize_exceptions(self):
        try:
            ...
            self.logger.exception("Captured finalization exception")
    ...
    def writer_options(self):
        return self.get_info("writer_options", cache=True)

    @property
    def chunk_options(self):
        writer_options = self.writer_options
        if writer_options:
            return writer_options.get("chunk_options", dict())
        else:
            return dict()
    ...
    def device(self, subscan, node) -> Optional[Dict]:
        """
    ...
    def device_parent_context(
        self, subscan: Subscan, device: Dict
    ) -> Iterator[Optional[h5py.Group]]:
        """
        Get device information of the node belonging to the subscan
    ...
    @property
    def recommended_disk_space(self):
        """In MB"""
        return self.saveoptions["recommended_disk_space"]
    ...
    def disk_space_error(self) -> str:
        ...
        return self._required_disk_space_msg(self.required_disk_space)

    @property
    def disk_space_warning(self) -> str:
        """Returns an warning message when the disk space is running low."""
        return self._required_disk_space_msg(self.recommended_disk_space)

    def _required_disk_space_msg(self, required_disk_space) -> str:
        path = self.filename
        if not path:
            return ""
    ...
    def _on_subscan_creation(self, subscan):
        ...
        if subscan.enabled:
            self.logger.info("Start writing subscan %s to %s", name, uri)
        else:
    ...
    def _h5missing(self, variable):
        ...
        :param str variable:
        """
        self.logger.debug("HDF5 group not created yet ('%s' missing)", variable)
    ...
    def _mark_running(self, subscan):
        """
        The NXentry is running

        :param Subscan subscan:
        """
        self._update_subscan_status(subscan, "RUNNING")
    ...
    def _mark_done(self, subscan):
        ...
        if self.state == self.STATES.FAULT:
            ...
            status = "SUCCEEDED"
        self._update_subscan_status(subscan, status)

    def _update_subscan_status(self, subscan, status: str):
        """
        The NXentry is completed
        ...
        with self.nxentry(subscan) as nxentry:
            if nxentry is None:
                ...
                yield nexus.nxInstrument(
                    nxentry, "instrument", nxdict=self.instrument_info
                )
    ...
    def _initialize_subscan(self, subscan):
        """
    ...
    def log_progress(self, msg=None):
        ...
        if msg:
            self.logger.info("%s (%s %s %s)", msg, progress, data, duration)
        else:
            self.logger.info(" %s %s %s", progress, data, duration)
    ...
    def _process_event(self, event_type, node, event_data):
        ...
        if event_type == event_type.NEW_NODE:
            ...
            self.logger.debug("Untreated event: %s", event_info)
    ...
    def _event_new_node(self, node):
        ...
        if node.type in ["scan", "scan_group"]:
            self.logger.debug("Start scan %s", name)
            self._event_start_scan(node)
        elif node.type == "channel":
            self.logger.debug("New channel %s", name)
            self._event_new_datanode(node)
        elif node.type == "node_ref_channel":
            self.logger.debug("New reference channel %s", name)
            self._event_new_datanode(node)
        elif node.type == "lima":
            self.logger.debug("New Lima %s", name)
            self._event_new_datanode(node)
        elif node.parent.type in ["scan", "scan_group"]:
            self.logger.debug("New subscan %s", name)
            self._event_new_subscan(node)
        else:
            self.logger.debug("New '%s' node event on %s not treated", node.type, name)
    ...
    def _event_new_data(self, node, event_data):
        ...
        if node.type == "channel":
            ...
            self.logger.warning("New '%s' data for '%s' not treated", node.type, name)
    ...
    def _datanode_subscan(self, node):
        ...
        for subscan in list(self._subscans):
            ...
            self.logger.warning(
                "data node '%s' does not belong to any of the subscans %s",
                node.fullname,
                subscans,
            )
            return None
        if not subscan.enabled:
            self.logger.warning(
                "Writing of subscan '%s' is disabled (probably a concurrent writer)",
                subscan.name,
            )
            return None
        if subscan.name not in self._expected_subscans:
    ...
    def scan_ndim(self, subscan):
        """
        Number of dimensions of the subscan (0 means a scalar scan but currently
        no scan has this)
    ...
    def _node_ignore_data(self, node):
        ...
        # NEW_DATA events, they will be accepted regardless.
        if node.type == "lima":
    ...
    def _node_expect_data(self, node):
        ...
        :returns bool:
        """
        # TODO: this is currently only used to log warnings (Bliss doesn't see those)
    ...
    def _fetch_new_data(self, node, nproxy, event_data=None):
        ...
        if node.type == "channel":
            ...
            self.logger.warning(
                "data is not fetched from unknown node type '%s'", node.type
            )
    ...
    def _detector_data_info(self, node):
        ...
        if detector_shape is not None:
            # Cannot handle zeros in the detector for lima
            # because we are not allowed to check the actual
            # lima data, we only have references.
            # We can handle zeros for other detectors
            # because we have the data in memory.
            if node.type != "lima" or all(detector_shape):
    ...
    def _fetch_node_metadata(self, node, skip):
        ...
        if not dproxy.exists:
            return
        metadata = {"units": node.info.get("unit")}
        dproxy.add_metadata(metadata, parent=False, create=False)

    def _fetch_device_metadata(self, subscan):
        """
        Get all device metadata and save it under the appropriate NXdetector or NXpositioner groups.

        :param Subscan subscan:
        """
        channels = self.get_info("channels")
        devices = self.get_info("devices")
        visited = set()
        for fullname, device in self.devices[subscan.name].items():
            ...
            with self.device_parent_context(subscan, device) as parent:
                ...
                if not parent.name.endswith("measurement"):
                    dictdump.dicttonx(metadata, parent, update_mode="add")
    ...
    def _fetch_new_redis_refdata(self, rproxy, node, event_data=None):
        ...
        for refnode in nodes:
            if refnode is None:
                self.logger.warning(
                    "Node reference in '%s' is `None`. Check Redis memory as keys might be evicted",
                    node.name,
                )
            elif refnode.info.get("save"):
    ...
    def _fetch_subscan_metadata(self, subscan):
        ...
        if metadict:
            ...
            metadict.pop("positioners", None)
            metadict.pop("SamplePositioners_value", None)
            metadict.pop("SamplePositioners_name", None)
    ...
    def _subscan_dicttonx(self, subscan, nxtreedict):
        ...
        with self.nxentry(subscan) as parent:
            ...
            try:
                ...
                self._set_state(self.STATES.FAULT, e)
                subscan.logger.exception(
                    "Scan metadata cannot be saved:\n%s", pformat(nxtreedict)
                )
            else:
                subscan.logger.info(
                    "Updated metadata groups: %s", list(nxtreedict.keys())
                )
