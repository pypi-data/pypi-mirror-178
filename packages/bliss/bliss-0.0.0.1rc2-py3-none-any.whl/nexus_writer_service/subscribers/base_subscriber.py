...
class BaseSubscriber:
    ...
    def _cleanup_action(self, action):
        ...
        try:
            ...
            self.logger.exception("Exception while %s", action)
    ...
    def __greenlet_main(self):
        ...
        try:
            ...
            self.logger.exception("Stop listening to Redis events due to exception")
            raise
        finally:
    ...
    def _listen_event_loop(self, **kwargs):
        ...
        try:
                ...
                if event_type == event_type.END_SCAN:
                    ...
                    try:
                        ...
                        self.logger.exception(
                            "Processing '%s' event caused an exception", event_type.name
                        )
            ...
            try:
                ...
                self._set_state(self.STATES.FAULT, e)
                self.logger.exception("Not properly finalized due to exception")
    ...
    def _event_new_node(self, node):
        ...
        """
        self.logger.info("New node '%s' (type: %s)", node.db_name, node.type)
    ...
    def log_progress(self, msg=None):
        ...
        if msg:
            self.logger.info("%s (%s %s)", msg, progress, duration)
        else:
            self.logger.info(" %s %s", progress, duration)
