...
from bliss.common.utils import rounder, shorten_signature, typecheck
...
def ascan(
    ...
    *counter_args: _countables,
    ...
    Keyword Args:
        ...
        stab_time (float): sleep time after each move [default: None]
...
def dscan(
    ...
    *counter_args: _countables,
    ...
    Keyword Args:
        ...
        save (bool): save scan data to file [default: True]
        sleep_time (float): sleep time between 2 points [default: None]
        stab_time (float): sleep before each count [default: None]
        run (bool): if True (default), run the scan. False means just create
                    scan object and acquisition chain
        return_scan (bool): True by default
    """
...
def lookupscan(
    ...
    *counter_args: _countables,
    ...
    arguments:
        motor_pos_tuple_list: a list of tuples of the following type: (motor, positions).
            Positions can be provided as a numpy array, a list or a tuple.
            ex: [ (m1, [1,2,3]) , (m2, [8,7,6]) ]
        count_time: count time in seconds
        *counter_args: as many counter-providing objects as used in
                       the scan (seperated by comma)
    ...
    chain = DEFAULT_CHAIN.get(
        scan_params,
        counter_args,
        top_master=VariableStepTriggerMaster(*motors_positions, stab_time=stab_time),
    )
