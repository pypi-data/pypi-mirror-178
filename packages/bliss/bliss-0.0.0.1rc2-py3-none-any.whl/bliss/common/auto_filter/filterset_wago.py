...
class FilterSet_Wago(FilterSet):
    ...
    def set_filter(self, filter_id):
        ...
        if filter_id not in self._fpattern:
            raise ValueError(
    ...
    def build_filterset(self, short_list=[]):
        ...
        p = [0]
        t = [1.0]
        self._built_filters = [{"position": 0, "transmission_calc": 1.0}]

        # if get a short list, remove the mask with do disabled filters
        filt_ids = list(range(1, self._filtmask + 1))
        filt_ids_temp = list(filt_ids)
        if len(short_list) != 0:
            for f in range(self._config_nb_filters):
                if f not in short_list:
                    exc = 1 << f
                    for idx in filt_ids_temp:
                        if (idx & exc) and idx in filt_ids:
                            filt_ids.remove(idx)

        nbits = self._config_nb_filters

        for filt_id in filt_ids:
