...
class FilterSet(CounterContainer, HasMetadataForScan):
    ...
    def __init__(self, name, config):
        ...
        self.__short_list = QueueSetting(f"{self.name}:short_list")
        sl = list(self.__short_list)
        self._filter_positions = [f["position"] for f in self._config_filters]
        if len(sl) == 0:
            self.__short_list.set(self._filter_positions)
    ...
    def enabled_filters(self, sl):
        if len(sl):
            ...
            sl = self._filter_positions
        if sl != list(self.__short_list):
            self.__short_list.set(sl)

            # now reinitialize the filterset to get new list of filters and absorption table
            self.initialize(sl)

    def initialize(self, short_list=[]):
        """
        ...
        self._nb_filters = self.build_filterset(short_list)
    ...
    def build_filterset(self, short_list=[]):
        """
...
class FilterSetCounterController(SamplingCounterController):
    ...
    def __init__(self, filterset):
        ...
        self.tags = ["filteridx", "transmission"]
...
class FilterSetCounter(SamplingCounter):
    def __init__(self, name, config, controller):
        ...
        SamplingCounter.__init__(self, name, controller, mode=SamplingMode.SINGLE)
