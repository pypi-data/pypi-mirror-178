...
def get_scan_entries(filename, timeout=3):
    if not filename or not os.path.isfile(filename):
        return []
    try:
        # Bypass HDF5 file locking without changing the process environment
        fh = open(filename, mode="rb")
    except IOError as e:
        raise RuntimeError(f"cannot get the scan names from '{filename}'") from e
    try:
        local_timeout = None
        exception = None
        try:
            with gevent.Timeout(timeout) as local_timeout:
                while True:
                    try:
                        with h5py.File(fh, mode="r") as h5f:
                            return list(h5f["/"])
                    except Exception as e:
                        # Scans are being added or file is corrupt
                        exception = e
                        gevent.sleep(0.1)
            ...
            if local_timeout is not raised_timeout:
                raise
            raise RuntimeError(
                f"cannot get the scan names from '{filename}'"
            ) from exception
    finally:
        fh.close()
...
class Writer(FileWriter):
    ...
    def finalize_scan_entry(self, scan):
        ...
        self.file.create_group(f"{scan_name}/instrument")
        ...
        for dev in scan.acq_chain.nodes_list:
            ...
            if not dev.name:
                continue
            device_key = scan_info._get_key_from_acq_obj(dev)
            device_metadata = scan_info["devices"][device_key].get("metadata")
