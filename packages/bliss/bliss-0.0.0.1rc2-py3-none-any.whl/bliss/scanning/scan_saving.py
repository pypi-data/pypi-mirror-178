...
class ESRFScanSaving(BasicScanSaving):
    ...
    def icat_register_dataset(
        self, dataset_name_or_id: Union[DatasetId, str], raise_on_error=True
    ):
        proposal = self.proposal
        node = proposal.get_dataset_node(dataset_name_or_id)
        if node is None:
            msg = f"dataset '{dataset_name_or_id}' does not exist in Redis"
            if raise_on_error:
                raise RuntimeError(msg)
            else:
                proposal._log_debug(msg)
                return
    ...
    def _close_proposal(self) -> bool:
        """Close the current proposal."""
        if self._proposal:
            if not self._final_icat_confirm_datasets():
                return False
            # Metadata of unregistered datasets will be lost:
    ...
    def _final_icat_confirm_datasets(self, timeout=10) -> bool:
        ...
        while (time.time() - t0) < timeout:
            _, unconfirmed = self.icat_confirm_datasets(timeout=timeout)
            if not unconfirmed:
                logtools.user_print("All datasets are registered with ICAT")
                return True
            gevent.sleep(0.5)

        warn_msg = f"The current proposal has {len(unconfirmed)} datasets that are not registered with ICAT"

        if not is_bliss_shell():
            logtools.user_warning(warn_msg)
            return True
        ...
        dlg = UserYesNo(label="\n".join(text))
        return bool(display(dlg, title=warn_msg))
