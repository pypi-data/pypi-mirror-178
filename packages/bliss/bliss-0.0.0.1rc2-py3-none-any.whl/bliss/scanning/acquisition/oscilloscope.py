...
class OscilloscopeAcquisitionSlave(BaseCounterAcquisitionSlave):
    ...
    def _do_add_counter(self, counter):
        ...
        # add additional channel for Time
        if isinstance(counter, OscilloscopeAnalogChannel):
            self.channels.append(
                AcquisitionChannel(
                    f"{counter.fullname}_time", counter.dtype, counter.shape, unit=None
                )
            )
            self._counters[counter].append(self.channels[-1])

    def prepare(self):
        self.device._scope._device.acq_prepare(self.count_time)
    ...
    def reading(self):
        ...
        for counter, channels in self._counters.items():
            if isinstance(counter, OscilloscopeAnalogChannel):
                reading = self.device._scope._device.acq_read_channel(
                    counter.name, self.count_time
                )
                data[
                ...
                data[self.channels.index(channels[2])] = reading.time_data
