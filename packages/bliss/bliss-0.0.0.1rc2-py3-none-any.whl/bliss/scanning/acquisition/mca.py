...
class McaAcquisitionSlave(AcquisitionSlave):
    ...
    def _init_datas(self):
        self._datas = dict()
        self._chan_names = list()
        for counter in self._counters:
            channame = f"{self.name}:{counter.name}"
            self._datas[channame] = list()
            self._chan_names.append(channame)
    ...
    def _append_datas(self, spectrums, stats):
        ...
        for counter, name in zip(self._counters, self._chan_names):
            point = counter.feed_point(spectrums, stats)
            self._datas[name].append(point)
    ...
    def _publish_datas(self):
        ...
        try:
            self.emit_progress_signal(
                {"last_pixel_triggered": self.device.last_pixel_triggered}
            )
