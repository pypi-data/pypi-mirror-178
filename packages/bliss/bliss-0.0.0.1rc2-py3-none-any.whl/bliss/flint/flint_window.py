...
class FlintWindow(qt.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.__customPlots: Dict[object, Tuple[CustomPlot, Optional[CustomPlot]]] = {}
    ...
    def createCustomPlot(
        ...
        if parentId:
            customParentPlot, _customParentParentPlot = self.__customPlots[parentId]
            w = customParentPlot.widget()
            ...
            self.__customPlots[plot_id] = (customPlot, customParentPlot)
            return
        ...
        self.__customPlots[plot_id] = (customPlot, None)
        plotWidget.show()

    def removeCustomPlot(self, plot_id):
        """Remove a custom plot by its id"""
        customPlot, customParentPlot = self.__customPlots.pop(plot_id)

        if customPlot.isPlotContainer():
            for subPlotId in customPlot.subPlotIds():
                del self.__customPlots[subPlotId]

        if customParentPlot is not None:
            parent = customParentPlot.widget()
    ...
    def customPlot(self, plot_id) -> CustomPlot:
        ...
        customPlot, _parent = self.__customPlots.get(plot_id, (None, None))
        return customPlot
