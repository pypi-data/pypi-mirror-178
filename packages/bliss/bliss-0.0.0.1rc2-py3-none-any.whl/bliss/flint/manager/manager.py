...
class ManageMainBehaviours(qt.QObject):
    ...
    def setIcatClientConfig(self, config: dict):
        if config:
    ...
    def getCompatiblePlots(self, widget, availablePlots) -> List[plot_model.Plot]:
    ...
    def updateWidgetsWithPlots(
        self, scan: Optional[scan_model.Scan], plots, useDefaultPlot, defaultPlot
    ):
    ...
    def updateWidgetWithPlot(
        self, widget, scan: Optional[scan_model.Scan], plotModel, useDefaultPlot
    ):
        ...
        if isinstance(plotModel, plot_item_model.CurvePlot):
            ...
            if useDefaultPlot or previousPlotModel is None:
                ...
                if plotModel is not None and not model_helper.isAnyItemValid(plotModel):
                    channel = model_helper.getAnyDisplayableChannelAsCurve(scan)
                    if channel is not None:
                        model_helper.updateDisplayedChannelNames(
                            plotModel, scan, [channel.name()]
                        )
            else:
