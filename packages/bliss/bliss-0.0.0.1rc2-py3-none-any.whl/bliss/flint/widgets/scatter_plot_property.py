...
class _DataItem(_property_tree_helper.ScanRowItem):
    ...
    def __valueAxisChanged(self, item: qt.QStandardItem):
        if self.__plotItem is not None:
            ...
            scatter, wasUpdated = model_helper.createScatterItem(
                plotModel, self.__channel
            )
            if wasUpdated:
    ...
    def __xAxisChanged(self, item: qt.QStandardItem):
        ...
        if len(scatters) == 0:
            ...
            newItem = plot_item_model.ScatterItem(plotModel)
            newItem.setXChannel(plot_model.ChannelRef(plotModel, channelName))
    ...
    def __yAxisChanged(self, item: qt.QStandardItem):
        ...
        if len(scatters) == 0:
            # Create an item to store the y-value
            newItem = plot_item_model.ScatterItem(plotModel)
            newItem.setYChannel(plot_model.ChannelRef(plotModel, channelName))
            plotModel.addItem(newItem)
        else:
...
class ScatterPlotPropertyWidget(qt.QWidget):
    ...
    def __createToolBar(self):
        ...
        action.setIcon(icon)
        action.setText("Reset to scan description")
        action.setToolTip(
            "Reset the plot to the original description stored in this scan"
        )
    ...
    def __resetPlotWithOriginalPlot(self):
        ...
        if len(plots) == 0:
            _logger.warning("No scatter plot to display")
            qt.QMessageBox.warning(
                None, "Warning", "There was no scatter plot in this scan"
            )
            return
        plotModel = plots[0]
    ...
    def __loadScanFromHistory(self, nodeName: str):
        ...
        if widget is not None:
            ...
            # Reuse only available values
            if isinstance(previousWidgetPlot, plot_item_model.ScatterPlot):
    ...
    def __updateTree(self):
        ...
        model.setHorizontalHeaderLabels(
            ["Name", "X", "Y", "V", "Displayed", "Style", "Message"]
        )
