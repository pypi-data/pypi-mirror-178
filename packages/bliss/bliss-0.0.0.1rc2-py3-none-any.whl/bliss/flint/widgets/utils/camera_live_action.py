...
class _CustomEditorWidget(qt.QWidgetAction):
    ...
    def __init__(self, parent, scan):
        ...
        if isinstance(scan, monitoring.LimaVideoMonitoringScan):
            button = qt.QPushButton(widget)
            if scan.isLive():
                button.setText("Pause")
                button.clicked.connect(self.__stopVideoLive)
                iconName = qt.QStyle.SP_MediaPause
            else:
                button.setText("Live")
                button.clicked.connect(self.__startVideoLive)
        ...
        try:
            shutter_close_time = proxy.shutter_close_time
            duration_time += shutter_close_time
        except AttributeError:
            shutter_close_time = None
        ...
        def format_shutter(shutter_close_time):
            if shutter_close_time is None:
                return ""
            return f"<li>+ shutter close time: {shutter_close_time}s</li>"

        toolTip = (
            "<html>Time between 2 consecutive frames, in milliseconds."
            + "<p>Computed from:</p>"
            + "<ul>"
            + f"<li>exposure time: {exposure_time}s</li>"
            + format_shutter(shutter_close_time)
            + f"<li>+ readout + user latency time: {latency_time}s</li>"
            + "</ul>"
            + "</html>"
        )
        ...
        if duration_time != 0:
            ...
            _rate, normalized = integerize(rate * 10)
            if rate >= 0.05:
                about = "≈ " if normalized else ""
                toolTip = "Number of frames per seconds, in hertz"
        ...
        if isinstance(scan, monitoring.LimaVideoMonitoringScan):
            ...
            exposureValue.editingFinished.connect(self.__updateVideoExposure)
...
class CameraLiveAction(qt.QWidgetAction):
    ...
    def __feedMenu(self):
        ...
        menu.clear()
        scan = self.__scan
        editor = _CustomEditorWidget(self.defaultWidget(), scan)
