...
class AutoFocusOnNewScanAction(qt.QAction):
    """QAction controlling the auto focus mode from the actual session."""

    def __init__(self, parent, model: FlintState):
        super(AutoFocusOnNewScanAction, self).__init__(parent)
        self.setCheckable(True)
        self.triggered.connect(self._actionTriggered)
        self.__model = model
        self.setText("Auto focus on new scan")
        self.setToolTip(
            "If checked, on a new scan Flint will guess the main "
            "used widget and will put the focus on it."
        )

    def updateState(self):
        workspace = self.__model.workspace()
        self.setChecked(workspace.autoFocusOnNewScan())

    def _actionTriggered(self, checked=False):
        workspace = self.__model.workspace()
        workspace.setAutoFocusOnNewScan(self.isChecked())
