...
class PlotConfiguration:
    ...
    def __init__(self):
        ...
        self.use_data_margins: Optional[bool] = True
        # View
...
class FlintPlot(PlotWindow):
    ...
    def setDefaultDataMargins(self, *margins):
        self.__dataMargins = margins
...
class FlintImageDensityMap(ImageDataAggregated, FlintItemMixIn):
    def __init__(self):
    ...
    def getFlintTooltip(self, index, flintModel, scan: scan_model.Scan):
        ...
        text = f"""
        <li style="white-space:pre">{char} <b>Max: </b> {value}</li>
        <li style="white-space:pre">     <b>{xName}:</b> {x}</li>
        <li style="white-space:pre">     <b>{yName}:</b> {y}</li>
        """
        return x + 0.5, y + 0.5, text
...
class FlintImageRgba(ImageRgba, FlintItemMixIn):
    ...
    def getFlintTooltip(self, index, flintModel, scan: scan_model.Scan):
        ...
        image = self.getData(copy=False)
        value = image[index]
        char = self._getColoredChar(value, flintModel, color=value)
