...
class ShowDataMargins(PlotAction):
    """QAction controlling X axis log scale on a :class:`.PlotWidget`.
    ...
    def __init__(self, plot, parent, flintPlot):
        super(ShowDataMargins, self).__init__(
            plot,
            icon="selected",
            text="Use data margins",
            tooltip="Enable/disable data margin around the plot",
            triggered=self._actionTriggered,
            checkable=True,
            parent=parent,
        )
        self.flintPlot = weakref.ref(flintPlot)

    def update(self):
        plot = self.plot
        self.setChecked(plot.isDataMarginsEnabled())

    def _actionTriggered(self, checked=False):
        plot = self.plot
        plot.setDataMarginsEnabled(checked)
        plot.resetZoom()
...
class CustomAxisAction(qt.QWidgetAction):
    def __init__(self, plot, parent, kind="any"):
        ...
        action = ShowDataMargins(plot, self, parent)
        menu.addAction(action)
        menu.aboutToShow.connect(action.update)
