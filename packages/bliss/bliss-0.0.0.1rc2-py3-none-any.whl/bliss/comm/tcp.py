...
class BaseSocket:
    ...
    def __init__(
        ...
        timeout=5.0,  # default timeout for read write
    ):
        self._host = host
        self._port = port
        self._socket = None
    ...
    def _readline(self, eol=None, timeout=-1):
        if timeout == -1:
            timeout = self._timeout
        timeout_errmsg = "timeout on socket(%s, %d)" % (self._host, self._port)
        with capture_exceptions() as capture:
            with gevent.Timeout(timeout, SocketTimeout(timeout_errmsg)):
...
class Command:
    ...
    def __init__(
        ...
        port,
        eol=b"\n",  # end of line for each rx message
        timeout=3.0,  # default timeout for read write
    ):
        self._host = host
        self._port = port
        self._socket = None
