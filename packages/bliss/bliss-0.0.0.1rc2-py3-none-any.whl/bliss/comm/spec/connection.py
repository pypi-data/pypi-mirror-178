    ...
    while True:
        ...
        try:
            ...
            with gevent.Timeout(1, SpecClientNotConnectedError):
                ...
                conn.socket = s
                conn.send_msg_hello()
                m = message.message.spec_message(version=None)
                m.readFromStream(conn.socket.recv(1024))
                if m.cmd == message.HELLO_REPLY:
...
def connectionHandler(conn, socket_to_spec):
    ...
    while True:
        ...
        while offset < len(s):
            if message is None:
                message = message.spec_message(version=serverVersion)
            ...
            if message.isComplete():
                try:
                    try:
                        ...
                        elif message.cmd == message.EVENT:
                            try:
                                ...
                                channel.update(
                                    message.data, message.flags == message.DELETED
                                )
                    except BaseException:
...
class SpecConnection:
    ...
    def send_msg_cmd_with_return(self, cmd, callback=None):
        ...
        return self.__send_msg_with_reply(
            ...
            *message.msg_cmd_with_return(cmd, version=self.serverVersion),
        )
    ...
    def send_msg_func_with_return(self, cmd, callback=None):
        ...
        if self.serverVersion < 3:
            ...
            args = message.msg_func_with_return(cmd, version=self.serverVersion)
            return self.__send_msg_with_reply(*args, replyCallback=callback)
    ...
    def send_msg_cmd(self, cmd):
        ...
        """
        self.__send_msg_no_reply(message.msg_cmd(cmd, version=self.serverVersion))
    ...
    def send_msg_chan_read(self, chanName, callback=None):
        ...
        return self.__send_msg_with_reply(
            replyCallback=callback,
            *message.msg_chan_read(chanName, version=self.serverVersion),
        )
    ...
    def send_msg_chan_send(self, chanName, value, wait=False):
        ...
        self.__send_msg_no_reply(
            message.msg_chan_send(chanName, value, version=self.serverVersion), wait
        )
    ...
    def send_msg_register(self, chanName):
        ...
        """
        self.__send_msg_no_reply(
            message.msg_register(chanName, version=self.serverVersion)
        )
    ...
    def send_msg_unregister(self, chanName):
        ...
        """
        self.__send_msg_no_reply(
            message.msg_unregister(chanName, version=self.serverVersion)
        )

    @try_connect
    def send_msg_close(self):
        """Send a close message."""
        self.__send_msg_no_reply(message.msg_close(version=self.serverVersion))

    @try_connect
    def send_msg_abort(self, wait=False):
        """Send an abort message."""
        self.__send_msg_no_reply(message.msg_abort(version=self.serverVersion), wait)

    def send_msg_hello(self):
        """Send a hello message."""
        self.__send_msg_no_reply(message.msg_hello())
