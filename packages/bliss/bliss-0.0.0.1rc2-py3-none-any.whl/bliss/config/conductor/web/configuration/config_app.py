...
def __get_plugin(name, member=None):
    try:
        mod = importlib.import_module(f"bliss.config.plugins.{name}")
    except ImportError:
        # plugin has an error
        return None
    if member:
...
def create_app():
    ...
    def get_db_file(filename):
        if flask.request.method == "PUT":
            ...
            return flask.json.dumps(
                dict(message="%s successfully saved", type="success")
            )
        else:
    ...
    @app.route("/tree/<view>/<item>")
    def sub_tree(view, item):
        if view == "files":
            return __config.tree_files
        elif view == "items":
            return __config.tree_items
        elif view == "plugins":
            return __config.tree_plugins
        elif view == "tags":
            return __config.tree_tags
        elif view == "sessions":
            return __config.tree_sessions
        else:
