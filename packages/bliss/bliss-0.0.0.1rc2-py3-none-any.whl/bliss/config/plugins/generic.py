...
def create_objects_from_config_node(cfg_obj, cfg_node):
    ...
    if cfg_node.plugin == "bliss":  # act as the older Bliss plugin
        ...
        tango_config = node.get("tango-server")
        if tango_config:
            klass, _ = find_class_and_node(tango_config)
        ...
        tango_config = ctrl_node.get("tango-server")
        if tango_config:
            klass, _ = find_class_and_node(tango_config)
