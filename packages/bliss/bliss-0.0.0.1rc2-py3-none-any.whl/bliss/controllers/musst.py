...
class Musst(BlissController):
    class Channel(object):
        ...
        def _convert(self, string_value):
            ...
            if self._mode_number == self.COUNTER:
                ...
                return int(string_value) * (10.0 / 0x7FFFFFFF)
            elif self._mode_number == self.ADC5:
                return int(string_value) * (5.0 / 0x7FFFFFFF)
    ...
    # Backward compatibility with BLISS <= 1.9
    channel = Channel
    ...
    def __info__(self):
        ...
        info_str += "PROGRAM:\n"
        info_str += f"     path={self.__prg_root}\n"

        info_str += f"TIMEBASE: {timebase}\n"
        info_str += "MEMORY:\n"
        info_str += f"     MCA:     size (32b values): {hmem:>8}, buffers: {hbuf:>8}\n"
        info_str += f"     STORAGE: size (32b values): {emem:>8}, buffers: {ebuf:>8}\n"
        info_str += "CHANNELS:\n"
    ...
    def ct(self, acq_time=None, wait=True):
        ...
        """
        self._timer_factor = self.get_timer_factor()
        diff = time.perf_counter() - self._last_run
        if diff < 0.02:
    ...
    def upload_program(self, program_data, template_replacement={}):
        ...
        if self.__one_line_programing:
            ...
            prg = b"".join([b"+%s\r\n" % line for line in program_data.splitlines()])
            self._cnx.write(prg)
    ...
    def get_data(self, nb_counters, from_event_id=0, max_data_rate=0):
        ...
        if from_event_id == 0:
            self._last_eptr_pointer = [0, 0, time.time()]
        for idx in range(5):
            ...
            (
                ...
                current_buffer_id,
                current_time,
            ) = self.get_event_memory_pointer()
            gevent.sleep(100e-3)
            next_offset, next_buffer_id, next_time = self.get_event_memory_pointer()
            last_offset, last_buffer_id, last_time = self._last_eptr_pointer

            if next_offset >= current_offset and next_buffer_id >= current_buffer_id:
                if max_data_rate == 0 or from_event_id == 0:
                    break
                if current_offset >= last_offset:
                    nblines = (current_offset - last_offset) // nb_counters
                else:
                    nblines = (
                        buffer_memory - last_offset + current_offset
                    ) // nb_counters
                if nblines <= 2:
                    break
                seen_rate = nblines / (current_time - last_time)
                if seen_rate < max_data_rate:
                    break
                log_warning(
                    self,
                    "get_data() EPTR %d filter too high data rate %.2f [>= %.2f]",
                    idx,
                    seen_rate,
                    max_data_rate,
                )
            ...
            log_debug(
                self,
                "get_data() EPTR %d filter [last] %d %d / [curr] %d %d / [next] %d %d",
                idx,
                last_offset,
                last_buffer_id,
                current_offset,
        ...
        self._last_eptr_pointer = [current_offset, current_buffer_id, current_time]
    ...
    def get_event_memory_pointer(self):
        ...
        for idx in range(5):
            eptr_raw = self.putget("?EPTR")
            eptr_time = time.time()
            eptr_values = [int(x) for x in eptr_raw.split()]
            if (
                eptr_values != [64, 0]
                and eptr_values != [256, 0]
                and eptr_values[0] < buff_values[0]
                and eptr_values[1] >= 0
            ...
            log_debug(
                self,
                "get_event_memory_pointer() EPTR %d filter bad values %s",
                idx,
                eptr_raw,
            )
            gevent.sleep(100e-3)

        return eptr_values + [eptr_time]
...
class Switch(BaseSwitch):
    """This class wraps musst commands to emulate a switch.
    ...
    class FakeCnx:
        ...
        def _readline(self, rxterm):
            ...
            if cmd.startswith("?"):
                ...
                elif cmd.startswith("?BTRIG"):
                    return self.last_btrig

                else:
                    return b"XX XX"
            else:
                if "BTRIG" in cmd:
                    self.last_btrig = cmd.split()[-1].encode()
                return b"OK"
