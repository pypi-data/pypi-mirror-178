...
class Micos(Controller):
    ...
    def initialize_hardware_axis(self, axis):
        ...
        #         to explicitly call here set_velocity() and
    ...
    def move_to_reference(self, axis, max_displacement):
        ...
        'to_reference_velocity' as set in the initialize_hardware_axis()
    ...
    def _reset_axis(self, axis):
        """
        Makes a software reset of the axis. The axis is initialized
    ...
    def _set_endswitch_types(self, axis, loest=Undefined, hiest=Undefined):
        """
        ...
        if loest is not Undefined and hiest is not Undefined:
    ...
    def _set_hw_limits(self, axis, hw_low_limit=Undefined, hw_high_limit=Undefined):
        """
        ...
        # If parameters passed, use them.
        if hw_low_limit is not Undefined and hw_high_limit is not Undefined:
    ...
    def _set_tofrom_endsw_velocity(self, axis, tofrom_endsw_velocity=Undefined):
        """
        ...
        if tofrom_endsw_velocity is not Undefined:
    ...
    def _set_action_at_powerup(self, axis, powerup_action):
        ...
        if powerup_action & 32 and axis.axis_on:
    ...
    def _get_cloop_window(self, axis):
        ...
        if axis.cloop_gstbit5sel is True:
    ...
    def _set_cloop_window(
        self, axis, winsize=Undefined, gstbit5sel=Undefined, trigopsel=Undefined
    ):
        """
        ...
        if gstbit5sel is Undefined:
            gstbit5sel = axis.cloop_gstbit5sel
        else:
            if gstbit5sel is not False and gstbit5sel is not True:
                raise ValueError(
                    "General status bit 5 enable/disable flag has wrong value %s"
                    % gstbit5sel
                )
            axis.cloop_gstbit5sel = gstbit5sel
        if gstbit5sel is True:
            gstbit5sel = 1
        else:
            gstbit5sel = 0

        if trigopsel is Undefined:
    ...
    def _set_cloop_wintime(self, axis, wintime=Undefined):
        """
    ...
    def _set_to_reference_velocity(self, axis, to_reference_velocity=Undefined):
        """
        ...
        if to_reference_velocity is not Undefined:
    ...
    def _wait_home_func(self, axis):
        try:
            ...
            _ = self._set_endswitch_types(axis, 2, 2)
