...
"""
    ...
    def connect(self):
        ...
        self.comm.connect()  # host=host, port=port, timeout=timeout)
    ...
    def dump(self):
        """
        return formatted info gathered from controller.
        """
        # READ INFO
        try:
            full_system_status = self.full_system_status
            full_actuators_status = self.full_actuators_status
            cs = self._user_and_object_cs  # coordinate system
            ...
            return "Cannot read info from controller"
        ...
        actuators_status_table = tabulate(rows, headers=headers, disable_numparse=True)
