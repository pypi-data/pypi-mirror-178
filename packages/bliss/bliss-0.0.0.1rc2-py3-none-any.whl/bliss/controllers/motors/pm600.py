...
from blissdata.settings import QueueSetting
...
class PM600(Controller):
    ...
    def _check_echo(self, cmd, reply):
        """Check if reply contains cmd

        The PM600 always echoes the command sent to it, before sending the response.
        It is terminated with a carriage return.
        """
        if cmd.endswith("\r"):
            cmd = cmd[:-1]
        echo, answer = reply.split("\r")

        try:
            assert echo == cmd, "PM600 Error: No echoed command"
        except AssertionError:
            # MCL to deal with unexpected chars on the line : '\x031OS\r01:10000000'
            # \x03 is controlC pushed to controller when need to stop running trajectory that echoes
            # should be fixed now, but keep it just in case ...
            log_warning(self, "corrupted echo: {0}".format(echo))
            assert echo.endswith(cmd), "PM600 Error: No echoed command"
        return answer

    def _check_error(self, reply):
        assert not reply.startswith("!"), f"PM600 Error: {reply[1:]}"
        return reply

    def _check_channel(self, channel, reply):
        chan, ret = reply.split(":")

        assert int(chan) == int(
            channel
        ), f"PM600 Error: reply from wrong channel ({chan}), reply {repr(reply)}"

        return ret

    def io_command(self, command, channel, value=None, nreplies=1, programming=False):
        cmd = f"{channel}{command}{value if value is not None else ''}\r"

        ans = self.sock.write_readlines(cmd.encode(), nreplies, eol=b"\r\n")

        if len(ans) == 1 and not programming:
            ans = ans.pop()
            reply = ans.decode()
            try:
                ret = self._check_channel(
                    channel, self._check_error(self._check_echo(cmd, reply))
                )
            except Exception as e:
                raise RuntimeError(
                    f"PM600: command {repr(cmd)} failed; reply: {repr(reply)}"
                ) from e
        else:
            reply_list = [r.decode() for r in ans]
            try:
                ret = self._check_error(self._check_echo(cmd, reply_list[0]))
    ...
    def flush(self):
        try:
            ans = self.sock.readline()
            while ans:
                log_warning(self, "flushing serial line:{0}".format(ans))
                ans = self.sock.readline()
        except:
            pass
