...
class Communication:
    ...
    def com_initialize(self):
        """
        Open the TCP socket to communicate with the controller.
        """
        # self.config comes from Controller class.
        # the PI_XXX class inherits from both Controller and Communication classes.
        self.sock = get_pi_comm(self.config, TCP)  # pylint: disable=no-member
        global_map.register(self, children_list=[self.sock])

        # ???
        connect(self.sock, "connect", self._clear_error)
        self.sock.flush()  # Purge eventual input from controller.
    ...
    def get_error(self):
        """
        Query controller for any error and return error number and description.
        """
    ...
    def command(self, cmd: str, nb_line: int = 1):
        r"""
        ...
        Parameters:
            cmd: Command. Not encoded; Without terminator character.
            nb_line: Number of lines expected in answer.
        ...
        with self.sock.lock:
            ...
            if need_reply:
                ...
                if not reply:  # it's an error
                    # self.name comes from Controller class
                    # the PI_XXX class inherits from both Controller and Communication classes.
                    errors = [self.name] + list(
                        self.get_error()
                ...
                if nb_line > 1:
                    ...
                    if len(commands) == nb_line:
                        ...
                        for command, rep in zip(commands, reply):
                            space_pos = command.find(b" ")
                            if space_pos > -1:
                                args = command[space_pos + 1 :]
                                parsed_reply.append(
                                    self._parse_reply(rep, args, command)
                                )
                            else:
