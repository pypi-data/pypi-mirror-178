...
from bliss.common.logtools import user_print, log_debug
    ...
    * implies:  implements a  .counters() method
    ...
    def _switch_to_mode(self, target_mode):
        ...
        if current_mode == target_mode:
            user_print(f"{self.name} is already in {MODE_TO_STR[target_mode]} mode.")
            return

        _t0 = time.time()
        if current_mode == MODE.MEASURE:
            user_print("stopping Measurement mode")
            self.dev.system.stopMeasurement()
        elif current_mode == MODE.ALIGN:
            user_print("stopping Alignement mode")
            self.dev.system.stopOpticsAlignment()
        elif current_mode != MODE.IDLE:
            user_print(f"Bad mode: {current_mode}  (wait or try stop() command ?)")
            return

        time.sleep(0.1)
        user_print("Waiting for idle mode...")
        while self.mode != MODE.IDLE:
            user_print(".", end="", flush=True)
            if (time.time() - _t0) > 5:
                raise RuntimeError(
        ...
        user_print("Now in idle mode.")

        starting_duration = STARTING_DURATION[target_mode]
        _t0 = time.time()
        user_print(f"Starting {target_mode} (~{starting_duration}s) ...")
        ...
        if target_mode == MODE.ALIGN:
            ...
            user_print(f"bad mode : {target_mode}")
        ...
        user_print(f"{self.mode} started in {int(time.time()-_t0)} seconds")
