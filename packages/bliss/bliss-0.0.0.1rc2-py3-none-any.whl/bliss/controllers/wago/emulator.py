...
from bliss.common.utils import get_open_ports, ReuseAddrTCPServer


class WagoEmulatorTCPServer(ReuseAddrTCPServer):
    timeout = 0.1
...
def Wago(
    ...
    app = get_server(WagoEmulatorTCPServer, address, RequestHandler)
    ...
    def interlock_handshake_phase_1(
        ...
        nonlocal regs_interlock
        if value == 0:
            regs_interlock.clear()  # empty memory
            interlock_phase = 2
            # next_step
    ...
    def interlock_handshake_read(
        ...
        if interlock_phase == 2:
            ...
            regs_interlock.clear()  # resets memory for next command
            interlock_phase = 3
