...
class TektronixOscCtrl(OscilloscopeHardwareController):
    ...
    def acq_read_channel(self, name, count_time=None):
        ...
        length = self.get_channel_shape()[0]
        ...
        time_data = numpy.linspace(0, header["NR_PT"] - 1, header["NR_PT"])
        time_data = time_data * header["XINCR"] + header["XZERO"]
    ...
    def acq_read_measurement(self, name):
        ...
        header = self.header_to_dict(head)
        value = self.strip(self.write_read(f"MEASUREMENT:{name}:VALUE?"))
        # is this always a float?
        return OscMeasData(float(value), header)
    ...
    def get_channel_shape(self):
        # TODO: which one?
        recordlength = self.strip(self.write_read("horizontal:recordlength?"))
        # recordlength = self.strip(self.write_read("HORizontal:MODE:RECOrdlength?"))
