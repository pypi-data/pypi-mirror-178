...
from bliss.scanning.acquisition.oscilloscope import OscilloscopeAcquisitionSlave
from bliss.controllers.counter import counter_namespace

OscAnalogChanData = namedtuple(
    "OscAnalogChanData", "length raw_data data time_data header"
)
...
class OscilloscopeHardwareController:
    """functions that deal with hardware related stuff"""
    ...
    def get_measurement_shape(self):
        return ()  # scalar
    ...
    def get_channel_shape(self):
        return (0,)  # spectrum of unknown length


class OscilloscopeMeasurement(Counter):
    @property
    def dtype(self):
        return self._counter_controller._scope._device.get_measurement_dtype()

    @property
    def shape(self):
        return self._counter_controller._scope._device.get_measurement_shape()


class OscilloscopeAnalogChannel(Counter):
    @property
    def dtype(self):
        return self._counter_controller._scope._device.get_channel_dtype()

    @property
    def shape(self):
        return self._counter_controller._scope._device.get_channel_shape()
