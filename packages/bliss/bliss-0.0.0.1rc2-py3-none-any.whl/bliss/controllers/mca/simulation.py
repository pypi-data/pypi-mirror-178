...
from .base import BaseMCA, PresetMode, TriggerMode, Brand, DetectorType
from .counter import Stats
...
class SimulatedMCA(BaseMCA):
    ...
    def stop_acquisition(self):
        if self._running:
            self._delta = time.perf_counter() - self._t0
    ...
    @property
    def delta(self):
        d = time.perf_counter() - self._t0 if self._running else self._delta
    ...
    def _generate_stats(self, realtime):
        ...
        icr = triggers / trigger_livetime if trigger_livetime else 0.0
        ocr = events / realtime if realtime else 0.0
        deadtime = 1 - ocr / icr if icr else 0.0
        return Stats(
            realtime,
            trigger_livetime,
            energy_livetime,
