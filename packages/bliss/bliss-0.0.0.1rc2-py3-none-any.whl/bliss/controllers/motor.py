...
class CalcController(Controller):
    ...
    def initialize(self):
        ...
        for real_axis in self.reals:
            event.connect(real_axis, "internal_move_done", self._real_move_done)
    ...
    def close(self):
        ...
        for real_axis in self.reals:
            event.disconnect(real_axis, "internal_move_done", self._real_move_done)
    ...
    def _get_pseudos_set_positions(self):
        return {
    ...
    def _real_position_update(self, pos, sender=None):
        self._real_pos_cache_dict[sender] = pos
    ...
    def _real_setpos_update(self, real_setpos):
        if any(m.is_moving for m in self.pseudos):
            return
        # user of real axis is used as dial for pseudo axis
        real_pos_dict = {
            self._axis_tag(axis): axis._set_position for axis in self.reals
        }

        new_pseudos_setpos = self.calc_from_real(real_pos_dict)

        for tagged_axis_name, setpos in new_pseudos_setpos.items():
    ...
    def check_limits(self, *axis_dial_pos_groups):
        ...
        axis_to_positions = {
            ...
            for tag, pos in self._get_complementary_pseudos_pos_dict(axes).items()
        }
        axis_to_positions.update(
            {
                self._axis_tag(axis): numpy.ones(position_len) * dial_pos
                for axis, dial_pos in axis_dial_pos_groups
            }
        )
            ...
            for pos in set(positions):
                try:
                    real_axis.controller.check_limits(
                        (real_axis, numpy.ones(1) * real_axis.user2dial(pos))
                    )
    ...
    def _calc_from_real(self):
        new_positions = self.calc_from_real(
            {
    ...
    def _get_motion_pos_dict(self, motion_list):
        # This is to be overwritten for specific cases ;
        # "motion list" is a list of Motion objects corresponding to the
        # pseudo axes that are required to move along with their target positions.
        # In the general case, return { pseudo_axis_tag: target } for all pseudos
        # ("target" == axis._set_position, since it is already set to desired
        # target from the move command)
        # The returned dict will be passed to "calc_to_real", in order to compute
        # the real axes target positions for the move.
        return self._get_pseudos_set_positions()
    ...
    def start_all(self, *motion_list):
        ...
        for tag, target_pos in self.calc_to_real(positions_dict).items():
            ...
            # dial positions calculated from calc controller are real axes user pos
    ...
    def set_position(self, axis, new_pos):
        ...
        dial_positions = self._get_pseudos_set_positions()
        dial_positions[self._axis_tag(axis)] = new_pos
        # dial of calc is user of real
        real_user_positions = self.calc_to_real(dial_positions)
        for real_axis_tag, user_pos in real_user_positions.items():
