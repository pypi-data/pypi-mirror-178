...
class NhqChannel:
    def __init__(self, nhq, channel):
        ...
        self._voltage_cached = SimpleSetting(f"{self.name}_voltage")
        self._voltage_cached_timestamp = SimpleSetting(f"{self.name}._voltage_date")
    ...
    @property
    def voltage(self):
        value = self._raw_read("voltage")
        self._voltage_cached.set(value)
        self._voltage_cached_timestamp.set(datetime.datetime.fromtimestamp(time.time()))
        return value

    @property
    def voltage_cached(self):
        if self._voltage_cached_timestamp.get() is None:
            return self.voltage
        return self._voltage_cached.get()
...
class Nhq(HasMetadataForScan):
    ...
    def __init__(self, name, config):
        ...
        self._metadata = config.get("metadata", ["chA", "chB"])
    ...
    def scan_metadata(self):
        ...
        for val in self._metadata:
            try:
                if val.endswith("_cached"):
                    meta_dict.update({val: meta_list[val].voltage_cached})
                    meta_dict.update(
                        {f"{val}_date": meta_list[val]._voltage_cached_timestamp.get()}
                    )
                else:
                    meta_dict.update({val: meta_list[val].voltage})
                meta_dict["@NX_class"] = "NXnhq"
            except Exception as e:
                log_warning(self, "could not read {0}: {1}".format(val, e))
