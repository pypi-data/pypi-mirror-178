...
class OpiomOutput:
    ...
    def _oo_polarity_set(self, value=False):
        ...
        """
        log_debug(
            self, "output polarity set to %s " % ("inversed" if value else "normal")
        )
    ...
    def _oo_tpg_add(self, duration, mask):
        ...
        Args:
            ...
            mask (int): the value of the unmasked bits (a bit is unmasked
                        if set to 1.
        """
        log_debug(self, "Add patern for %d ms, output mask %02x" % (duration, mask))
    ...
    def _oo_tpg_set(self, nb_repetition=1):
        ...
        Args:
            nb_repetion (int): number of pulse-series repetitions
                               (0 = infinite loop)
        """
        log_debug(self, "Set TPG channel, %d repetition(s)" % nb_repetition)
    ...
    def _oo_start(self, channel=None):
        ...
        if self.tpg_channel and channel is None:
            log_debug(self, "Start the TPG channel %d" % self.tpg_channel)
            self.opiom.comm("CNT %d START" % self.tpg_channel)
        else:
            log_debug(self, "Start channel %d" % channel)
    ...
    def config_sequence(self, duration, mask, polarity=None):
        ...
        try:
            ...
            log_info(self, "Added %d paterns" % duration.__len__())
    ...
    def status(self, verbose=False):
        ...
        if verbose:
            log_info(self, "Channel is %s, channel value = %d" % (state, val))
    ...
    def set_acquisition(self, filename=None, period=None, nb_repetition=1):
        """Ser the acquisition parameters to the opiom.

        - Get the acquisition parameters.
        - Convert them to opiom sequence.
