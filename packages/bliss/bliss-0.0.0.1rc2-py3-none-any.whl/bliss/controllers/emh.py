...
class EMH(CounterController):
    ...
    def read_all(self, *counters):
        ...
        for counter in counters:
            # ???
            if isinstance(counter.channel, Integral):
    ...
    def set_trigger_input(self, trigger_input):
        ...
        if trigger_input in TRIGGER_INPUTS:
            _ = self.raw_write_read("TRIG:INPU {}".format(trigger_input))
        else:
    ...
    def set_trigger_mode(self, trigger_mode):
        ...
        if trigger_mode in TRIGGER_MODES:
            _ = self.raw_write_read("TRIG:MODE {}".format(trigger_mode))
        else:
    ...
    def set_trigger_polarity(self, pola):
        ...
        if pola in TRIGGER_POLARITIES:
            _ = self.raw_write_read("TRIG:POLA {}".format(pola))
        else:
    ...
    def set_acq_filter_speed(self, speed):
        ...
        if speed in ACQU_FILTER_SPEEDS:
            _ = self.raw_write_read("ACQU:FILT {}".format(speed))
        else:
    ...
    def set_acq_mode(self, mode):
        ...
        if mode in ACQUISITION_MODES:
            _ = self.raw_write_read("ACQU:MODE {}".format(mode))
        else:
    ...
    def get_acq_data(self, start=None, count=1):
        ...
        for chan in range(4):
            ...
            _ = int(numbers[0])  # check that channel number is an int
    ...
    def get_acq_counts(self):
        ...
        """
        return int(self.raw_write_read("ACQU:NDAT?"))
    ...
    def set_acq_range(self, str_range):
        ...
        if str_range in RANGE_STRINGS:
            _ = self.raw_write_read("ACQU:RANGE {}".format(str_range))
    ...
    def set_acq_trig(self, trigger_count):
        ...
        """
        _ = self.raw_write_read("ACQU:NTRI {}".format(trigger_count))
    ...
    def get_acq_status(self):
        ...
        """
        return self.raw_write_read("ACQU:STUS?")
    ...
    def set_acq_time(self, acq_time):
        ...
        """
        _ = self.raw_write_read("ACQU:TIME {}".format(acq_time))
    ...
    def start_acq(self):
        ...
        _ = self.raw_write_read("ACQU:START")

    def start_acq_and_run(self):
        """Start acquisition and send a trigger if trigger mode is SOFTWARE."""
        _ = self.raw_write_read("ACQU:START SWTRIG")

    def stop_acq(self):
        """Stop an acquisition."""
        _ = self.raw_write_read("ACQU:STOP")
    ...
    def get_current(self, chan):
        ...
        """
        return float(self.raw_write_read("CHAN0{}:INSC?".format(chan)))
    ...
    def pulse(self):
        ...
        _ = self.raw_write_read("TRIG:SWSE True")
    ...
    def make_N_acq(self, acq_count, acq_time):
        ...
        _ = self.get_acq_counts()  # TODO: is this needed?
