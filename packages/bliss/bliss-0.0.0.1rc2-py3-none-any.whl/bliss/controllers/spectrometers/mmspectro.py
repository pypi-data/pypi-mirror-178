...
class MMAnalyser(Analyser):
    ...
    def _get_pos(self, tag):
        ...
        if tag == "rpos":
            ...
            return self.real_axes["theta"].position - 90
        elif tag == "yaw":
            return -self.real_axes["chi"].position
...
class MMDetector(Detector):
    ...
    def _get_pos(self, tag):
        ...
        if tag == "rpos":
            ...
            xpos = self.real_axes["dx"].position + dlong_center_x - self.dx_offset
            return xpos
        elif tag == "zpos":
            dlong_center_z = (
            ...
            pitch = 180 - self.real_axes["dth"].position
            return pitch
        raise RuntimeError(f"unknown tag {tag}")
