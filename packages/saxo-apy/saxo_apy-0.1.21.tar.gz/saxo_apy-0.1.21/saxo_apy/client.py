...
class SaxoOpenAPIClient:
    ...
    def openapi_request(
        ...
        headers = {
            "x-request-id": (
                f"saxo-apy/{self.client_session_id}/{token_urlsafe(ID_NUM_BYTES)}"
            ),
            "authorization": (
                f"Bearer {self._token_data.access_token}"  # type:ignore[union-attr]
            ),
        }

        logger.debug(f"performing request: {method} {_path}, " f"{params=}, {data=}")
