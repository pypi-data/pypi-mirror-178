...
class Requests(object):
    ...
    async def post(self, url, json=None, is_upload=False, headers=None, data=None):
        if is_upload:
            async with ClientSession() as Connection:
                response = await Connection._request('POST', url, data=data, headers=headers)
            ...
            async with ClientSession() as Connection:
                response = await Connection._request('POST', url, data=dumps(json).encode('utf-8'), headers={'Content-Type': 'application/json'})
                if response.status == 200:
                    response = await response.read()
                    return loads(response.decode('utf-8'))
                else: return False
    ...
    async def uploadFile(self, url, access_hash_send, file_id, byte):
        file_size = len(byte)
        if file_size <= 131072:
            header = {'access-hash-send': access_hash_send, 'file-id': str(file_id), 'part-number': '1', 'total-part': '1', 'chunk-size': str(file_size), 'auth': self.auth}
            while True:
                try:
                    ...
                    if response:
                        if response.get('status') == 'ERROR_GENERIC' or response.get('status') == 'ERROR_ACTION':
                            if response.get('status_det') == 'NOT_REGISTERED':
                                ...
                                raise InvaildAuth('Unfortunately, your account has been limited')
