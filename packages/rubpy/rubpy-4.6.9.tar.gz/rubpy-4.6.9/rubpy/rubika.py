...
class Client(object):
    ...
    async def baseUploader(self, **kwargs):
        upload_data = {
            'file_inline': {
                'dc_id': None,
                'file_id': None,
        ...
        if file.startswith('http'):
            if '?' in file.split('/')[-1]:
                ...
                file_name = file_splited.split('/')[-1]
                mime = '.{}'.format(file_splited.split('.')[-1])
            elif '.' in file.split('/')[-1]:
                file_name = file.split('/')[-1]
                mime = '.{}'.format(file.split('.')[-1])
            else:
                mime = '.unknown'
                file_name = 'file{}{}'.format(randint(100, 1000), mime)
        else:
            file_name, mime = file, file.split('.')[-1]
            ...
            if file_type == 'Music':
                duration = self.get_voice_duration(file)
                upload_data['file_inline']['time'] = duration
                upload_data['file_inline']['mime'] = mime
                upload_data['file_inline']['height'] = 0.0
                upload_data['file_inline']['width'] = 0.0
                upload_data['file_inline']['music_performer'] = kwargs.get('music_performer')
                upload_data['file_inline']['auto_play'] = False
            elif file_type == 'Voice':
                file_name, mime = '{}.ogg'.format(file.split('.')[0]), '.ogg'
                duration = self.get_voice_duration(file)
                upload_data['file_inline']['time'] = duration
                upload_data['file_inline']['mime'] = mime
            elif file_type == 'Gif':
            ...
            upload_data['file_inline']['size'] = file_size
            upload_data['file_inline']['file_name'] = file_name

        requestSendFile = await self.requestSendFile(file_name, mime, file_size)
        is_uploaded = await self.upload(requestSendFile.get('upload_url'), requestSendFile.get('access_hash_send'), requestSendFile.get('id'), file)
        upload_data['file_inline']['dc_id'] = requestSendFile.get('dc_id')
        upload_data['file_inline']['access_hash_rec'] = is_uploaded
        upload_data['file_inline']['file_id'] = requestSendFile.get('id')

        return await self.send('sendMessage', upload_data, 5)
