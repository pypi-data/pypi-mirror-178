...
def _hydrate_component(data_service_id: str, dataflow_id: str, component_id: str, base_dir: str):
  v2_name = Path(base_dir).resolve().joinpath(f'{data_service_id}', f'{dataflow_id}', 'components', f'{component_id}.py')
...
def _hydrate_dataflow(data_service_id: str, dataflow_id: str, new_dataflow_id: str, base_dir: str):
  v2_name = Path(base_dir).resolve().joinpath(f'{data_service_id}', f'{dataflow_id}', f'{dataflow_id}.py')
  file = next(filter(lambda x: os.path.exists(x), [v2_name]), None)
  if not file:
  ...
  module_d = importlib.util.module_from_spec(spec)
  spec.loader.exec_module(module_d)
  return vars(module_d)['construct_dataflow'](data_service_id, new_dataflow_id)
...
def clone_dataflow(
  ...
  with tempfile.TemporaryDirectory() as temp_dir:
    ...
    hydrated = _hydrate_dataflow(from_service, from_dataflow, new_dataflow_id, temp_dir)
