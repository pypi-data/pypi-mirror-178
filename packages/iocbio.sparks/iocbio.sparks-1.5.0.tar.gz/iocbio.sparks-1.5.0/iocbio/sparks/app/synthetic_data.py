...
class GroundTruth:
    ...
    def database_schema(db):
        db.query(
            ...
            + db.table(GroundTruth.database_table)
    ...
    def __init__(self, database, experiment_id, f0, gain):
        ...
        if self.database is not None:
            ...
            for row in self.database.query(
                "SELECT f0 FROM " + self.database.table(GroundTruth.database_table) + " WHERE experiment_id=:eid",
...
class Main(QWidget):
    def __init__(self, experiment_id, filename, comment, database, data, dx, dt, save_path=None):
        ...
        self.experiment = ExperimentHandler(
            experiment_id,
            filename,
            comment,
            database,
...
def app(args, f0):
    ...
    try:
        ...
        print("Initilizing tables failed:", e)
        database.close()
        exit()
    ...
    app = QApplication([])
    Main(experiment_id, filename, comment, database, data, dx=dx * 1.0e-6, dt=dt * 1.0e-3, save_path=args.save)
    app.exec_()
...
""",
    ...
    parser.add_argument(
        "--freq", type=float, default=1.5, help="Sparks frequency in sparks per 100 microns per 1 second"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=float,
        default=np.arange(0.05, 0.85, 0.05).tolist() + [1, 1.25, 1.5, 2],
