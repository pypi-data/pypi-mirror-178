...
class ROIHandler(QObject):
    ...
    sigRois = Signal(dict, list)

    sigActiveRoi = Signal(str)
    sigActiveSpark = Signal(Spark, list)
    ...
    def database_schema(db):
        db.query(
            ...
            + db.table(ROIHandler.database_table)
    ...
    def __init__(self, experiment_id, database, background, image):
        ...
        if self.database is not None:
            for row in self.database.query(
                "SELECT roi_id, x0, y0, x1, y1 FROM "
                ...
                self.rois[roi_id] = {
                    ...
                    "spark": Spark(self.experiment_id, roi_id, self.database, self.image, coords),
                }
        ...
        if self.database is None:
            ...
            return [
                ...
                for el in self.database.query(
                    "SELECT roi_id FROM "
                    + self.database.table(ROIHandler.database_table)
                    + " WHERE experiment_id=:eid ORDER BY x0, x1 ASC",
                    eid=self.experiment_id,
                )
            ]
    ...
    def trigger_updates(self):
        ...
        if self.current_roi is not None:
            self.sigActiveSpark.emit(
                self.rois[self.current_roi]["spark"], list(self.rois[self.current_roi]["data coordinates"])
            )
    ...
    def remove(self, roi_id):
        ...
        if self.database is not None:
            self.database.query(
                "DELETE FROM " + self.database.table(ROIHandler.database_table) + " WHERE roi_id=:rid", rid=roi_id
            )
