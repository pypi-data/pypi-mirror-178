...
NormalizationParameters = OrderedDict(
    [
        [
            "bg",
            NDesc(
                "Background", "Background intensity measured without cells [AU]", 0.0, "DOUBLE PRECISION", float, True
            ),
        ],
        ...
        [
            "sigma_space",
            NDesc(
                "Boxcar blur in spatial coordinate [microm]",
        ...
        [
            "median_space",
            NDesc(
                "Median filter in spatial coordinate [microm]",
        ...
        [
            "window",
            NDesc(
                "Distance between spline nodes for F0 calculation [s]",
                "Note that the distance between nodes should be significantly larger \
                    than expected sparks time-constants",
                5,
                "DOUBLE PRECISION",
                float,
                True,
...
SparkDetectorParameters = OrderedDict(
    [
        [
            "accept_as_possible_area",
            NDesc(
                "Minimal spark intensity",
        ...
        [
            "detector_median_time",
            NDesc(
                "Median filter in time [ms]",
                "Size of the median filter window in time. \
This filter is used while accessing spark intensity.",
                6.0,
                "DOUBLE PRECISION",
                float,
                True,
        ...
        [
            "detector_median_space",
            NDesc(
                "Median filter in spatial coordinate [microm]",
                "Size of the median filter window along spatial coordinate. \
This filter is used while accessing spark intensity.",
                0.6,
                "DOUBLE PRECISION",
                float,
        ...
        [
            "spark_detector_buffer_space",
            NDesc(
                "Space around automatically detected spark [microm]",
        ...
        [
            "spark_detector_buffer_time",
            NDesc(
                "Time around automatically detected spark [ms]",
...
class NormalizeImageSignals(QObject):
    sigNormalizedImage = Signal(str, np.ndarray)
    sigProperty = Signal(str, float)


class NormalizeImage(worker.Job):
    Parameters = dict(NormalizationParameters, **SparkDetectorParameters)
    ...
    def medblu(self, image):
        ...
        mb = ndimage.uniform_filter(
            mb, [int(round(self.sigma_time / 1e3 / self.dt)), int(round(self.sigma_space / 1e6 / self.dx))]
        )
        return mb

    def normalize(self):  # noqa C901
        ...
        if self.med_space < 3 or self.med_time < 3:
            msg = (
                "Too small window for median filter, please increase it. \
As it is, you tried to use the filter with %d pixels \
along spatial coordinate and %d pixels along time"
                % (self.med_space, self.med_time)
            )
        ...
        if self.det_med_space < 2 or self.det_med_time < 2:
            msg = (
                "Too small window for spark detector median filter, please increase it. \
As it is, you tried to use the filter with %d pixels along \
spatial coordinate and %d pixels along time"
                % (self.det_med_space, self.det_med_time)
            )
            info.error("Normalization error", "Spark detector median filter window too small", msg)
            return

        info.info(
            "Median filters: image blurring (%d pixels along spatial coordinate and \
%d pixels along time); spark detection (%d pixels along spatial coordinate \
and %d pixels along time)"
            % (self.med_space, self.med_time, self.det_med_space, self.det_med_time)
        )
        ...
        for iteration in iterations:
            ...
            for i in range(data.shape[1]):
                ...
                try:
                    ...
                    msg = "Failed to find F0 due to the spline fitter error. \
Please adjust the parameters for image correction"
                ...
                if so.min() <= 0.0:
                    msg = (
                        "Failed to fit line %d due to low or abrupt changes in intensity. \
Setting this line to the background level."
                        % i
                    )
                    info.warning(msg)
                    so = t_trace.copy()
                ...
                if so.min() <= 0.0:
                    msg = (
                        "Image background level is set to too high value (background = %.3f) or too many sparks \
were detected reducing the available data required for background correction. \
Jobs cancelled.\n\nConsider lower value for the image background."
                        % self.bg
                    )
...
class Image(QObject):
    sigUpdated = Signal(str)
    sigRoiAdd = Signal(list, bool)
    sigImageNormalized = Signal(bool)
    ...
    def database_schema_update_4_5(db):
        db.query(
            "ALTER TABLE " + db.table(Image.database_table) + " ADD detector_median_time DOUBLE PRECISION DEFAULT 6.0"
        )
        db.query(
            "ALTER TABLE " + db.table(Image.database_table) + " ADD detector_median_space DOUBLE PRECISION DEFAULT 0.6"
        )
    ...
    def __init__(self, raw, dt, dx, experiment_id, database):
        ...
        for k in keys:
            if isinstance(Image.Parameters[k].default, bool):
                v = bool(
                    int((settings.value(Image.settings_group + "/" + k, defaultValue=Image.Parameters[k].default)))
                )
            else:
                v = Image.Parameters[k].pytype(
                    settings.value(Image.settings_group + "/" + k, defaultValue=Image.Parameters[k].default)
                )
        ...
        if self.database is not None:
            ...
            for row in self.database.query(
                "SELECT " + psql + " FROM " + self.database.table(Image.database_table) + " WHERE experiment_id=:eid",
    ...
    def normalize(self, t0, t1, x0, x1, **kwargs):
        [it0, it1], [ix0, ix1] = self.get_array_indices([[t0, t1], [x0, x1]])
        # new_normalization_parameters = kwargs

        proceed = False
        if self.normalization_region != [[it0, it1], [ix0, ix1]]:
            proceed = True
        ...
        self.normalization_region = [[it0, it1], [ix0, ix1]]
        self.normalization_parameters.update(kwargs)

        self.processed = {}
        self.processed_offset = [it0, ix0]
        ...
        normalizer.set_data(self.raw[it0:it1, ix0:ix1])
        ...
        if self.database is not None:
            self.database.query(
                "DELETE FROM " + self.database.table(Image.database_table) + " WHERE experiment_id=:eid",
                eid=self.experiment_id,
            )
            ...
            args = args[:-1] + ")"
            self.database.query(
                "INSERT INTO " + self.database.table(Image.database_table) + "(" + pnames + ") VALUES " + args, **pvals
            )
    ...
    def get_slice(self, name, slc=None):
        ...
        if name in self.processed:
            ...
            if slc is None:
                ...
                [t0, t1], [x0, x1] = slc
                [bt0, bt1], [bx0, bx1] = self.normalization_region
                if t0 > bt0 and t1 < bt1 and x0 > bx0 and x1 < bx1:
    ...
    def get_array_indices(self, coords):
        ...
        """
        [t0, t1], [x0, x1] = coords
        return [int(round(t0 / self.dt)), int(round(t1 / self.dt))], [
            int(round(x0 / self.dx)),
            int(round(x1 / self.dx)),
        ]
    ...
    def _auto_region(self, r):
        [bt0, bt1], [bx0, bx1] = self.normalization_region
        r[0], r[1], r[2], r[3] = r[0] + bt0, r[1] + bx0, r[2] + bt0, r[3] + bx0
        ...
        if r[0] <= bt0 or r[1] <= bx0 or r[2] >= bt1 - 1 or r[3] >= bx1 - 1:
            return
