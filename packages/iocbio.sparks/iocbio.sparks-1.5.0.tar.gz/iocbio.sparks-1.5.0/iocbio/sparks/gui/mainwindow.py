...
from PySide6.QtCore import Qt, Signal, QSettings, QByteArray, QCoreApplication
from PySide6.QtWidgets import QWidget, QMainWindow, QHBoxLayout, QVBoxLayout, QSplitter, QTabWidget, QPushButton
from PySide6.QtWidgets import QMessageBox, QCheckBox, QScrollArea, QSystemTrayIcon, QApplication
...
from .custom_widgets import ImageAdjustments, ParametersSetter, SmallLabel, XYPlotRange
...
class MainGUI(QMainWindow):
    sigShiftOverViewPlot = Signal(Qt.Key)
    ...
    def __init__(self, experiment_id, filename, comment, database, data, dx, dt, transposed):
        ...
        QCoreApplication.processEvents()
        self.rois = ROIHandler(
            experiment_id=experiment_id, database=database, background=self.experiment.background, image=self.image
        )

        QCoreApplication.processEvents()

        self.spark_plot = SparkPlot(self.dx, self.dt)
        self.average_intensity_plot = AverageIntensityPlot(
            self.experiment.xdata, self.experiment.tdata, self.experiment.analysis_range
        )
        ...
        self.background_correction_plot = XYPlotRange(
            title="Normalized background SD",
        ...
        image_adjustments = ImageAdjustments(
            label="Set image intesity levels",
            vmin=vmin,
            vmax=vmax,
        ...
        self.background_correction_plot.sigAnalysisRegionChangeFinished.connect(
            self.experiment.set_analysis_range_space
        )
        ...
        if database.read_only:
            ...
            self.background_correction_plot.set_read_only(True)
