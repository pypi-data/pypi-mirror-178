...
class Observations(ApiEndpoint):
    ...
    @api_call(EntityObservationData)
    def get_all(self, search_criteria: Optional[str] = None) -> List[EntityObservationData]:
        return self.session.get(self.get_url(), params={"searchCriteria": search_criteria})

    @api_call(ObservationData)
    def get_device_observation_data(self, device_id: str, path: str) -> ObservationData:
