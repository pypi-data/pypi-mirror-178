...
def deserialize_one(json, dataclass: Optional[Any]):
    return from_dict(data_class=dataclass, data=json, config=config.Config(type_hooks=TYPE_HOOKS, cast=[Enum]))
...
def deserialize_dict(json: Dict[Any, Any], key_dataclass, value_dataclass):
    return {deserialize(key, key_dataclass): deserialize(value, value_dataclass) for key, value in json.items()}
...
def handle_response(response: Response, dataclass: Optional[Any]):
    try:
        if not response.ok or (response.json() is not None and 'error' in response.json()):
            raise _make_error("Received error response from the server", response)
        elif response.status_code != 204 and response.ok:
            return deserialize(json=response.json(), dataclass=dataclass)
        else:
            return None
    except Exception as e:
        if response.status_code == 404 and response.json() is None:
            return None
