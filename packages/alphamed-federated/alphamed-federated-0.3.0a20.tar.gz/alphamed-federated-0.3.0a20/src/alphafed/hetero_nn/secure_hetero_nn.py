...
class SecureHeteroNNHostScheduler(SecureHeteroNNScheduler):
    ...
    def _save_model(self):
        ...
        with open(f'{os.path.join(save_dir, "feature_model_ckp.pt")}', 'wb') as f:
            torch.save(self.feature_model.state_dict(), f)
        with open(f'{os.path.join(save_dir, "project_layer_ckp.pt")}', 'wb') as f:
            torch.save(self.project_layer.state_dict(), f)
        with open(f'{os.path.join(save_dir, "infer_model_ckp.pt")}', 'wb') as f:
    ...
    def _prepare_task_output(self) -> Tuple[str, str]:
        ...
        with open(feature_model_file, 'wb') as f:
            torch.save(self.feature_model.state_dict(), f)
        project_layer_file = os.path.join(save_dir, 'project_layer.pt')
