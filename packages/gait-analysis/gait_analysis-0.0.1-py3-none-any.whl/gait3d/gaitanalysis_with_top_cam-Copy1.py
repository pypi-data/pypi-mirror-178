...
class RecordingTop():
    ...
    Attributes:
        loaded_tracking_df(pandas.DataFrame): the Dataframe containing all bodyparts with x, y-coordinates and likelihood as returned by DLC
    ...
    def __init__(self, filepath: Path, week_id: int)->None:
        ...
        self.df_successfully_loaded, self.loaded_tracking_df = self._attempt_loading_df_from_file(filepath = filepath)
    ...
    def preprocess(self,
        ...
        marker_ids_to_preprocess = self._get_preprocessing_relevant_marker_ids(df = self.loaded_tracking_df)
    ...
    def _smooth_tracked_coords_and_likelihood(self, marker_ids: List[str], window_length: int, polyorder: int=3) -> pd.DataFrame:
        ...
        """
        smoothed_df = self.loaded_tracking_df.copy()
    ...
    def inspect_processing(self,
        ...
        if verbose:
            ...
            for marker_id in marker_ids_to_inspect:
                coverage = twoD.utils.compute_coverage(df = self.bodyparts[marker_id].df, critical_marker_ids = [marker_id])
