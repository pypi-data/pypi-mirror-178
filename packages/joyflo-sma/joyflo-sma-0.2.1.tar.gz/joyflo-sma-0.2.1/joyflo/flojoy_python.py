...
class CtrlPanel():
    ...
    def get_state(self):
        ...
        path = os.path.join(root, 'PYTHON/WATCH/fc.json')
        f = open(path)
...
def flojoy(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            FN = func.__name__
            job_status_key = jobset_id + '_'+ FN + '_STATUS'
            r.set(job_status_key, 'RUNNING')
            sys_status = '🏃‍♀️ Running python job: ' + FN
            r_obj = get_redis_obj(jobset_id)
            r.set(jobset_id, json.dumps({**r_obj,'SYSTEM_STATUS':sys_status,
              'RUNNING_NODE': FN}))
            ...
            # result_string = json.dumps(result, cls=PlotlyJSONEncoder)
            r.set(job_status_key, 'FINISHED')
            job_result_key = jobset_id + '_NODE_RESULTS'
            r.rpush(job_result_key, json.dumps({'cmd':FN, 'id': node_id, 'result': result}, cls=PlotlyJSONEncoder))
            # r_obj = get_redis_obj(jobset_id)
            # r.set(jobset_id, json.dumps({**r_obj,
            #   'RUNNING_NODE': ''}))
            return result
        except Exception:
            print(traceback.format_exc())
