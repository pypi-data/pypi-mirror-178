...
class obj:
    def __init__(self,AppId,AppSecret,token):
        ...
        if os.path.exists(self.path):
            ...
            self.list["id"] = []     # 建议ID索引，用于通过索引编号找到ID
        self.get_list()
    ...
    def get_list(self,
        ...
        for pageIndex in range(start,count):
            ...
            for item in res:
                ...
                res_data[IdNo] = [CardType, Title, RemainingCount, UsedCount, TotalCount, CreateTime]
                self.list[IdNo] = [CardType, Title, RemainingCount, UsedCount, TotalCount, CreateTime]
                if IdNo not in self.list["id"]:
                    self.list["id"].append(IdNo)

            if len(res_data) % pageSize != 0:
                break
        # 更新self.list
        if count == 10000:
            for id in self.list["id"]:
                if id not in res_data:
                    self.list["id"].remove(id)
                    if id in self.list:
                        self.list.pop(id)
        # 保存文件
