...
path = Path(__file__).parent/'../data/stopwords.txt'
with path.open() as f:
...
class Preprocess:
    ...
    def _remove_stopword(self, text):
        text = [t for t in text.split(' ') if not re.compile(r'\b\w{1,2}\b').search(t) and t not in stopwords]
        return ' '.join(text)
    ...
    def list(self, text_list):
        texts = [' '.join(self.get(text)) for text in text_list]
        return texts
