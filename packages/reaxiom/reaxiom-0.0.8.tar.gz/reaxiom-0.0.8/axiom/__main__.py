...
def get_global_values():
    ...
    return json.load(open(global_value_file))
...
def main(sys_argv):
    ...
    if sys_argv[1] == 'ls_old': # show the axiom.obj
        if len(sys_argv) == 3:
    ...
    if sys_argv[1] == 'ls':
        ...
        if len(sys_argv) == 2:
            axiom.common.fs_func[axiom.common.get_server()]['ls'](cwd, None)
        else:
            axiom.common.fs_func[axiom.common.get_server()]['ls'](cwd, sys_argv[2])

    if sys_argv[1] == 'ls.update':
        cwd = get_global_values()['cwd']
        
        if len(sys_argv) == 2:
            axiom.common.fs_func[axiom.common.get_server()]['ls'](cwd, None, update=True)
        else:
            axiom.common.fs_func[axiom.common.get_server()]['ls'](cwd, sys_argv[2], update=True)
    ...
    if sys_argv[1] == 'set.server': # set the server, from [local, huggingface, bypy]
        server_setting_file = os.path.join(config['axiom_path'], 'server')
        open(server_setting_file, 'w').write(sys_argv[2])

    
    if sys_argv[1] == 'set.huggingface': # settup the huggingface token and username
        token = getpass.getpass('huggingface token:')
        user_name = input('huggingface private db user:')
        obj_db = input('private huggingface obj db: %s/' % user_name)
        info_db = input('private huggingface info db: %s/' % user_name)
        sha256_info_db = input('private huggingface sha256 info db: %s/' % user_name)
        public_user_name = input('huggingface public db user:')
        public_obj_db = input('public huggingface obj db: %s/' % public_user_name)
        public_info_db = input('public huggingface info db: %s/' % public_user_name)
        public_sha_256_info_db = input('public huggingface sha256 info db: %s/' % public_user_name)
        ...
        d['obj_db'] = obj_db
        d['info_db'] = info_db
        d['sha256_info_db'] = sha256_info_db
        d['public_user_name'] = public_user_name
        d['public_obj_db'] = public_obj_db
        d['public_info_db'] = public_info_db
        d['public_sha_256_info_db'] = public_sha_256_info_db
        json.dump(d, open(os.path.join(huggingface_path, 'setting.json'), 'w'))

    if sys_argv[1] == 'show.server':
