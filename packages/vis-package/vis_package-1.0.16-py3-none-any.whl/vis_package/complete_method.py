    ...
    def _crElement_complete(self, elements, times):
        if len(self._index_vetcan_complete) == times:
            ...
            for i in range(start, len(elements)):
                for op in ['+', '-']:
                    if i == start and op != self._last_op_complete and self._last_op_complete != "":
                        continue
                    else:
                        ...
                        self._element_complete = self._element_complete[:len(self._element_complete)-len(elements[0])-1]
