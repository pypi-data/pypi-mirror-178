...
class TimeSeriesClient(Generic[TimeSeriesT, TimeSeriesMetadataT]):
    ...
    async def read_time_series_bulk(
        ...
        **kwargs,
        ...
        while len(ts_a_iters) > 0:
            ts_list = await self._bulk_concurrent(awaitables=[i.__anext__() for i in ts_a_iters])
            index_shift = 0
    ...
    async def read_coverage_bulk(
        ...
        *,
        ...
        return await self._bulk_concurrent(
            awaitables=[
                self.read_coverage(path=path, t0=t0_i, dispatch_info=dispatch_info_i, member=member_i)
        ...
        **kwargs: Any,
        ...
        while unfinished_iters > 0:
            ensemble_list = await self._bulk_concurrent(
                awaitables=[i.__anext__() for i in ensemble_a_iters]
            )
        ...
        **kwargs: Any,
        ...
        while unfinished_iters > 0:
            comments_list = await self._bulk_concurrent(
                awaitables=[i.__anext__() for i in comments_a_iters]
            )
        ...
        await self._bulk_concurrent(
            awaitables=[
                ...
                for path, comments_i, t0_i, dispatch_info_i, member_i in zip(
                    paths, comments, make_iterable(t0), make_iterable(dispatch_info), make_iterable(member)
                )
            ]
        )
