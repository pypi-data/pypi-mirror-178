...
from .coco_datastructure import CocoAnnotation, SuperClass, to_attributes_field
...
class CocoEncoder:
    ...
    def get_bbox_for_polyline(self, polygon: list) -> List[float]:
        if len(polygon) == 2:
...
def generate_coco_file(df: pd.DataFrame) -> dict:
    """
    ...
    """
    with st.spinner("Generating COCO file"):
        project_dir = st.session_state.project_dir
        ontology_file = st.session_state.ontology_file

        # Load label rows and get metrics dict
        label_rows = load_label_rows(df)
...
def df_to_nested_dict(df: pd.DataFrame) -> dict:
    ...
    for row in tqdm(df.iterrows(), desc="Parsing metrics", total=len(df)):
        ...
        if len(out) == 3:
            ...
            label_hash, data_hash, frame_number, object_hash = out
        else:
        ...
        if object_hash is None:  # Frame level metric
            frame_dict["frame-level"] = {
                k: v for k, v in row.items() if k not in ["identifier", "url"] and not pd.isnull(v)
            }
        else:  # Object level metric
            frame_dict[object_hash] = {
                k: v for k, v in row.items() if k not in ["identifier", "url"] and not pd.isnull(v)
            }
    return metrics


def load_label_rows(df: pd.DataFrame) -> dict[str, dict]:
    """
