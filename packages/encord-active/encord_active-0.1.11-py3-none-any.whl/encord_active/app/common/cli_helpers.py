...
def choose_local_project(config: AppConfig) -> Optional[Path]:
    ...
    if not projects_dir:
        print("[bold red] Projects dir is not defined[/bold] (Probably means you haven't imported a project).")
        return None
    else:
...
def get_local_project(project_dir: Path) -> Project:
    with open(project_dir / "project_meta.yaml", "r", encoding="utf-8") as f:
        project_meta = yaml.safe_load(f)

    ssh_key_path = Path(project_meta.get("ssh_key_path"))
