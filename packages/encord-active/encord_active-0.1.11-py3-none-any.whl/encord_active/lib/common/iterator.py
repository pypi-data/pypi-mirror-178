...
class Iterator(Sized):
    def __init__(self, project: Project, cache_dir: Path, use_images: bool = False, subset_size: int = -1, **kwargs):
        ...
        self.label_rows, self.image_paths = prepare_data(
            project, cache_dir, use_images=use_images, subset_size=subset_size, **kwargs
        )
...
class DatasetIterator(Iterator):
    ...
    def iterate(self, desc: str = "") -> Generator[Tuple[dict, Optional[Path]], None, None]:
        ...
        for label_hash, label_row in self.label_rows.items():
            ...
            if label_row.data_type in ["img_group", "image"]:
                ...
                image_paths: List[Path] = [None] * len(label_row.data_units)  # type: ignore
                if self.image_paths:
                    image_paths = self.image_paths[label_hash]

                data_units = sorted(label_row.data_units.values(), key=lambda du: int(du["data_sequence"]))
                for frame_idx, (data_unit, image_path) in enumerate(zip(data_units, image_paths)):
                ...
                # TODO link video frames paths when video frames are available as individual images
                data_unit = next(_ for _ in label_row["data_units"].values())
                self.du_hash = data_unit["data_hash"]
                ...
                for frame_sequence, frame_annotations in data_unit["labels"].items():
                    ...
                    if self.image_paths:
                        ...
                        image_path = list(image_folder.glob(f"{self.du_hash}_{frame_sequence}.*"))[0]
                    else:
                        image_path = None
    ...
    def get_label_logs(self, object_hash: Optional[str] = None, refresh: bool = False) -> List[dict]:
        ...
        data_hash = self.label_row_meta[self.label_hash]["data_hash"]
        try:
