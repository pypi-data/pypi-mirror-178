    ...
    for comp in arr_comps:
        ...
        if comp.type == "video":
            ...
            if comp.is_intro:
                ...
                rs.write_videofile(introPath, bitrate='4M', fps=24, codec='libx264', audio_bitrate='128000')
                is_cont = True
                rs.close()
            if comp.is_outro:
                ...
                if not rs.audio:
                    ...
                    vidPathTmp = FFmpeg.add_null_sound(comp.video_path)
                    rs = VideoFileClip(vidPathTmp)
                rs.write_videofile(outroPath, bitrate='4M', fps=24, codec='libx264', audio_bitrate='128000')
                is_cont = True
                rs.close()
            if is_cont:
                continue
