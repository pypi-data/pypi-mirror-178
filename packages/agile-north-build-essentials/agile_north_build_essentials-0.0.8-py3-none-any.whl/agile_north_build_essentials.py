...
def run_system_shell_command(args):
    proc = subprocess.check_output(get_system_shell_command(args))
    return str(proc).removeprefix('b\'').removesuffix('\'')
...
def use_nvm(version):
    print(run_command(['nvm', 'use', str(version)]))
    print(run_command(['nvm', 'alias', 'default', str(version)]))


def using_nvm_execute(version, args):
    nvm_installed(version)
    response = run_system_shell_command(['nvm', 'ls', str(version)])
    try:
        response.index(f"v{str(version)}")
    except ValueError:
        run_command(['nvm', 'install', str(version)])
    ret_code = run_command(['nvm', 'exec', str(version), ' '.join(args)])
    if ret_code == 1:
        raise OSError()
