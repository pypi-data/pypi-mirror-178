...
def has_valid_cells(p, parts):
    ...
    if len(p.cell_ids) > 0:  # has cells
        return True
    if len(p.subpart_names) == 0:  # no cell_ids, no subpart_names
        return False
    # no cell_ids, has subparts
    for i, sp_name in enumerate(p.subpart_names):
        pidx = get_part_index_by_name(parts, sp_name)
...
def part_cell_list_remove_invalid_cids(part_cell_list, invalid_cids=[],
    ...
    with open(output, 'w') as fo:
        ...
        block = utils.comment_a_block(block)
        fo.write(block)

        # write parts
        for i, p in enumerate(parts):
            if len(p.removed_cids) > 0:
                line = f"part: {p.name} removed {len(p.removed_cids)}/{len(p.cell_ids)+len(p.removed_cids)} cids: {utils.proper_str_list_of_ints(p.removed_cids)}"
                block = utils.length_control(line)
                fo.write(utils.comment_a_block(block))
