...
def part_cell_list_remove_invalid_cids(part_cell_list, invalid_cids=[],
    ...
    with open(output, 'w') as fo:
        ...
        line = f"IMPORTANT WARNING MESSAGE! Removed invalid cids: {utils.proper_str_list_of_ints(invalid_cids)}"
        ...
        for i, p in enumerate(parts):
            if len(p.removed_cids) > 0:
                line = f"part: {p.name} removed invalid cids: {utils.proper_str_list_of_ints(p.removed_cids)}"
                block = utils.length_control(line)
