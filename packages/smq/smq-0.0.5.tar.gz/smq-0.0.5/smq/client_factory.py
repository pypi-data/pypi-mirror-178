...
class ClientFactory:
	...
	def __init__(self, host, port, authkey):
		...
		m.connect()
		self._unique_key_manager = getattr(m, DEFAULT_UNIQUE_KEY_AGENT_NAME)()
		self._simple_manager = getattr(m, DEFAULT_SIMPLE_AGENT_NAME)()

	def create_simple_client(self, topic_name):
		simple_client = SimpleAgentClient(self._simple_manager, topic_name)
		return simple_client

	def create_unique_key_client(self, topic_name):
		unique_key_client = UniqueKeyAgentClient(self._unique_key_manager, topic_name)
