...
{' '*4}╚═╝      ╚══╝╚══╝ ╚═╝  ╚═══╝╚═════╝  ╚═════╝ ╚═╝  ╚═╝
    ...
    if args.command == "up" or args.command == "start":
        ...
        try:
            ...
            if not os.path.exists(ssh_keys_path):
                ...
                if len(glob.glob(f"{ssh_keys_path}/*.pub")) == 0:
                    pprint(
                        f"[red]=> Error: SSH keys not found. Please check if there are SSH keys in '{ os.path.expandvars(config['SSH']['SSH_KEY_PATH']) }' then try again.[/red]"
                    )
                    sys_exit(1)
            ...
            try:
                ...
                if config["CONTAINER"]["X11_FORWARDING"].lower() == "true":
                    ...
                    with console.status(
                        ...
                        if is_linux:
                            os.system(
                                "xhost +local:root +localhost >/dev/null 2>/dev/null"
                            )
                        else:
        ...
        if ssh_result == 0:
            ...
            os.execlp(
                "ssh", "-oStrictHostKeyChecking=no", "-X", "root@127.0.0.1", "-p 2222"
            )
        else:
            pprint(
            ...
            sys_exit(1)
    elif args.command == "down" or args.command == "stop":
        # Check if the container is running
        try:
            pwnbox_container = client.containers.get(config["CONTAINER"]["NAME"])
