...
class KitsuAPIWrapper(flow.Object):
    ...
    def get_shots_data(self, sequence):
        if isinstance(sequence, str):
            sequence = self.get_sequence_data(sequence)
    ...
    def get_assets_data(self, asset_type):
        ...
        return gazu.asset.all_assets_for_project_and_type(
            self._config.project_id.get(),
            asset_type
        )
    ...
    def get_shot_task_types(self, entity_type=None):
        types = gazu.task.all_task_types()
        types =  [
            t['name'] for t in types
            if entity_type is None or t['for_entity'] == entity_type
        ]
        return types
