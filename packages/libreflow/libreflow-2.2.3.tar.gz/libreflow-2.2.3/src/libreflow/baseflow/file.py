...
class CreateWorkingCopyFromRevision(flow.Action):
    ...
    def run(self, button):
        ...
        working_copy = file.create_working_copy(from_revision=self._revision.name())
...
bpy.ops.lfs.playblast(do_render=True, filepath='%s', studio='%s', project='%s', sequence='s%04i', scene='%s', do_simplify=%s, do_reduce_textures=%s, target_texture_width=%s, resolution_percentage=%s, version='%s')""" % (
            ...
            project_name,
            self._sequence_number_from_name(file_settings.get("sequence", "undefined")),
    ...
    def run(self, button):
        ...
        if not self._files.has_file(playblast_name, "mov"):
            ...
            df = next((f for f in tm.get_task_files(self._task.name()) if f[0] == f'{playblast_name}.mov'), None)
...
class PlayblastOutputModuleChoiceValue(flow.values.SessionValue):
    ...
    def choices(self):
        site = self.root().project().get_current_site()
        return site.ae_output_module_templates.keys()
    ...
    def revert_to_default(self):
        ...
        if value in self.choices():
            ...
            site = self.root().project().get_current_site()
            keys = site.ae_output_module_templates.keys()
            if keys:
                self.set(keys[0])
...
class RenderImageSequence(RenderWithAfterEffect):
    ...
    def extra_argv(self):
        ...
        if self.output_name.get() is not None:
            output_name = self.output_name.get().format(comp_name=comp_name, sequence=sequence_name, shot=shot_name, revision=revision.name())
        else:
...
class TrackedFile(File):
    ...
    def ensure_file_type(self):
        ...
        return self.file_type.set(file_type)
    ...
    def run(self, button):
        if button == 'Cancel':
            ...
            if self._folder.is_empty():
                ...
                working_copy = self._folder.create_working_copy(
                    from_revision=self.revision.get()
                )
...
class FileSystemRefs(EntityRefMap):
    ...
    def add_ref(self, source_oid, file_type):
        ref = super(FileSystemRefs, self).add_ref(source_oid)
