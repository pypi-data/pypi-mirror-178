...
def detect_attribute_is_required_from_type(type):
  tupe_origin = typing.get_origin(type)
  type_args = typing.get_args(type)
  if tupe_origin is types.UnionType and (types.NoneType in type_args):
    return False
  else:
    return True
...
def detect_attribute_type_from_type(type):
  tupe_origin = typing.get_origin(type)
  type_args = typing.get_args(type)
  if tupe_origin is types.UnionType and len(type_args) == 2 and type_args[1] is types.NoneType:
    type = type_args[0]
  if type is bool:
    return "BOOL"
  elif type is float:
