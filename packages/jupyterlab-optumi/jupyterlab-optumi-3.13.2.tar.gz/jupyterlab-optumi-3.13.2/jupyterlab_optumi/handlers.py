...
class StopNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            response = await IOLoop.current().run_in_executor(
                None, optumi.core.stop_notebook, workload, None # Remove the second argument when we move to 3.14
            )
            self.write(response.text)
            if response.status_code >= 300:
                jupyter_log.info(
...
class TeardownNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            workload = data["workload"]
            response = await IOLoop.current().run_in_executor(
                None, optumi.core.teardown_notebook, workload, None  # Remove the second argument when we move to 3.14
            )
            self.write(response.text)
            if response.status_code >= 300:
                jupyter_log.info(
