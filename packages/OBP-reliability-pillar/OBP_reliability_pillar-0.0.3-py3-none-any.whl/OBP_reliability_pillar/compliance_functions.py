...
def insert_to_res(res, Id: str, r_type: str, c_type: str, compliance: str) -> dict:
    if compliance == '':
        compliance = None
...
def s3_compliance(self) -> dict:
    ...
    for bucket in response['Buckets']:
        try:
            res = client.get_bucket_versioning(
                Bucket=bucket['Name'],
            )
            status = res['Status']
            if status == 'Enabled':
                res = insert_to_res(res, bucket['Name'], 'S3 Bucket', 'Bucket Versioning Enabled',
                              'Compliant')
            else:
                res = insert_to_res(res, bucket['Name'], 'S3 Bucket', 'Bucket Versioning Enabled',
                              'Non Compliant')
