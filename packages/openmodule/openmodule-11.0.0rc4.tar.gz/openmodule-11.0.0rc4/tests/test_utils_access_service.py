...
from openmodule.models.access_service import AccessCheckRequest, AccessTime, AccessCheckAccess, AccessCategory, \
    AccessRequestVehicle
from openmodule.models.base import OpenModuleModel
from openmodule.models.vehicle import LPRMedium, LPRCountry, Medium
...
class WithSessionsFunctionTest(AccessServiceWithSessionsTestMixin, TestCase):
    ...
    def test_access(self):
        self.access_service_class.access_by_lpr = {"GARIVO2": dict(id="access-lpr", category=AccessCategory.permanent,
                                                                   media=dict(lpr="GARIVO2"), accepted=True,
                                                                   access_data={},
        ...
        self.access_service_class.access_by_qr = {"qr1": dict(id="access-qr", category=AccessCategory.permanent,
                                                              media=dict(qr="qr1"), accepted=True, access_data={},
        ...
        self.access_service_class.access_by_nfc = {"nfc1": dict(id="access-nfc", category=AccessCategory.permanent,
                                                                media=dict(nfc="nfc1"), accepted=True, access_data={},
                                                                used_medium=Medium(type="nfc", id="nfc1"),
                                                                access_infos={}, source="test")}
        self.access_service_class.access_by_pin = {"pin1": dict(id="access-pin", category=AccessCategory.permanent,
                                                                media=dict(pin="pin1"), accepted=True, access_data={},
                                                                used_medium=Medium(type="pin", id="pin1"),
                                                                access_infos={}, source="test")}
...
class TestAccessServiceWithDatabase(AccessServiceWithDatabase):
    ...
    def db_model_to_access(self, db_model: TestAccessModelBase, used_medium: Medium, request: AccessCheckRequest) \
            -> AccessCheckAccess:
        media = {"lpr": db_model.lpr_id, "qr": db_model.qr_id, "nfc": db_model.nfc_id, "pin": db_model.pin_id,
                 "regex": db_model.regex}
        ...
        common_data = dict(id=db_model.id, group_id=db_model.group_id, customer_id=db_model.customer_id,
                           ...
                           parking_area_id=request.gate, category=AccessCategory.whitelist, media=media)
        access_data = dict(matching_scheme=db_model.matching_scheme, matching_version=db_model.matching_version,
                           **common_data)
        return AccessCheckAccess(cost_entries=[], access_data=access_data, used_medium=used_medium, accepted=True,
                                 **common_data)
...
class WithDatabaseFunctionTest(AccessServiceWithDatabaseTestMixin):
    ...
    def test_find_qr_accesses(self):
        ...
        accesses = self.access_service.find_qr_accesses(Medium(type="qr", id="QR1"),
                                                        AccessCheckRequest(name=settings.NAME, gate="gate1",
                                                                           vehicle=AccessRequestVehicle()))
        ...
        accesses = self.access_service.find_qr_accesses(Medium(type="qr", id="QR3"),
                                                        AccessCheckRequest(name=settings.NAME, gate="gate1",
                                                                           vehicle=AccessRequestVehicle()))
    ...
    def test_find_nfc_accesses(self):
        ...
        accesses = self.access_service.find_nfc_accesses(Medium(type="nfc", id="NFC1"),
                                                         AccessCheckRequest(name=settings.NAME, gate="gate1",
                                                                            vehicle=AccessRequestVehicle()))
        self.assertEqual(2, len(accesses))
        self.assertEqual("1", accesses[0].id)
        self.assertEqual("3", accesses[1].id)
        accesses = self.access_service.find_nfc_accesses(Medium(type="nfc", id="NFC3"),
                                                         AccessCheckRequest(name=settings.NAME, gate="gate1",
                                                                            vehicle=AccessRequestVehicle()))
        self.assertEqual(0, len(accesses))
    ...
    def test_find_pin_accesses(self):
        ...
        accesses = self.access_service.find_pin_accesses(Medium(type="pin", id="PIN1"),
                                                         AccessCheckRequest(name=settings.NAME, gate="gate1",
                                                                            vehicle=AccessRequestVehicle()))
        self.assertEqual(2, len(accesses))
        self.assertEqual("1", accesses[0].id)
        self.assertEqual("3", accesses[1].id)
        accesses = self.access_service.find_pin_accesses(Medium(type="pin", id="PIN3"),
                                                         AccessCheckRequest(name=settings.NAME, gate="gate1",
                                                                            vehicle=AccessRequestVehicle()))
        self.assertEqual(0, len(accesses))
    ...
    def test_deduplicate(self):
        accesses = [AccessCheckAccess(id=str(i), parking_area_id="1", media={}, access_data={}, accepted=True,
    ...
    def test_gate_check(self):
        def create_access_dummy(id, parking_area_id):
            return AccessCheckAccess(id=id, parking_area_id=parking_area_id, media={}, access_data={}, accepted=True,
                                     category=AccessCategory.whitelist, used_medium=Medium(type="qr", id=""),
                                     access_infos={}, source="test")

        def create_access_dummies():
            return [create_access_dummy(id="1", parking_area_id="pa1"),
                    create_access_dummy(id="2", parking_area_id="pa2"),
                    create_access_dummy(id="3", parking_area_id="pa3"),
                    create_access_dummy(id="6", parking_area_id=None),
                    create_access_dummy(id="7", parking_area_id="all")
                    ]

        class ParkingAreaModel(OpenModuleModel):
            id: str
            gates: Optional[List[str]]  # list of Gate.gates, None means applies to all

            def matches_gate(self, gate: str) -> bool:
                """
                checks if parking area matches gate
                """
                return self.gates is None or gate in self.gates

        accesses = create_access_dummies()
        parking_areas = {}
        self.access_service.check_accesses_valid_at_gate(accesses, "gate1", parking_areas)
        ...
        accesses = create_access_dummies()
        parking_areas = {"pa1": ParkingAreaModel(id="pa1", gates=["gate1", "gate2"]),
                         "pa2": ParkingAreaModel(id="pa2", gates=["gate2"])}
        self.access_service.check_accesses_valid_at_gate(accesses, "gate1", parking_areas)
        self.assertEqual([True, False, True, True, True], [a.accepted for a in accesses])

        accesses = create_access_dummies()
        self.access_service.check_accesses_valid_at_gate(accesses, "gate2", parking_areas)
        self.assertEqual([True, True, True, True, True], [a.accepted for a in accesses])

        accesses = create_access_dummies()
        self.access_service.check_accesses_valid_at_gate(accesses, "gate3", parking_areas)
        self.assertEqual([False, False, True, True, True], [a.accepted for a in accesses])

        parking_areas["all"] = ParkingAreaModel(id="all", gates=None)
        accesses = create_access_dummies()
        self.access_service.check_accesses_valid_at_gate(accesses, "gate3", parking_areas)
        self.assertEqual([False, False, True, True, True], [a.accepted for a in accesses])
...
class RpcTest(AccessServiceWithSessionsTestMixin, RPCServerTestMixin, TestCase):
    ...
    def test_access_rpc(self):
        ...
        response = self.rpc("access_service", "auth", request=request)
        self.assertEqual(True, response["response"]["success"])
