...
class KVStoreRPCTestCase(SQLiteTestMixin, RPCServerTestMixin, OpenModuleCoreTestMixin):
    ...
    def test_rpcs(self):
        ...
        response = KVSetResponse.parse_obj(self.rpc(channel="kv_sync", type="set", request=set_request)["response"])
        ...
        response = KVSyncResponse.parse_obj(self.rpc(channel="kv_sync", type="sync", request=sync_request)["response"])
    ...
    def test_filter(self):
        ...
        with self.assertRaises(TimeoutError):
            self.rpc(channel="kv_sync", type="set", request=request)
    ...
    def test_start_sync(self):
        def start_sync(*_):
            ...
            sync_response = KVSyncResponse.parse_obj(
                self.rpc(channel="kv_sync", type="sync", request=sync_request)["response"])
            ...
            set_response = KVSetResponse.parse_obj(
                self.rpc(channel="kv_sync", type="set", request=set_request)["response"])
