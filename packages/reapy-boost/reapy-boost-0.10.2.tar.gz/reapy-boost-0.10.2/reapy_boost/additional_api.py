...
All fixes will be applied to `reapy_boost.reascript_api` by reapy_boost during
...
from reapy_boost import is_inside_reaper
from reapy_boost import inside_reaper
if is_inside_reaper():
    from reapy_boost.reascript_api import _RPR
...
def MIDI_InsertEvt(take, selected, muted, ppqpos, bytestr, bytestr_sz):
    ...
    return f(
        _RPR.rpr_packp("MediaItem_Take*", take), ct.c_byte(selected),
        ct.c_byte(muted), ct.c_double(ppqpos), packs_l(bytestr),
        ct.c_int(bytestr_sz)
    )
...
def MIDI_InsertTextSysexEvt(
    ...
    return f(
        _RPR.rpr_packp("MediaItem_Take*", take), ct.c_byte(selected),
        ct.c_byte(muted), ct.c_double(ppqpos), ct.c_int(type_),
        packs_l(bytestr), ct.c_int(bytestr_sz)
    )
...
def MIDI_SetAllEvts(take, buf, buf_sz):
    ...
    return f(
        _RPR.rpr_packp("MediaItem_Take*", take), packs_l(buf, size=buf_sz),
        ct.c_int(buf_sz)
    )
