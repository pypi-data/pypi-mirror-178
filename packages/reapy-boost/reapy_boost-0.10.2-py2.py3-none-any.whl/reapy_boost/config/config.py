...
import reapy_boost
from reapy_boost.errors import OutsideREAPERError
from reapy_boost.reascripts import activate_reapy_server
from .resource_path import get_resource_path
from .shared_library import get_python_shared_library

__all__ = [
    'add_web_interface', 'configure_reaper', 'create_new_web_interface',
    'delete_web_interface', 'disable_dist_api', 'enable_dist_api',
    'enable_python', 'REAPY_SERVER_PORT', 'WEB_INTERFACE_PORT'
]

REAPY_SERVER_PORT = 2308
WEB_INTERFACE_PORT = 2309
...
class Config(ConfigParser):
    ...
    def write(self):
        # Backup config state before user has ever tried reapy_boost
        before_reapy_file = self.ini_file + '.before-reapy_boost.bak'
...
def add_reascript(resource_path: str, script_path: str) -> str:
    ...
    In other cases, make use of :func:`reapy_boost.add_reascript`.
...
def configure_reaper(
    ...
    """Configure REAPER to allow reapy_boost connections.

    Allows to use reapy_boost from outside REAPER.
    ...
    3. Add a web interface on port 2307 to listen to reapy_boost
       connections.
    4. Add the ReaScript ``reapy_boost.reascripts.activate_reapy_server``
       to the *Actions* list.
    ...
    resource_path : str or None, optional
        ...
        the result of
        :func:`reapy_boost.config.resource_path.get_resource_path`. Use it
    ...
    set_ext_state(
        "reapy_boost", "activate_reapy_server", action, resource_path
    )
...
def create_new_web_interface(port: int) -> None:
    ...
    .. deprecated:: 0.8.0
          ``create_new_web_interface`` will be removed in reapy_boost 1.0.0.
          Use :func:`reapy_boost.config.add_web_interface` that works from
          outside REAPER.
    ...
    msg = (
        ...
        "Use reapy_boost.config.add_web_interface instead."
    )
    warnings.warn(FutureWarning(msg))
    config = Config(reapy_boost.get_ini_file())
    csurf_count = int(config["reaper"].get("csurf_cnt", "0"))
    csurf_count += 1
    config["reaper"]["csurf_cnt"] = str(csurf_count)
    key = "csurf_{}".format(csurf_count - 1)
...
def disable_dist_api() -> None:
    ...
    Delete ``reapy_boost`` Web interface, and remove the ReaScript
    ``reapy_boost.reascripts.activate_reapy_server`` from the
    Actions list.
    """
    if not reapy_boost.is_inside_reaper():
        raise OutsideREAPERError
    delete_web_interface(reapy_boost.get_resource_path(), WEB_INTERFACE_PORT)
    reascript_path = get_activate_reapy_server_path()
    reapy_boost.remove_reascript(reascript_path)
    message = ("reapy_boost will be disabled as soon as you restart REAPER.")
    reapy_boost.show_message_box(message)
...
def enable_dist_api() -> None:
    ...
    .. deprecated:: 0.8.0
          ``enable_dist_api`` will be removed in reapy_boost 1.0.0.
          Use :func:`reapy_boost.config.configure_reaper` that works
          even from outside REAPER.

    Create a Web interface and add the ReaScript
    ``reapy_boost.reascripts.activate_reapy_server`` to the Actions list.
    """
    msg = (
        "Function enable_dist_api is deprecated since 0.8.0. " +
        "Use reapy_boost.config.configure_reaper instead."
    )
    warnings.warn(FutureWarning(msg))
    if not reapy_boost.is_inside_reaper():
        raise OutsideREAPERError
    ...
    reascript_path = get_activate_reapy_server_path()
    action_id = reapy_boost.add_reascript(reascript_path)
    command_name = json.dumps(reapy_boost.get_command_name(action_id))
    section, key, value = "reapy_boost", "activate_reapy_server", command_name
    reapy_boost.set_ext_state(section, key, value, persist=True)
    message = (
        "reapy_boost successfully enabled!\n\nPlease restart REAPER.\n\nYou will "
        + "then be able to import reapy_boost from the outside."
    )
    reapy_boost.show_message_box(message)
...
def set_ext_state(
    ...
    Only use this function at setup time to configure REAPER.
    In other cases, make use of :func:`reapy_boost.set_ext_state`.
