...
class FXParam(float):
    ...
    def add_envelope(self) -> 'reapy_boost.Envelope':
        """
...
class FXParamsList(ReapyObjectList):
    ...
    def __init__(
        self,
        parent_fx: Optional['reapy_boost.FX'] = None,
        parent_id: Optional[str] = None,
        parent_fx_index: Optional[int] = None
    ) -> None:
        if parent_fx is None:
            parent_fx = reapy_boost.FX(
                parent_id=parent_id, index=parent_fx_index
            )
    ...
    @property
    def parent_fx(self) -> 'reapy_boost.FX':
        """
        Parent FX.

        :type: FX
        """
        fx = reapy_boost.FX(parent_id=self.parent_id, index=self.fx_index)
        return fx
