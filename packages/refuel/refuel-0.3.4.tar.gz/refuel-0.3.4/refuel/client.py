...
class RefuelClient:
    ...
    def _query_helper(self, url: str, params: Dict = None, verbose: bool = False) -> pd.DataFrame:
        ...
        max_events = min(params.get("max_events"), self._event_count_helper(url, params))
        ...
        for batch_num, offset_start in enumerate(offset_starts):
            request_params = copy.deepcopy(params)
            request_params["offet"] = offset_start
            request_params["max_events"] = min(max_events - offset_start, RefuelClient.QUERY_STEP_SIZE)
        ...
        sorted_by_batch_idx = [item[1]
                            for item in sorted(batch_idx_to_events.items())]
    ...
    def query_annotation_queue(
        ...
        return self._query_helper(
            url=self._api_base_url + "/query_annotation_queue", params=params, verbose=verbose
        )
    ...
    def query_similar_events(
        ...
        return self._query_helper(
            url=self._api_base_url + "/query_similar_events", params=params, verbose=verbose
        )
