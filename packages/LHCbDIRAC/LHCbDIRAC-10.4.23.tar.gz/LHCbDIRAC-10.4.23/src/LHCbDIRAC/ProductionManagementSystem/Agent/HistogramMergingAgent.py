...
from LHCbDIRAC.Core.Utilities.XMLTreeParser import XMLTreeParser, XMLNode
...
class HistogramMergingAgent(AgentModule):
    ...
    def execute(self):
        ...
        request_ids = list(set(int(t["TransformationFamily"]) for t in transformations))
    ...
    def _handle_transformation(
        ...
        metadata = self._format_metadata(transform, request, {"LFN": output_lfn, "PFN": output_pfn})
    ...
    def _query_lfns_exist(self, lfns: list[str], *, expected: str = "Failed") -> dict:
        ...
        result = returnValueOrRaise(self.bk_client.getFilesInformations(lfns))
    ...
    def _commit_and_push_metadata(self, new_files: list[Path], message: str):
        ...
        with self.simdq_repo.git.custom_environment(GIT_SSH_COMMAND=f"ssh -i {self.ssh_key}"):
            self.simdq_repo.remotes.origin.pull(rebase=True)
