...
class LaharFlowData(object):
    ...
    def _get_data_txt(self, LaharFlowFile, var=None):
        try:
            data = pd.read_csv(
                LaharFlowFile,
                delimiter=",",
            ...
            if var is not None:
                drop_vars = [d for d in data.columns if d not in var]
    ...
    def _get_data_nc(self, LaharFlowFile, var=None):

        raster = nc2xr(LaharFlowFile, var=var)
        raster = raster.rename({"x_distance": "x", "y_distance": "y"})
        raster["x"] = raster["x"] + self.utm[0]
        raster["y"] = raster["y"] + self.utm[1]
        raster.rio.write_crs(self.utmCode, inplace=True)
    ...
    def _get_data(self, filename, var=None):
        ...
        if fileext == ".txt":
            return self._get_data_txt(LaharFlowFile, var=var)
        elif fileext == ".nc":
            try:
                d = self._get_data_nc(LaharFlowFile, var=var)
            except:
    ...
    def raster(
        ...
        raster = self._get_data(filename, var=var)
