...
def load_candle_by_date(
    ...
    '''
    # 检查是否有数据
    if not check_candle_day_path(instType=instType, instId=instId, start_date=start_date, end_date=end_date,
...
def save_candle_by_date(
    ...
    for date in dates:
        ...
        this_df = df[(df[ts_column_name] >= start_ts) & (df[ts_column_name] <= end_ts)]

        if valid_diff and not (np.diff(this_df[ts_column_name]) == bar_interval).all():
            msg = '{instId}的candle的时间间隔错误'.format(instId=instId)
            raise exception.CandleDiffError(msg)
        if valid_start and this_df[ts_column_name].min() != start_ts:
            msg = '{instId}的candle的时间起始错误，时间的起点是：{datetime}'.format(
                instId=instId,
                datetime=finance_utils.date.to_fmt(date=int(this_df[ts_column_name].min()), timezone=timezone,
                                                   fmt='%Y-%m-%d %H:%M:%S'),
            )
            raise exception.CandleStartError(msg)
        if valid_end and this_df[ts_column_name].max() != end_ts:
            msg = '{instId}的candle的时间终止错误，时间终点是：{datetime}'.format(
                instId=instId,
                datetime=finance_utils.date.to_fmt(date=int(this_df[ts_column_name].max()), timezone=timezone,
                                                   fmt='%Y-%m-%d %H:%M:%S')
            )
