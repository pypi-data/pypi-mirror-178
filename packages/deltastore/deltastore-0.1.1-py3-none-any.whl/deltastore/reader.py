...
class DeltaSharingReader:
    ...
    def _to_pandas(
        ...
        pa_table = (
            pa_dataset.to_table(filter=filters, columns=columns).slice(length=limit)
            if limit is not None
            else pa_dataset.to_table(filter=filters, columns=columns)
        )
