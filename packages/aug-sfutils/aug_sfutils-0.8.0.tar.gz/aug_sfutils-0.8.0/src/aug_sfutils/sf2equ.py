...
# Eq 12 in Coco paper
        ...
        self.Bt /=  2.*np.pi * self.psi_sign * self.phi_sign
...
# Equation 9, table I, equation 39, 45
    ...
    def timeDownsample(self, time_ds, noELMs=False):

        indbin  = None # Compute first time
        ind_elm = None # Compute first time
        self.noELMs = noELMs
        nt = len(self.time)
        self.pfm, self.ind_elm, self.indbin = time_slice.map2tgrid(time_ds, self.time, np.transpose(self.pfm, axes=(2, 0, 1)), nshot=self.shot, noELMs=self.noELMs)
        ...
        for attr, val in self.__dict__.items():
            ...
            if hasattr(val, 'shape'):
                ...
                if val.ndim > 0:
                    if val.shape[0] == nt:
                        tmp, _, _ = time_slice.map2tgrid(time_ds, self.time, getattr(self, attr), ind_elm=ind_elm, indbin=indbin)
