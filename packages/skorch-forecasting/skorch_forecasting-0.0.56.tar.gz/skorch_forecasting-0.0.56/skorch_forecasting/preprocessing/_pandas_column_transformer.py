...
def _empty_ndarray(shape):
    """Private function that returns empty numpy array of desired shape.
    """
    return np.ndarray(shape=shape)


def _hstack(arrays, cast_to_object=True):
    """Private function horizontal stacking of numpy arrays.
...
class PandasColumnTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        self._fit_sklearn_column_transformer(X)
        self.feature_dtypes_in_ = X.dtypes.to_dict()
        self.feature_names_in_ = self.column_transformer_.feature_names_in_
        return self

    def _fit_sklearn_column_transformer(self, X):
        self.column_transformer_ = _make_sklearn_column_transformer(
            self.transformers)
    ...
    def get_feature_name_out_for_transformer(
        ...
        """
        input_features = _check_feature_names_in(self.column_transformer_,
                                                 input_features)
...
class PandasInverseTransformer:
    ...
    Parameters
    ----------
    pandas_column_transformer : PandasColumnTransformer
        Fitted :class:`PandasColumnTransformer` instance.
    """

    def __init__(
            self,
            pandas_column_transformer,
            non_inverse_transformers='ignore'
    ):
        check_is_fitted(pandas_column_transformer)
        self.pandas_column_transformer = pandas_column_transformer
    ...
    def inverse_transform(self, X):
        ...
        inverse_transforms = []
        pandas_columns = []
        for name, trans, features_in, _ in self.pandas_column_transformer.iter():
            features_out = self.pandas_column_transformer \
                .get_feature_name_out_for_transformer(
    ...
    def _to_pandas(self, inverse_transforms, pandas_columns):
        ...
        arr = _hstack(inverse_transforms)
        return pd.DataFrame(arr, columns=columns).astype(dtypes)
    ...
    def _get_feature_dtypes_in(self):
        """Returns feature dtypes in from ``self.pandas_column_transformer``.
        """
        return self.pandas_column_transformer.feature_dtypes_in_
