...
KeyMakerFn = Callable[[Optional[int], Optional[int], Optional[int]], str]


def _default_key_maker(
    bot_id: Optional[int] = None, chat_id: Optional[int] = None, user_id: Optional[int] = None, /,
) -> str:
    if bot_id is None or chat_id is None and user_id is None:
        raise ValueError(
    ...
    if user_id is None and chat_id is not None:
        ...
        chat_id = user_id
    return f"{bot_id}_{chat_id}:{user_id}"
...
class UserCage(Generic[StorageDataT]):
    ...
    def __init__(
        ...
        self.key = key_maker(bot_id, chat_id, user_id)
