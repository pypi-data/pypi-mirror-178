	...
	def items(self) -> List[dict]:
		...
		query = \
			...
			SELECT ?path ?name ?order ?class ?minCount ?maxCount
						?datatype (lang(?name) as ?lang) WHERE {
				?_ sh:path ?path ;
					sh:name ?name ;
					sh:order ?order .
		...
		for result in results:
			...
			if lang == "de":
				items_de.append(properties)
			else:
				items_en.append(properties)

		# Fuse language lists into one
		for index, item in enumerate(items_de):
			item["name"] = [item["name"], items_en[index]["name"]]
			items.append(item)
		return items
