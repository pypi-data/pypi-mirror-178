import UtailController as utilController
import LogController as logController
import config
from datetime import datetime
class GenericController:
    def __init__(self, app="", path_log_app=""):
        self.app_name = app
        self.data_generic_config = utilController.readFileJson(config.ConfigGenericApp.path_generic_config)
        self.log = logController.LogController("", app, path_log_app)

        if(self.data_generic_config == False):
            # self.log.genLog("Generic not found !!", "error")
            print("Generic not found !!")
            return

        # if(app in self.data_generic_config["app_name"]):
        self.data_app_config = utilController.readFileJson(config.ConfigGenericApp.path_app)
        # self.data_app_config = utilController.readFileJson(config.ConfigProcess.path_generic_app + "/" + app + "/app_config.json")

        if(self.data_app_config == False):
            # self.log.genLog("App not found !!", "error")
            print("App not found !!")
            return 

        self.controller_app = {}

        # if(self.data_generic_config["GLOBAL_SOURCE"] in self.data_app_config["controller_app"]):
        #     module = __import__("controller." + app + ".SourceDataController", fromlist=["SourceDataController"])
        #     my_class = getattr(module, "SourceDataController")
        #     self.controller_app["source"] = my_class

        if(self.data_generic_config["GLOBAL_REPORT"] in self.data_app_config["controller_app"]):
            module = __import__("controller." + app + ".ReportController", fromlist=["ReportController"])
            my_class = getattr(module, "ReportController")
            self.controller_app["report"] = my_class

        if(self.data_generic_config["GLOBAL_MEMBER"] in self.data_app_config["controller_app"]):
            module = __import__("controller." + app + ".MemberController", fromlist=["MemberController"])
            my_class = getattr(module, "MemberController")
            self.controller_app["member"] = my_class

        if(self.data_generic_config["GLOBAL_SUMMARY"] in self.data_app_config["controller_app"]):
            module = __import__("controller." + app + ".SummaryController", fromlist=["SummaryController"])
            my_class = getattr(module, "SummaryController")
            self.controller_app["summary"] = my_class
            
        # else:
        #     self.data_generic_config = False
        #     self.log.genLog("App not found !!", "error")
        #     print("App not found !!")
    ...
    def handleSetUpSourceData(self, connect_db, generic, source_for_process):
        if(source_for_process != ""):
            return self.controller_app["report"].setUpSourceData(self.controller_app["report"], connect_db, generic, source_for_process)
        return source_for_process

    def handlegetSourceData(self, connect_db_input, generic, last_update_source):
        return self.controller_app["source"].getSourceData(self.controller_app["source"], connect_db_input, generic, last_update_source)

    def handlegetSourceDetail(self, connect_db_input, generic, source_data): 
        return self.controller_app["source"].getSourceDataDetail(self.controller_app["source"], connect_db_input, generic, source_data)
    ...
    def handleProcessMember(self, connect_db_output, source, source_by_create_date, source_detail_by_create_date, source_data_no_detail, model_report, model_summary, flag_data, data_cluster_representative, data_explore_cluster, generic):
        self.controller_app["member"].processMember(self.controller_app["member"], connect_db_output, source, source_by_create_date, source_detail_by_create_date, source_data_no_detail, model_report, model_summary, flag_data, data_cluster_representative, data_explore_cluster, generic)
    ...
    def handleProcessSummary(self, connect_db_output, source, source_by_create_date, source_detail_by_create_date, model_report, model_summary, flag_data, data_cluster_representative, generic):
        self.controller_app["summary"].processSummary(self.controller_app["summary"], connect_db_output, source, source_by_create_date, source_detail_by_create_date, model_report, model_summary, flag_data, data_cluster_representative, generic)

    def handleProcessReport(self, source, model_report, flag_data, data_cluster_representative, generic):
        return self.controller_app["report"].processReport(self.controller_app["report"], source, model_report, flag_data, data_cluster_representative, generic)
    ...
    def executeInsertCluster(self, connect_db_output, flag_data):
        try:
            ...
            if(len(flag_data["sql_for_insert_cluster_execute"]["member_sql"]) > 0):
                ...
                # utilController.genLog(sql_member_insert_cluster, "info")
            ...
            if(len(flag_data["sql_for_insert_cluster_execute"]["member_details_sql"]) > 0):
                ...
                # utilController.genLog(sql_member_detail_insert_cluster, "info")
            ...
            if(len(flag_data["sql_for_insert_cluster_execute"]["summary_sql"]) > 0):
                ...
                # utilController.genLog(sql_summary_insert_cluster, "info")
            ...
            if(len(flag_data["sql_for_insert_cluster_execute"]["summary_details_sql"]) > 0):
                ...
                # utilController.genLog(sql_summary_detail_insert_cluster, "info")
    ...
    def executeInsertNewCluster(self, connect_db_output, flag_data):
        try:
            if(len(flag_data["sql_for_insert_execute"]["report_sql"]) > 0):
                ...
                # utilController.genLog(sql_member_insert, "info")
                ...
                if(len(flag_data["sql_for_insert_execute"]["member_details_sql"]) > 0):
                    ...
                    # utilController.genLog(sql_member_detail_insert, "info")       
                ...
                # utilController.genLog(sql_summary_insert, "info")
                ...
                if(len(flag_data["sql_for_insert_execute"]["summary_details_sql"]) > 0):
                    ...
                    # utilController.genLog(sql_summary_detail_insert, "info")
    ...
    def executeSQLUpdateCluster(self, connect_db_output, flag_data, generic):
        try:     
            if(len(flag_data["sql_for_update_execute"]["member_sql"]) > 0):
                ...
                # utilController.genLog(sql_member_update, "info")
            ...
            if(len(flag_data["sql_for_update_execute"]["member_details_sql"]) > 0):
                ...
                # utilController.genLog(sql_member_detail_update, "info")
            ...
            if(len(flag_data["sql_for_update_execute"]["summary_sql"]) > 0):
                ...
                # utilController.genLog(sql_summary_update, "info")
                ...
                if(my_cursor.rowcount == 0 and len(flag_data["sql_for_update_execute"]["summary_false_details_sql"]) and len(flag_data["sql_for_update_execute"]["summary_false_details_sql_values"])):
                    ...
                    if(connect_db_output.flag_return_data != None):
                        ...
                        if(len(data_update) > 0):
                            my_cursor = connect_db_output.queryExecute(self.controller_app["summary"].genSummaryDetailFalseUpdate(self.controller_app["report"], data_update, generic), None, "")
                # utilController.genLog(sql_summary_detail_update, "info")
            ...
            if(flag_data["flag_update_report"]):
                print("Update report ...")
                flag_data["sql_for_update_execute"] = self.controller_app["report"].genSqlReportUpdateByRepresentativeProcess(self.controller_app["report"], connect_db_output, flag_data["sql_for_update_execute"], flag_data["flag_representative_list_df"], generic)
                sql_report_update = flag_data["sql_for_update_execute"]["report_sql"]
                # utilController.genLog(sql_report_update, "info")
    ...
    def executeSQLForProcess(self, connect_db_output, flag_data, generic):
        try:
            ...
            #     # utilController.genLog(sql_member_detail_delete, "info")
