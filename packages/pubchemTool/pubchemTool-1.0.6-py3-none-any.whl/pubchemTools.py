...
""" Query the pubchem database, and create a record compatible to FURTHRmind
    ...
    Warrany: None
"""
# handle pubchem entries the nice way:
# import pubchempy
#
...
class Pubchem():
    ...
    def cas(self)->list:
        ...
        """
        return get_cas(self._record_)

    @property
    def name(self)->str:
        """ name in the pubchem record
        """
...
def search(search_string: str ='', search_type: str ='name', what: str ='cids')->dict:
    """ Run a search in the pubchem database based on direct web call.
        ...
        what:           what to return? E.g. cids or record
...
def get_cas(pchem: dict, all_elements: bool = False) -> list:
    """ try digging out cas values from the resulted pubchem dict
        obtained using pubchem_search
        Remove deprecated CAS and Related CAs values.

        @param pchem:   the result of a pubchem_search() call
        @param all_elements:     if set True, merge all CAS numbers into a list

        @return: a list of values found
    """
    res = get_value('CAS', pchem)

    if not all_elements:
        kill = get_value('Related CAS', pchem) + \
                    get_value('Deprecated CAS', pchem)
