...
"""%(self.pro_dir, yaml_file, self.pro_dir,
    self.pipe_dir, self.pro_dir, self.pro_dir))
        ...
        fail_msn = "hello, %s 的 上传失败! 路径为 %s ."%(self.contract_number, oss_path)
        fail_talk = ding_talk(fail_msn, self.analyst_phone_number)
...
python3 -m excelParse.upload -i %s/Uplaod/ -o %s 
...
""" %(self.pro_dir,
      self.pipe_dir, yaml_file, self.pro_dir,
      self.pro_dir , oss_path,
      talk,
      fail_talk))
    ...
    def copy_mkdir(self):
        os.system("mkdir -p %s/log/slurmlog" %(self.pro_dir))
        shutil.copy(os.path.join(self.pipe_dir,"pipeline.yaml"),self.pro_dir)