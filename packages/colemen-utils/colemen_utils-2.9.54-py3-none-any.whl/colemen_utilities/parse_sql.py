...
# # import colemen_utilities.files.file as cfu
# import colemen_utilities.file_utils as cfu
# # # from sqlparse import sql
# import colemen_utilities.string_utils as mod
# import colemen_utilities.dict_utils as obj
# import colemen_utilities.list_utils as lu
...
#     # Capture the comment from the end of the string.
#     comment_value = None
#     match = re.findall(r'comment\s+\'(.*)',value,re.IGNORECASE)
#     if match is not None:
#         if len(match)>0:
#             cmt = match[0]
#             value = re.sub(r'comment\s+\'(.*)',"",value,re.IGNORECASE)
#             comment_value = mod.strip(cmt,[" ",",","'"])
...
#             data['keys'] = lu.append(data['keys'],parse_key(line))
#             pk = parse_primary_key(line)
#             if pk is not None:
#                 data['primary_keys'] = lu.append(data['primary_keys'],pk['primary_key'])
#             # data['primary_keys'] = lu.append(data['primary_keys'],parse_primary_key(line))

#         if is_line_constraint(line):
#             data['constraints'] = lu.append(data['constraints'],parse_constraint(line))


#         data['columns'] = lu.append(data['columns'],parse_column_data(line))
...
#     vals = value.split(",")
#     vals = [mod.strip(x,[" "]) for x in vals]
#     return ','.join(vals)
...
#         if mod.array_in_string(["not null","NOT NULL"],s,False):
#             nn_found = True
...
#             col_data['comment'] = mod.strip(cmt,[" "])
#             break
...
#             keys = keys.split(",")
#             keys = [mod.strip(x,[" "]) for x in keys]
#             primary_keys = keys
#         # print(match)
#     return primary_keys
...
#             lu.append(data['tables'],state['data'])
#             continue
...
#             lu.append(data['schemas'],state['data'])
#             continue
