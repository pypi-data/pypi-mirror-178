from BroachRpc.common import GlobalVariable, CommonDef
from BroachRpc.network import MsgHandler
from BroachRpc.threadPool import BroachPool
import socket
import logging
import threading
...
class RpcServer:
    def __init__(self, rpcClient):
        ...
        self.threadPool = BroachPool.Pool(core=5,
                                          threadMax=10,
                                          keepAliveTime=10,
                                          queue=BroachPool.ArrayQueue(
                                              queueMax=10000,
                                              isExcInfo=False,
                                              createThreadThreshold=5),
                                          poolName="rpcServerPool")
        self.broachPool = GlobalVariable.BroachPool
        self.__callBackNES = []
        self.__callBackCIM = []
    ...
    def __NESHandler(self, msg):
        ...
        if msgLen == 1:
            ...
            for fn in self.__callBackNES:
                self.broachPool.submit(fn, msgPart.decode("utf-8"), revIp, int(revPort))
            return
        with self.__NESRevLock:
            ...
            if (len(msgPartDict)-1) == msgLen:
                ...
                #通知上游函数处理
                for fn in self.__callBackNES:
                    self.broachPool.submit(fn, revMsg.decode("utf-8"), revIp, int(revPort))
